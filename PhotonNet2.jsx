...
class NoiseMatrixCalibration {
  ...
  static ABSOLUTE_QBER_ALERT_THRESHOLD = 0.11;
  ...
  static _newestObservedAt(rows) {
    ...
    for (const r of rows || []) {
      if (typeof r.observedAt === "number" && Number.isFinite(r.observedAt)) {
        if (best === null || r.observedAt > best) best = r.observedAt;
      }
    }
    return best;
  }
  ...
  static _resolveTies(rows) {
    const byKm = new Map();
    for (const r of (rows || [])) {
      if (!r || typeof r.km !== "number") continue;
      const prev = byKm.get(r.km);
      if (!prev || r.qberMean > prev.qberMean) byKm.set(r.km, r);
    }
    return [...byKm.values()];
  }
...
class CalibrationRateLimiter {
  static WINDOW_MS = 1000;             // hız sınırı penceresi (1 saniye)
  static MAX_UPDATES_PER_WINDOW = 5;   // pencere başına izin verilen GİRİŞİM sayısı (kabul/red FARK ETMEZ)
  static SUSPENSION_MS = 30000;        // aşım sonrası yetki askıya alma süresi
  ...
  status(key, now = Date.now()) {
    ...
    if (!bucket) return { suspended: false, updatesInWindow: 0, suspendedUntil: null };
...
class LinkRiskReputationEngine {
  static MIN_SAMPLES_FOR_TRUST = 3;  // N_min — bu kadar KABUL EDİLMİŞ örnek birikmeden EMA'ya güvenilmez
  static QUARANTINE_RISK = 0.5;      // n < N_min iken raporlanan nötr (ne güvenli ne tehlikeli) risk
  static EMA_ALPHA = 0.3;            // yeni örneğin ağırlığı; küçük değer = daha yavaş/kararlı, büyük = daha çevik
  ...
  static _versionOf(row) {
    if (typeof row.version === "number" && Number.isFinite(row.version)) return row.version;
    if (typeof row.observedAt === "number" && Number.isFinite(row.observedAt)) return row.observedAt;
    return undefined;
  }
  ...
  shortestPath(src, dst, isNodeLive, weightCtx, policy, excludeLinkKeys) {
    ...
    while (heap.size > 0) {
      ...
      for (const { to, link } of this.neighbors(u)) {
        ...
        if (excludeLinkKeys && excludeLinkKeys.has(NetworkTopology.linkKey(link))) continue;
  ...
  disjointPaths(src, dst, isNodeLive, weightCtx, policy, k = 2) {
    ...
    for (let i = 0; i < k; i++) {
      ...
      for (const step of path) {
        if (step.link) excluded.add(NetworkTopology.linkKey(step.link));
      }
    }
    return results;
  }
...
function routeIsSuspect(route, measuredRisk, threshold = DOS_SUSPECT_RISK_THRESHOLD) {
  if (!route || !measuredRisk) return false;
  for (const lk of route.segs) {
    const r = measuredRisk[`${lk.a}-${lk.b}`] ?? measuredRisk[`${lk.b}-${lk.a}`] ?? 0;
...
function routeCalculationResilient(nodes, links, src, dst, linkLoad, getNode, linkDown, predictiveRisk, measuredRisk) {
  ...
  const excludeLinkKeys = new Set((primary?.segs || []).map(lk => NetworkTopology.linkKey(lk)));
...
function PhotonNet() {
  ...
  async function transmit(){
    ...
    // riski için DEĞİL.
    const measuredRisk = {};
    for (const l of links) measuredRisk[`${l.a}-${l.b}`] = linkRiskReputationEngine.riskForLink(`${l.a}-${l.b}`).risk;
