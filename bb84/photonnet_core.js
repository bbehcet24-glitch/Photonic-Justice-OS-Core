...
const WL = {
    ...
    633: { hex: "#f43f5e", label: "VIS-633", loss: 3.00, r: 54.0 },
};
...
class QuantumKeyDistribution {
    ...
    deriveSiftedKey(bits, totalKm, evesdrop, historicalRng, detectorCtx) {
        ...
        siftedIndices.forEach((origIdx, k) => {
            ...
            if (!kr.ok) {
                ...
                // EK 44 (DetectorNoiseModel): ama karanlık sayım/gündüz arka plan
                // gürültüsü (detectorCtx verilmişse) dedektörün YİNE DE rastgele
                // bir "hayalet" click üretmesine yol açabilir — bu, foton
                // kaybından TAMAMEN BAĞIMSIZ, dedektör-taraflı bir olay. Hayalet
                // click Alice'in bitinden bağımsız rastgele bir değer taşır, bu
                // yüzden sifted kümeye %50 ihtimalle hata olarak katılır — gerçek
                // dedektör davranışıyla tutarlı.
                const phantomP = detectorCtx ? DetectorNoiseModel.phantomClickProbability(!!detectorCtx.isNight) : 0;
                if (phantomP > 0 && bitRng() < phantomP) {
    ...
    deriveSiftedKeyChain(bits, totalKm, nHops, evesdrop, detectorCtx) {
        ...
        for (let h = 0; h < n; h++) {
            ...
            const r = hopQkd.deriveSiftedKey(bits, hopKm, evesdrop, undefined, detectorCtx);
            hops.push({ hop: h, km: hopKm, qber: r.qber, siftedLen: r.siftedKeyBits.length,
                matchRate: r.recon.matchRate, siftedKeyBits: r.siftedKeyBits,
        ...
        return {
            hops, nHops: n, hopKm, bottleneckLen, avgQber, avgMatchRate, worstHop, eavesdropDetected, keyBits,
...
//
// computeRepeaterGain(nm, km, reps) — propPhoton'ın kendi hesaplamasıyla
// BİREBİR AYNI formül, tek bir yerde toplanmış (hem propPhoton hem de
// aşağıdaki estimateLinkSurvival/dynamicRedundancyFor tarafından paylaşılır
// — iki ayrı yerde formülün birbirinden SAPMASI riski ortadan kalkar).
// reps=0 ise her zaman 1.0 (etkisiz) döner — tekrarlayıcısız hatlarda
// hiçbir davranış değişikliği YOKTUR.
function computeRepeaterGain(nm, km, reps) {
    if (!(reps > 0))
        return 1.0;
    const w = WL[nm] || WL[1550];
    const sk = km / (reps + 1);
    const segmentLossDb = w.loss * sk;
    const compensationRatio = Math.min(0.95, 0.5 + reps * 0.15); // reps arttıkça telafi oranı artar
...
function estimateLinkSurvival(nm, km, reps) {
    const gain = computeRepeaterGain(nm, km, reps);
    const sk = reps > 0 ? km / (reps + 1) : km;
    const segT = Math.min(1.0, fiberT(nm, sk) * gain);
    return Math.pow(segT, reps + 1); // tüm segmentlerden ardışık hayatta kalma (ABSORB dışı kayıp modları hariç, yaklaşık üst sınır)
}
function dynamicRedundancyFor(nm, km, reps, targetArrival = 0.995, maxRedundancy = 20) {
    const s = Math.max(0.0005, Math.min(1, estimateLinkSurvival(nm, km, reps)));
    if (s >= 0.85)
 ...
 */
function propPhoton(nm, km, reps, evesdrop, rng, legaCtx) {
    var _a;
    const rand = rng || Math.random;
    const w = WL[nm] || WL[1550];
    const sk = reps > 0 ? km / (reps + 1) : km;
    ...
    // ══════════════════════════════════════════════════════════
    const repeaterGainFactor = computeRepeaterGain(nm, km, reps);
...
class EdgeWeightPolicy {
    /**
     * @param {{ loadFactor?: number }} [opts] - loadFactor: canlı yükün
     *   toplam maliyete katkı ağırlığı (0=yalnız fiziksel, 1=yük baskın).
     *   Varsayılan 0.6 — kullanıcı kararına göre fiziksel VE canlı yük
     *   birlikte belirleyici olsun diye orta-üst bir çarpan seçildi.
     */
    constructor(opts = {}) {
        var _a;
    ...
    computeWeight(link, ctx = {}) {
        ...
        return base * (1 + this.loadFactor * load);
    }
}
 ...
 */
function dijkstra(nodes, links, src, dst, linkLoad) {
    ...
    const weightCtx = linkLoad ? { linkLoad } : undefined;
    return networkTopology.shortestPath(src, dst, (id) => liveIds.has(id), weightCtx);
}
// ── Topoloji ──────────────────────────────────────────────────
const NODES = [
    { id: "IST", label: "İstanbul", x: 355, y: 152, type: "hub", on: true, reps: 3, lat: 41.0, lon: 28.9 },
    { id: "ANK", label: "Ankara", x: 510, y: 208, type: "hub", on: true, reps: 2, lat: 39.9, lon: 32.9 },
    { id: "IZM", label: "İzmir", x: 228, y: 262, type: "node", on: true, reps: 1, lat: 38.4, lon: 27.1 },
    { id: "BUR", label: "Bursa", x: 308, y: 192, type: "node", on: true, reps: 1, lat: 40.2, lon: 29.1 },
    { id: "ADA", label: "Adana", x: 520, y: 318, type: "node", on: true, reps: 1, lat: 37.0, lon: 35.3 },
    { id: "TZN", label: "Trabzon", x: 598, y: 132, type: "node", on: true, reps: 1, lat: 41.0, lon: 39.7 },
    { id: "KON", label: "Konya", x: 432, y: 288, type: "node", on: true, reps: 1, lat: 37.9, lon: 32.5 },
    { id: "SAM", label: "Samsun", x: 518, y: 144, type: "node", on: true, reps: 1, lat: 41.3, lon: 36.3 },
    { id: "ANT", label: "Antalya", x: 340, y: 362, type: "node", on: true, reps: 1, lat: 36.9, lon: 30.7 },
    { id: "DIY", label: "Diyarbakır", x: 628, y: 268, type: "node", on: true, reps: 1, lat: 37.9, lon: 40.2 },
    { id: "EZR", label: "Erzurum", x: 660, y: 188, type: "node", on: true, reps: 1, lat: 39.9, lon: 41.3 },
    { id: "VAN", label: "Van", x: 706, y: 244, type: "node", on: false, reps: 0, lat: 38.5, lon: 43.4 },
    { id: "GAZ", label: "Gaziantep", x: 568, y: 344, type: "node", on: true, reps: 1, lat: 37.1, lon: 37.4 },
    { id: "MER", label: "Mersin", x: 482, y: 346, type: "node", on: true, reps: 0, lat: 36.8, lon: 34.6 },
    { id: "ATH", label: "Atina", x: 162, y: 340, type: "intl", on: true, reps: 3, lat: 37.9, lon: 23.7 },
    { id: "SOF", label: "Sofya", x: 240, y: 122, type: "intl", on: true, reps: 2, lat: 42.7, lon: 23.3 },
    { id: "BAG", label: "Bağdat", x: 682, y: 378, type: "intl", on: true, reps: 2, lat: 33.3, lon: 44.4 },
    { id: "CAI", label: "Kahire", x: 408, y: 452, type: "intl", on: true, reps: 3, lat: 30.1, lon: 31.2 },
];
const LINKS = [
    { a: "IST", b: "BUR", km: 85, nm: 1550 }, { a: "IST", b: "ANK", km: 452, nm: 1550 }, { a: "IST", b: "SAM", km: 750, nm: 1310 },
    { a: "IST", b: "SOF", km: 560, nm: 1310 }, { a: "BUR", b: "IZM", km: 250, nm: 1550 }, { a: "BUR", b: "ANK", km: 380, nm: 1550 },
    { a: "IZM", b: "ANT", km: 480, nm: 1310 }, { a: "IZM", b: "ATH", km: 650, nm: 1310 }, { a: "ANK", b: "KON", km: 260, nm: 1550 },
    { a: "ANK", b: "SAM", km: 420, nm: 1550 }, { a: "ANK", b: "ADA", km: 490, nm: 1310 }, { a: "ANK", b: "TZN", km: 620, nm: 1310 },
    { a: "ANK", b: "EZR", km: 870, nm: 1310 }, { a: "KON", b: "ANT", km: 310, nm: 1550 }, { a: "KON", b: "ADA", km: 350, nm: 1550 },
    { a: "ADA", b: "DIY", km: 360, nm: 850 }, { a: "ADA", b: "MER", km: 75, nm: 1550 }, { a: "ADA", b: "GAZ", km: 220, nm: 1550 },
    { a: "TZN", b: "EZR", km: 325, nm: 1550 }, { a: "SAM", b: "TZN", km: 360, nm: 1550 }, { a: "DIY", b: "EZR", km: 300, nm: 850 },
    { a: "DIY", b: "GAZ", km: 185, nm: 1550 }, { a: "EZR", b: "VAN", km: 215, nm: 850 }, { a: "VAN", b: "BAG", km: 640, nm: 850 },
    { a: "GAZ", b: "BAG", km: 560, nm: 1310 }, { a: "MER", b: "CAI", km: 1100, nm: 1310 }, { a: "ATH", b: "CAI", km: 1750, nm: 1310 },
 ...
 */
function routeCalculation(nodes, links, src, dst, linkLoad, getNode) {
    const path = dijkstra(nodes, links, src, dst, linkLoad);
    if (!path)
        return null;
...
function physicalSimulation(segs, bits, entanglementSeed, evesdrop, getNode, legaOpts) {
    ...
    for (let si = 0; si < segs.length; si++) {
        ...
        const redundancy = (legaOpts === null || legaOpts === void 0 ? void 0 : legaOpts.dynamicFecEnabled)
            ? dynamicRedundancyFor(lk.nm, lk.km, rp)
            : 1;
        ...
        for (let i = 0; i < recv.length; i++) {
            ...
            for (let c = 0; c < redundancy && successes.length < VOTE_TARGET; c++) {
                ...
                const photonCtx = legaCtx && nodeCoeffCache
                    ...
                    : legaCtx;
                const attempt = propPhoton(lk.nm, lk.km, rp, evesdrop && si === Math.floor(segs.length / 2) && c === 0, bitRng, photonCtx);
                lastAttempt = attempt;
                if (attempt.ok)
            ...
            if (r.ok && legaOpts && nd) {
                ...
                const { gateResult: gate, latencyNs, interpolatedCoeffs } = inMemoryFabric.evaluateInPlace(nd.id, lk.nm, lk.km, lowPriority, legaCtx, gateBitRng);
...
function PhotonNet() {
    ...
    async function transmit() {
        ...
        const route = routeCalculation(nodes, links, src, dst, linkLoad, getNode);
        if (!route) {
        ...
        let recon, siftedKeyBits, qber, eavesdropDetected, lostCount, darkClickCount, qkdChain = null;
        if (usingTrustedChain) {
            ...
            ({ recon, siftedKeyBits, qber, eavesdropDetected, lostCount, darkClickCount } =
    ...
    return (React.createElement("div", { style: { minHeight: "100vh", background: "#020409", color: "#e2e8f0",
  ...
  `),
        React.createElement("div", { style: { height: 46, background: "#030610", borderBottom: "1px solid #0a162888",
            ...
            React.createElement("div", { style: { display: "flex", gap: 2, marginLeft: "auto" } }, [["MAP", "🌐 AĞ"], ["SAT", "🛰 UYDU"], ["TRF", "📊 TRAFİK"], ["TML", "⧉ ZAMAN ÇİZ."], ["GRAPH", "🕸 GRAPH"], ["LOG", "📋 KAYIT"]].map(([k, l]) => (React.createElement("button", { key: k, className: `ptab ${tab === k ? "on" : ""}`, onClick: () => setTab(k) }, l)))),
                ...
                tab === "MAP" && (React.createElement("div", { style: { flex: 1, position: "relative", zIndex: 1 } },
                    ...
                    path && (React.createElement("div", { style: { position: "absolute", bottom: 10, left: "50%", transform: "translateX(-50%)",
                        ...
                        React.createElement("span", null,
                            path.length - 1,
                            " HOP"))),
                    React.createElement("svg", { width: "100%", height: "100%", viewBox: "0 0 780 490", style: { display: "block" } },
                        React.createElement("defs", null,
                            ...
                            React.createElement("filter", { id: "gl2" },
                                ...
                                React.createElement("feMerge", null,
                                    React.createElement("feMergeNode", { in: "b" }),
                                    React.createElement("feMergeNode", { in: "SourceGraphic" })))),
                        Array.from({ length: 20 }, (_, i) => (React.createElement("line", { key: `v${i}`, x1: i * 40, y1: 0, x2: i * 40, y2: 490, stroke: "#00d4ff", strokeWidth: .3, strokeOpacity: .03 }))),
                        Array.from({ length: 14 }, (_, i) => (React.createElement("line", { key: `h${i}`, x1: 0, y1: i * 38, x2: 780, y2: i * 38, stroke: "#00d4ff", strokeWidth: .3, strokeOpacity: .03 }))),
                        memoryTraces.map(mt => {
...
module.exports = { propPhoton, QuantumKeyDistribution, mulberry32, combineSeed, WL, fiberT, lega,
  LinkGradedEavesdropThresholdAlgorithm, eavesdropProbability, ScintillationModel, ScintillationEngine,
  scintillationEngine, PointingBudget, DetectorNoiseModel, AtmosphericWindowModel, satQ,
  physicalSimulation, hEnc, hDec, t2b, b2t, computeRepeaterGain, estimateLinkSurvival, dynamicRedundancyFor };
