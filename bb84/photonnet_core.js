...
class AtmosphericWindowModel {
    ...
    static windowFactor(nm, conditions = {}) {
        var _a, _b;
        const turbidity = Math.max(0, Math.min(1, (_a = conditions.turbidity) !== null && _a !== void 0 ? _a : 0.2));
        const rangeKm = (_b = conditions.rangeKm) !== null && _b !== void 0 ? _b : 500;
...
function combineSeed(entanglementSeed, segIndex, bitIndex, copy = 0) {
    ...
    if (copy)
        h = Math.imul(h ^ (copy * 0x2545F491 + 1), 0x27d4eb2f);
    h = h ^ (h >>> 16);
    return h >>> 0;
}
// ══════════════════════════════════════════════════════════════
...
function deriveOtpKeyBits(siftedBits, neededLen, seed) {
    ...
    while (key.length < neededLen)
        key.push(expandRng() < 0.5 ? 0 : 1);
    return key.slice(0, neededLen);
}
...
class QuantumKeyDistribution {
    ...
    deriveSiftedKeyChain(bits, totalKm, nHops, evesdrop, detectorCtx) {
        ...
        for (let h = 0; h < n; h++) {
            ...
            const hopBits = bits.length >= STAT_BATCH_MIN ? bits : (() => {
                ...
                for (let i = 0; i < STAT_BATCH_MIN; i++)
                    arr[i] = i < bits.length ? bits[i] : (expandRng() < 0.5 ? 0 : 1);
                return arr;
            })();
    ...
    static computeRawKeyRate(siftedKeyBitCount, matchRate) {
        ...
        let label;
        if (rawKeyRateBps >= 1e6)
            label = `${(rawKeyRateBps / 1e6).toFixed(2)} Mbit/s`;
        else if (rawKeyRateBps >= 1e3)
            label = `${(rawKeyRateBps / 1e3).toFixed(1)} kbit/s`;
        else
            label = `${rawKeyRateBps.toFixed(0)} bit/s`;
        return {
            rawKeyRateBps,
...
function toeplitzPackBits(bitsArray, offset, len) {
    offset = offset || 0;
    len = (len == null) ? (bitsArray.length - offset) : len;
    const words = new Uint32Array((len + 31) >>> 5);
    for (let i = 0; i < len; i++) {
        if (bitsArray[offset + i])
            words[i >>> 5] |= (1 << (i & 31));
    }
    return words;
}
...
class ToeplitzAsyncEngine {
    static selfTest() {
        ...
        try {
            ...
            for (const [n, ell] of trials) {
                ...
                if (naive.length !== packed.length || !naive.every((v, i) => v === packed[i])) {
                    ok = false;
                    break;
                }
            }
            ...
            if (typeof console !== "undefined")
                console.error("[ToeplitzAsyncEngine] öz-test istisna fırlattı, güvenli moda geçiliyor:", e);
        }
        return this._selfTestOk;
    }
    ...
    static _hashPackedSyncRaw(inputBits, ell, seedBits, rowStart, rowCount) {
        ...
        for (let w = 0; w < n; w++)
            reversedInput[w] = inputBits[n - 1 - w];
...
`;
    ...
    static _ensurePool() {
        if (this._pool === "unavailable" || this._pool)
            return this._pool;
        try {
            ...
            const poolSize = Math.max(1, Math.min(this.MAX_WORKERS, (typeof navigator !== "undefined" && navigator.hardwareConcurrency) || 4));
            ...
            if (typeof console !== "undefined")
                console.warn("[ToeplitzAsyncEngine] Worker havuzu oluşturulamadı, ana-iş-parçacığı parçalı geri düşüşe geçiliyor:", e);
            this._pool = "unavailable";
        }
        return this._pool;
    }
    ...
    static _runOnWorker(worker, task) {
        return new Promise((resolve, reject) => {
            ...
            const onMsg = (ev) => {
                if (ev.data.taskId !== taskId)
                    return; // bu worker'a AYNI ANDA yalnızca bir görev veriyoruz, ama güvenlik payı
                worker.removeEventListener("message", onMsg);
                worker.removeEventListener("error", onErr);
                if (ev.data.ok)
                    resolve(ev.data);
                else
                    reject(new Error(ev.data.error || "worker hatası"));
            };
            const onErr = (err) => {
                worker.removeEventListener("message", onMsg);
                worker.removeEventListener("error", onErr);
    ...
    static async _hashViaWorkerPool(inputBits, ell, seedBits, pool) {
        ...
        for (let w = 0; w < n; w++)
            reversedInputFull[w] = inputBits[n - 1 - w];
    ...
    static async _hashChunkedMainThread(inputBits, ell, seedBits, useNaive) {
        ...
        for (let rowStart = 0; rowStart < ell; rowStart += chunkRows) {
            ...
            const rows = useNaive
                ? QKDSecurityProof._toeplitzHashNaiveReference(inputBits, rowCount, seedBits.slice(rowStart, rowStart + rowCount + inputBits.length - 1))
                : this._hashPackedSyncRaw(inputBits, ell, seedBits, rowStart, rowCount);
            for (let i = 0; i < rows.length; i++)
                out[rowStart + i] = rows[i];
    ...
    static async hashAsync(inputBits, ell, seedRng) {
        ...
        if (pool && pool !== "unavailable") {
            try {
                ...
                if (typeof console !== "undefined")
                    console.warn("[ToeplitzAsyncEngine] Worker havuzu hata verdi, ana-iş-parçacığı parçalı geri düşüşe geçiliyor:", e);
...
ToeplitzAsyncEngine.SYNC_THRESHOLD_OPS = 5e8;
ToeplitzAsyncEngine.MAX_WORKERS = 8;
ToeplitzAsyncEngine.CHUNK_ROWS_FALLBACK = 4096; // Worker yoksa ana iş parçacığında nefes-payı chunk boyutu
ToeplitzAsyncEngine._selfTestOk = null; // null=henüz test edilmedi, true/false=sonuç
ToeplitzAsyncEngine._pool = null; // { workers:[Worker], workerUrl:string } | null | "unavailable"
// ══════════════════════════════════════════════════════════
class QKDSecurityProof {
    ...
    static secureKeyLengthWithMu(n, qBit, mu, opts = {}) {
        var _a, _b, _c;
        const epsCor = (_a = opts.epsCor) !== null && _a !== void 0 ? _a : 1e-15; // doğrulama (correctness) başarısızlık olasılığı
        const epsPA = (_b = opts.epsPA) !== null && _b !== void 0 ? _b : 1e-10; // gizlilik yükseltme başarısızlık olasılığı
        const fEC = (_c = opts.fEC) !== null && _c !== void 0 ? _c : 1.16; // Cascade tipi hata düzeltme verimsizliği (gerçekçi literatür değeri)
        if (n <= 0 || qBit == null || Number.isNaN(qBit)) {
            return { n, qBit: qBit !== null && qBit !== void 0 ? qBit : null, mu, ell: 0, secure: false, reason: "anahtar örneklemi boş" };
        }
    ...
    static secureKeyLength(n, qBit, opts = {}) {
        var _a;
        const epsPE = (_a = opts.epsPE) !== null && _a !== void 0 ? _a : 1e-10; // parametre kestirimi başarısızlık olasılığı
        if (n <= 0 || qBit == null || Number.isNaN(qBit)) {
            return { n, qBit: qBit !== null && qBit !== void 0 ? qBit : null, ell: 0, secure: false, reason: "sifted key boş" };
        }
...
class CascadeReconciliation {
    ...
    static reconcile(aliceBits, bobBits, estimatedQber, rng, opts = {}) {
        ...
        let leakedBits = 0;
        const numPasses = (_a = opts.numPasses) !== null && _a !== void 0 ? _a : 4;
...
class LDPCReconciliation {
    static _constructRegularLDPC(n, m, colWeight, rng) {
        ...
        for (let v = 0; v < n; v++) {
            ...
            for (const c of chosen) {
                varToChecks[v].push(c);
                checkToVars[c].push(v);
            }
        }
    ...
    static _computeSyndrome(H, bits) {
        ...
        for (let c = 0; c < H.m; c++) {
            let acc = 0;
            for (const v of H.checkToVars[c])
                acc ^= bits[v];
            s[c] = acc;
        }
        return s;
    ...
    static _bpDecode(H, receivedBits, syndrome, qber, maxIter) {
        ...
        let vToC = {}, cToV = {};
        for (let v = 0; v < n; v++)
            for (const c of H.varToChecks[v])
                vToC[`${v}_${c}`] = channelLLR[v];
        for (let c = 0; c < m; c++)
            for (const v of H.checkToVars[c])
                cToV[`${c}_${v}`] = 0;
        ...
        for (let iter = 0; iter < maxIter; iter++) {
            ...
            for (let v = 0; v < n; v++) {
                ...
                totalLLR[v] = total;
                for (const c of checks)
                    newVToC[`${v}_${c}`] = this._clampLLR(total - cToV[`${c}_${v}`]);
            }
            vToC = newVToC;
    ...
    static reconcile(aliceBits, bobBits, estimatedQber, rng, opts = {}) {
        ...
        const rateMargin = (_a = opts.rateMargin) !== null && _a !== void 0 ? _a : 2.0;
        let m = Math.ceil(n * h2(p) * rateMargin);
        m = Math.max(4, Math.min(n - 1, m));
        const colWeight = (_b = opts.colWeight) !== null && _b !== void 0 ? _b : 3;
        const H = this._constructRegularLDPC(n, m, colWeight, rng);
        const syndrome = this._computeSyndrome(H, aliceBits);
        const { correctedBits, converged, iterations } = this._bpDecode(H, bobBits, syndrome, p, (_c = opts.maxIter) !== null && _c !== void 0 ? _c : 100);
        let residualErrors = 0;
        for (let i = 0; i < n; i++)
            if (aliceBits[i] !== correctedBits[i])
                residualErrors++;
        return {
...
class ProductionSecurityAudit {
    ...
    static audit(block, opts = {}) {
        ...
        const epsPE = (_a = opts.epsPE) !== null && _a !== void 0 ? _a : 1e-10;
        const epsCor = (_b = opts.epsCor) !== null && _b !== void 0 ? _b : 1e-15;
        const epsPA = (_c = opts.epsPA) !== null && _c !== void 0 ? _c : 1e-10;
        if (!block || block.n == null || block.n <= 0)
            throw new Error("ProductionSecurityAudit.audit: block.n (anahtar örneklemi) gerekli ve >0 olmalı");
        if (!block.test || !block.test.records)
            throw new Error("ProductionSecurityAudit.audit: block.test.records (ham test verisi) gerekli — özet bir 'qEstimated' YETERLİ DEĞİLDİR");
        const qberResult = this.computeQberFromRaw(block.test.records);
        const n = block.n, k = qberResult.k;
        const gain = this.computeGain(n, k, (_d = block.sentPulses) !== null && _d !== void 0 ? _d : null);
        ...
        if (recon && recon.converged === false) {
            bound = {
                qPhUpper: Math.min(0.5, ((_e = qberResult.qber) !== null && _e !== void 0 ? _e : 0) + mu),
            ...
            const ecOpts = { ...opts };
            if (recon && recon.leakedBits != null)
                ecOpts.realLeakEC = recon.leakedBits;
        ...
        return {
            routeKey: (_f = block.routeKey) !== null && _f !== void 0 ? _f : null, blockIndex: (_g = block.blockIndex) !== null && _g !== void 0 ? _g : null,
            n, k, sentPulses: (_h = block.sentPulses) !== null && _h !== void 0 ? _h : null,
...
class KeyPoolBuffer {
    ...
    feed(src, dst, siftedKeyBits, bobKeyBits, entanglementSeed, sentPulses) {
        ...
        if (!this.pools[key])
            this.pools[key] = { bitPairs: [], sentPulses: 0, totalFed: 0, blocksFinalized: 0 };
    ...
    _finalizeBlock(routeKey, pool, entanglementSeed) {
        ...
        const qberEstForEC = (_a = pe.qEstimated) !== null && _a !== void 0 ? _a : 0.02; // PE'nin tahmini QBER'i — gerçek protokollerde blok/syndrome boyutu buna göre ayarlanır
...
KeyPoolBuffer.BLOCK_THRESHOLD_BITS = 4000; // blok tamamlanma eşiği (gerçek sistemler 10⁴-10⁶ kullanır; burada bu simülatörün mesaj-başına-onlarca/yüzlerce-bit verimine göre demo ölçeğinde seçildi)
KeyPoolBuffer.TEST_FRACTION = 0.2; // PE için ayrılan pay — literatürde tipik %10-%30 aralığından, istatistiksel güç/anahtar-verimi dengesi için
...
class KeyDeliveryStore {
    ...
    register(routeKey, finalKeyBits, blockIndex, idRng) {
        if (!finalKeyBits || !finalKeyBits.length)
            return null;
        const entry = {
        ...
        if (!this.byRoute[routeKey])
            this.byRoute[routeKey] = [];
...
class ClassicalAuthChannel {
    constructor(bootstrapSeed = 0xACE55ACE) {
        ...
        this.pool = [];
        for (let i = 0; i < ClassicalAuthChannel.MIN_POOL_BITS; i++)
            this.pool.push(rng() < 0.5 ? 0 : 1);
    ...
    static _computeTag(messageBits, authKeyBits64) {
        let coeff = 0;
        for (let i = 0; i < 32; i++)
            coeff = ((coeff << 1) | (authKeyBits64[i] || 0)) >>> 0;
        if (coeff === 0)
            coeff = 1; // 0 katsayı dejenere hash üretir — kaçın
        let hash = 0;
        for (const b of messageBits)
            hash = (Math.imul(hash ^ b, coeff) >>> 0);
        let mask = 0;
        for (let i = 0; i < 32; i++)
            mask = ((mask << 1) | (authKeyBits64[32 + i] || 0)) >>> 0;
        return (hash ^ mask) >>> 0;
    }
...
ClassicalAuthChannel.AUTH_TAG_BITS = 32; // MAC etiketi uzunluğu (bit) — pratik sistemlerde tipik 32-128bit
ClassicalAuthChannel.MIN_POOL_BITS = 4096; // başlangıç "tohum" kimlik doğrulama anahtar havuzu
...
function openDB() {
    return new Promise((resolve, reject) => {
        ...
        req.onsuccess = () => {
            if (settled)
                return; // zaman aşımı zaten tetiklendiyse geç gelen sonucu yok say
            settled = true;
...
async function encryptWithWebCrypto(obj) {
    ...
    if (!key)
        return null; // Web Crypto desteklenmiyor
...
function hDec(bits, withLog = false, bypass = false) {
    ...
    for (let i = 0, blockIdx = 0; i < bits.length; i += 7, blockIdx++) {
        ...
        if (corrected)
            b[s - 1] ^= 1;
        if (withLog)
            syndromeLog.push({ blockIdx, syndrome: s, corrected, bypassed: wouldCorrect && bypass, bitPos: corrected ? s - 1 : null });
...
function buildQuantumStateLog(syndromeLog, entanglementSeed) {
    return syndromeLog.map(entry => {
        ...
        return ({
            ...
            stateLabel: (_a = QSTATE_LABELS[entry.syndrome]) !== null && _a !== void 0 ? _a : "|???⟩",
...
function correlateErrorMatrices(primeEc, mirrorEc) {
    ...
    if (allTypes.length === 0)
        return { matrix: [], correlation: null };
    const primeVec = allTypes.map(t => { var _a; return (_a = primeEc[t]) !== null && _a !== void 0 ? _a : 0; });
    const mirrorVec = allTypes.map(t => { var _a; return (_a = mirrorEc[t]) !== null && _a !== void 0 ? _a : 0; });
...
function buildTelemetryPayload({ entanglementSeed, quantumStateLog, chirality, tabId, role, qber }) {
    var _a, _b, _c;
    const lastSyndrome = (quantumStateLog === null || quantumStateLog === void 0 ? void 0 : quantumStateLog.length) ? quantumStateLog[quantumStateLog.length - 1].syndrome : 0;
    const globalEntropy = shannonEntropy((quantumStateLog || []).reduce((h, s) => { var _a; h[s.syndrome] = ((_a = h[s.syndrome]) !== null && _a !== void 0 ? _a : 0) + 1; return h; }, {}));
    return {
        seed: entanglementSeed,
        stateCount: (_a = quantumStateLog === null || quantumStateLog === void 0 ? void 0 : quantumStateLog.length) !== null && _a !== void 0 ? _a : 0,
        // Durum dağılımı özeti (tam log değil — bant genişliği için özet)
        stateHistogram: (quantumStateLog || []).reduce((h, s) => {
            var _a;
            h[s.syndrome] = ((_a = h[s.syndrome]) !== null && _a !== void 0 ? _a : 0) + 1;
            return h;
        }, {}),
        chiralityState: (_b = chirality === null || chirality === void 0 ? void 0 : chirality.chiralityState) !== null && _b !== void 0 ? _b : null,
        symmetryRatio: (_c = chirality === null || chirality === void 0 ? void 0 : chirality.symmetryRatio) !== null && _c !== void 0 ? _c : null,
        // EK 34: kullanıcının belirttiği tam şema
        sender_tab_id: tabId !== null && tabId !== void 0 ? tabId : null,
        local_role: role !== null && role !== void 0 ? role : "Primary_Observer",
        ...
        qber: qber !== null && qber !== void 0 ? qber : null, // EK 37: alıcı tarafta taşıyıcı/modülatör frekansını güncellemek için
    };
}
...
function analyzeManyWorlds(timeline) {
    ...
    timeline.forEach(t => {
        Object.entries(t.result.ec || {}).forEach(([k, v]) => {
            var _a;
            globalEc[k] = ((_a = globalEc[k]) !== null && _a !== void 0 ? _a : 0) + v;
        });
    });
    ...
    timeline.forEach(t => {
        (t.segs || []).forEach(seg => {
            if (!seg)
                return;
            if (!nmStats[seg.nm])
                nmStats[seg.nm] = { count: 0, berSum: 0 };
    ...
    return {
        ...
        successRate,
        entanglementDensity,
        mostStableWavelength: (_b = (_a = nmRanking[0]) === null || _a === void 0 ? void 0 : _a.nm) !== null && _b !== void 0 ? _b : null,
        leastStableWavelength: (_d = (_c = nmRanking[nmRanking.length - 1]) === null || _c === void 0 ? void 0 : _c.nm) !== null && _d !== void 0 ? _d : null,
    };
}
function b2t(b) {
    let s = "";
...
class LinkGradedEavesdropThresholdAlgorithm {
    ...
    computeThresholds(nm, km, ctx = {}, coeffOverride) {
        ...
        const key = LinkGradedEavesdropThresholdAlgorithm.bucketKey(nm, km);
        const fatigue = (_b = (_a = this.fatigueMap.get(key)) === null || _a === void 0 ? void 0 : _a.count) !== null && _b !== void 0 ? _b : 0;
        const load = Math.max(0, Math.min(1, (_c = ctx.linkLoad) !== null && _c !== void 0 ? _c : 0));
        const histBer = Math.max(0, Math.min(1, (_d = ctx.historicalBer) !== null && _d !== void 0 ? _d : 0));
        const alpha = (_e = coeffOverride === null || coeffOverride === void 0 ? void 0 : coeffOverride.alpha) !== null && _e !== void 0 ? _e : this.alpha;
        const beta = (_f = coeffOverride === null || coeffOverride === void 0 ? void 0 : coeffOverride.beta) !== null && _f !== void 0 ? _f : this.beta;
        ...
        const scatterDeathProb = Math.max(0.25, Math.min(0.85, 0.55 + (histBer - 0.1) * alpha));
...
class NoiseMatrixCalibration {
    constructor(data, meta = {}) {
        ...
        this.meta = { authenticated: (_a = meta.authenticated) !== null && _a !== void 0 ? _a : "internal", rejectedRows: (_b = meta.rejectedRows) !== null && _b !== void 0 ? _b : [] };
    }
    ...
    static _sanitizeRiskTable(table) {
        ...
        for (const row of table) {
            ...
            const reasons = [];
            if (typeof km !== "number" || !Number.isFinite(km) || km < kmLo || km > kmHi)
                reasons.push(`km=${km} makul aralık dışı [${kmLo},${kmHi}]`);
            if (typeof q !== "number" || !Number.isFinite(q) || q < qLo || q > qHi)
                reasons.push(`qberMean=${q} makul aralık dışı [${qLo},${qHi}]`);
            if (reasons.length) {
                rejected.push({ row, reasons });
                if (typeof console !== "undefined")
                    console.warn(`[NoiseMatrixCalibration] satır REDDEDİLDİ: ${reasons.join("; ")}`, row);
            }
            else {
                clean.push(row);
            }
    ...
    static _finishLoad(obj, authenticated) {
        const { clean, rejected } = NoiseMatrixCalibration._sanitizeRiskTable(obj === null || obj === void 0 ? void 0 : obj.measuredRiskByDistanceKm);
    ...
    static _canonicalize(obj) {
        if (Array.isArray(obj))
            return "[" + obj.map(NoiseMatrixCalibration._canonicalize).join(",") + "]";
    ...
    static async verifyAndLoad(payload, signatureHex, cryptoKey, opts = {}) {
        if (!signatureHex) {
            if (opts.allowUnauthenticated) {
                if (typeof console !== "undefined")
                    console.warn("[NoiseMatrixCalibration] İMZASIZ kalibrasyon KASITLI OLARAK kabul edildi (allowUnauthenticated=true) — ÜRETİMDE KULLANILMAMALI.");
        ...
        if (!ok)
            throw new Error("NoiseMatrixCalibration.verifyAndLoad: İMZA DOĞRULAMASI BAŞARISIZ — kalibrasyon REDDEDİLDİ (veri değiştirilmiş veya yanlış anahtarla imzalanmış olabilir)");
    ...
    static _interpolate(table, km) {
        ...
        if (km >= sorted[sorted.length - 1].km)
            return norm(sorted[sorted.length - 1].qberMean);
    ...
    getCalibratedLossDbPerKm(fallback) {
        var _a;
        const v = (_a = this.data.attenuationDbPerKm) === null || _a === void 0 ? void 0 : _a.measuredMean;
    ...
    getCalibratedDarkRateHz(fallback) {
        var _a;
        const v = (_a = this.data.darkRateHz) === null || _a === void 0 ? void 0 : _a.measuredMean;
        const [lo, hi] = NoiseMatrixCalibration.VALID_DARK_RATE_HZ_RANGE;
        return (typeof v === "number" && Number.isFinite(v) && v >= lo && v <= hi) ? v : fallback;
    }
    isRealHardware() { return !!this.data.sourceIsRealHardware; }
}
NoiseMatrixCalibration.DEFAULT_CALIBRATION = {
...
NoiseMatrixCalibration.VALID_KM_RANGE = [0, 20000]; // kıtalar arası denizaltı kablosu mertebesi
...
class NodeTransitGate {
    ...
    evaluate(nodeId, nm, km, lowPriority, legaCtx, rng) {
        ...
        const networkBusy = ((_a = legaCtx === null || legaCtx === void 0 ? void 0 : legaCtx.linkLoad) !== null && _a !== void 0 ? _a : 0) > 0.6; // "sistem çok yoğun" eşiği
        if (V < T) {
...
class ChaosSuppressor {
    ...
    sweep(ghostPkts, activeReplayIds) {
        ...
        if (orphans.length === 0)
            return { orphanCount: 0, purgeIds: [], antiPackets: this.activeAntiPackets() };
...
class SoftLandingFilter {
    ...
    evaluate(packets, occupancy) {
        ...
        for (const p of packets) {
            ...
            if (strength <= this.EVICT_STRENGTH_FLOOR)
                toEvict.push(p.id);
        }
        if (mode === "graceful")
            this.stats.gracefulEvictions += toEvict.length;
        else
            this.stats.emergencyDrops += toEvict.length;
        return {
            mode,
            evictIds: toEvict,
...
class BurstTrafficGenerator {
    ...
    _plateauProfile(t, peak) {
        ...
        if (t < rampFrac)
            return peak * (t / rampFrac);
        if (t > 1 - rampFrac)
            return peak * ((1 - t) / rampFrac);
        return peak;
    }
    ...
    async generateBurst({ profile = "spike", peak = 0.95, durationMs = 3000, linkKeys = ["BURST-TEST"] } = {}, setLinkLoad, onTick) {
        ...
        try {
            while (this.running && this._cancelToken === token) {
                ...
                let value;
                if (profile === "plateau")
                    value = this._plateauProfile(t, peak);
                else if (profile === "sawtooth")
                    value = this._sawtoothProfile(t, peak);
                else
                    value = this._spikeProfile(t, peak);
                ...
                onTick === null || onTick === void 0 ? void 0 : onTick(value);
                if (t >= 1)
                    break;
            }
        }
...
class NodeWatchdog {
    ...
    async triggerAnomaly({ nodeId, type, healMs } = {}, io) {
        ...
        if (this._active.has(nodeId))
            return; // zaten karantinada — tekrar tetikleme yok
        const durations = { lockup: 4000, leak: 6000, unresponsive: 3500 };
        const dur = (_a = healMs !== null && healMs !== void 0 ? healMs : durations[type]) !== null && _a !== void 0 ? _a : 4000;
        const labels = {
        ...
        setNodeHealth(prev => {
            ...
            return ({ ...prev, [nodeId]: {
                    ...(prev[nodeId] || {}),
                    status: "CRIT",
                    cpu: type === "lockup" ? 100 : ((_b = (_a = prev[nodeId]) === null || _a === void 0 ? void 0 : _a.cpu) !== null && _b !== void 0 ? _b : 30),
                    mem: type === "leak" ? 97 : ((_d = (_c = prev[nodeId]) === null || _c === void 0 ? void 0 : _c.mem) !== null && _d !== void 0 ? _d : 35),
                    heartbeat: (type === "unresponsive" || type === "lockup") ? 0 : ((_f = (_e = prev[nodeId]) === null || _e === void 0 ? void 0 : _e.heartbeat) !== null && _f !== void 0 ? _f : 1),
                } });
        });
        ...
        openDrillDown === null || openDrillDown === void 0 ? void 0 : openDrillDown(nodeId);
        ...
        if (((_b = this._active.get(nodeId)) === null || _b === void 0 ? void 0 : _b.token) !== token)
            return; // bu arada erken iyileştirilmiş/iptal edilmiş
        ...
        if (((_c = this._active.get(nodeId)) === null || _c === void 0 ? void 0 : _c.token) !== token)
            return;
        ...
        onHealed === null || onHealed === void 0 ? void 0 : onHealed(nodeId);
    }
    ...
    forceHealNow(nodeId) {
        ...
        if (rec === null || rec === void 0 ? void 0 : rec.resolveEarly) {
    ...
    async triggerPreventiveMaintenance({ nodeId, reason, healMs = 2600 } = {}, io) {
        ...
        if (this.isBusy(nodeId))
            return; // reaktif bir anomali VEYA başka bir proaktif döngü zaten sürüyor
        const startedAt = Date.now();
        const token = {};
        ...
        if (((_a = this._preventive.get(nodeId)) === null || _a === void 0 ? void 0 : _a.token) !== token)
            return;
        const downMs = Date.now() - startedAt;
        setNodes(prev => prev.map(n => n.id === nodeId ? { ...n, on: true } : n));
...
class PredictiveTelemetryEngine {
    ...
    constructor(opts = {}) {
        var _a, _b, _c, _d;
        this.windowSize = (_a = opts.windowSize) !== null && _a !== void 0 ? _a : 24;
        this.smaWindow = (_b = opts.smaWindow) !== null && _b !== void 0 ? _b : 8;
        this.horizonMs = (_c = opts.horizonMs) !== null && _c !== void 0 ? _c : 300000;
        this.emaAlpha = (_d = opts.emaAlpha) !== null && _d !== void 0 ? _d : null;
    ...
    analyze(key, { warnThreshold = 0.55, critThreshold = 0.85, nowMs } = {}) {
        ...
        if (!buf || buf.length < 6)
            return null; // yeterli örnek yok — henüz tahmin yapılamaz (erken/az örnekli regresyon güvenilmez)
        ...
        if (predicted >= critThreshold) {
            ...
            if (etaMs != null && etaMs > this.horizonMs)
                etaMs = null; // ufkun (5dk) ötesindeyse "yakın" sayılmaz
        }
...
class MetricTrendInjector {
    ...
    _plateauProfile(t, span) {
        const rampFrac = 0.25;
        if (t < rampFrac)
            return span * (t / rampFrac);
        if (t > 1 - rampFrac)
            return span * ((1 - t) / rampFrac);
        return span;
    }
    /**
    ...
    async run({ peak = 0.95, durationMs = 18000, baseline = 0.15 } = {}, apply, reset) {
        if (this.running)
            throw new Error("Bir trend enjeksiyonu zaten aktif — önce iptal edin.");
        this.running = true;
        const token = {};
        this._cancelToken = token;
        const startTime = performance.now();
        try {
            ...
            reset === null || reset === void 0 ? void 0 : reset(baseline);
            this.running = false;
            this._cancelToken = null;
        }
    }
...
class CoefficientEvolutionEngine {
    ...
    evaluateFitness(genome, samples, nodeRejectionPenaltyWeight = 0.4) {
        ...
        for (const s of samples) {
            ...
            for (let i = 0; i < PROBE_PHOTON_COUNT; i++) {
                const result = probeGate.evaluate("PROBE_NODE", 1550, 500, /* lowPriority */ i % 3 === 0, { linkLoad: s.linkLoad, historicalBer: s.historicalBer, coeffOverride: { alpha: genome.alpha, beta: genome.beta } }, probeRng);
                if (!result.passed)
                    rejections++;
            }
    ...
    measureNodeRejectionRate(genome, samples) {
        ...
        for (const s of samples) {
            ...
            let rejections = 0;
            for (let i = 0; i < PROBE_PHOTON_COUNT; i++) {
                const result = probeGate.evaluate("PROBE_NODE", 1550, 500, i % 3 === 0, { linkLoad: s.linkLoad, historicalBer: s.historicalBer, coeffOverride: { alpha: genome.alpha, beta: genome.beta } }, probeRng);
                if (!result.passed)
                    rejections++;
            }
...
class MemristorCell {
    ...
    get activeCoeffs() {
        ...
        if (elapsed >= MemristorCell.MICRO_CLOCK_WINDOW_MS)
            return activeRaw;
    ...
    get localThreshold() {
        ...
        if (elapsed >= MemristorCell.SWITCH_WINDOW_MS)
            return activeRaw;
...
class NodeRegisterMatrix {
    ...
    read(nodeId) {
        var _a;
        return (_a = this.registers.get(nodeId)) !== null && _a !== void 0 ? _a : null;
    }
...
class OpticalPhaseLockedLoop {
    ...
    constructor(opts = {}) {
        var _a, _b, _c, _d;
        this.kp = (_a = opts.kp) !== null && _a !== void 0 ? _a : 0.35; // Faz Dedektörü → Döngü Filtresi oransal terim
        this.ki = (_b = opts.ki) !== null && _b !== void 0 ? _b : 0.08; // Döngü Filtresi integral terim (biriken hata)
        this.lockThreshold = (_c = opts.lockThreshold) !== null && _c !== void 0 ? _c : 0.05; // radyan — bu değerin altı "kilitli" sayılır
        this.lockPersistenceTicks = (_d = opts.lockPersistenceTicks) !== null && _d !== void 0 ? _d : 5;
    ...
    phaseDampingFactor(nm, km) {
        ...
        if (!channel || !channel.locked)
            return 1.0; // kilitli değilse hiç etki yok — eski davranış
    ...
    notifyCoefficientInjection() {
        ...
        if (this.injectionHistory.length > 20)
...
class InMemoryComputeFabric {
    ...
    evaluateInPlace(nodeId, nm, km, lowPriority, legaCtx, rng) {
        ...
        const { gateResult, coefficientsWereUpdated } = legaDecisionCore.decide(cell, nodeId, nm, km, lowPriority, legaCtx, rng);
...
function computeRepeaterGain(nm, km, reps, ceilingOverride) {
    ...
    const ceiling = ceilingOverride !== null && ceilingOverride !== void 0 ? ceilingOverride : 0.95;
...
function dynamicRedundancyFor(nm, km, reps, legaCtx, targetArrival = 0.999, maxRedundancy = 150) {
    ...
    if (s >= 0.85)
        return 1; // kanal zaten güvenilir — ekstra foton israfına gerek yok
...
function propPhoton(nm, km, reps, evesdrop, rng, legaCtx, repeaterCeilingOverride) {
    ...
    const thresholds = (legaCtx === null || legaCtx === void 0 ? void 0 : legaCtx.coeffOverride)
    ...
    for (let seg = 0; seg <= reps && alive; seg++) {
        ...
        if (rand() < (w.r / 1200) * sk) {
            ...
            if (rand() < thresholds.scatterDeathProb) {
                alive = false;
                break;
            }
        }
        ...
        const atmosphericFactor = (legaCtx === null || legaCtx === void 0 ? void 0 : legaCtx.atmosphericConditions)
        ...
        const ac = legaCtx === null || legaCtx === void 0 ? void 0 : legaCtx.atmosphericConditions;
        ...
        const pointingFactor = (ac && ac.elevationDeg != null)
            ? PointingBudget.lossFactor(km, (_a = ac.pointingPrecision) !== null && _a !== void 0 ? _a : 0.7, scintillationEngine.chiVariance)
            : 1.0;
        const effectiveTransmittance = Math.min(1.0, fiberT(nm, sk) * repeaterGainFactor * atmosphericFactor * scintillationFactor * pointingFactor);
    ...
    if (!alive) {
        if (legaCtx)
            lega.recordOutcome(nm, km, true); // yorgunluk sayacını güncelle
        return { ok: false, evs };
    }
    if (legaCtx)
        lega.recordOutcome(nm, km, false); // hayatta kaldı → yorgunluk azalır
    ...
    if (legaCtx === null || legaCtx === void 0 ? void 0 : legaCtx.opllEnabled) {
    ...
    const phaseDamping = (_c = (_b = legaCtx === null || legaCtx === void 0 ? void 0 : legaCtx.coeffOverride) === null || _b === void 0 ? void 0 : _b.phaseDamping) !== null && _c !== void 0 ? _c : 1.0;
...
function propPhotonRelayChain(nm, km, reps, evesdrop, rng, legaCtx) {
    ...
    for (let h = 0; h < nHops; h++) {
        ...
        // decay mekanizması).
        const attempt = propPhoton(nm, hopKm, h === 0 ? Math.max(reps, MSG_RELAY_HOP_REPS) : MSG_RELAY_HOP_REPS, evesdrop && h === 0, rng, { coeffOverride: MSG_RELAY_COEFF_OVERRIDE }, MSG_RELAY_COMPENSATION);
        ...
        if (attempt.flip)
            flipParity = !flipParity; // ardışık hop'lardaki flip'ler kümülatif XOR'lanır
    }
...
class EdgeWeightPolicy {
    ...
    constructor(opts = {}) {
        var _a, _b, _c;
        this.loadFactor = (_a = opts.loadFactor) !== null && _a !== void 0 ? _a : 0.6;
        this.predictiveFactor = (_b = opts.predictiveFactor) !== null && _b !== void 0 ? _b : 0.5;
        this.measuredRiskFactor = (_c = opts.measuredRiskFactor) !== null && _c !== void 0 ? _c : 0.4;
    }
    ...
    readLoad(link, linkLoad) {
        ...
        return (_b = (_a = linkLoad[kAB]) !== null && _a !== void 0 ? _a : linkLoad[kBA]) !== null && _b !== void 0 ? _b : 0;
    }
    /**
    ...
    readPredictiveRisk(link, predictiveRisk) {
        ...
        const kAB = EdgeWeightPolicy.linkKey(link.a, link.b);
        const kBA = EdgeWeightPolicy.linkKey(link.b, link.a);
        return (_b = (_a = predictiveRisk[kAB]) !== null && _a !== void 0 ? _a : predictiveRisk[kBA]) !== null && _b !== void 0 ? _b : 0;
    }
    /**
    ...
    readMeasuredRisk(link, measuredRisk) {
        ...
        const kAB = EdgeWeightPolicy.linkKey(link.a, link.b);
        const kBA = EdgeWeightPolicy.linkKey(link.b, link.a);
        return (_b = (_a = measuredRisk[kAB]) !== null && _a !== void 0 ? _a : measuredRisk[kBA]) !== null && _b !== void 0 ? _b : 0;
    }
    /**
    ...
    computeWeight(link, ctx = {}) {
        if (this.isDown(link, ctx.linkDown))
            return Infinity;
...
class MinHeap {
    ...
    push(key, dist) {
        ...
        while (i > 0) {
            const parent = (i - 1) >> 1;
            if (this.arr[parent].dist <= this.arr[i].dist)
                break;
    ...
    pop() {
        ...
        if (this.arr.length > 0) {
            ...
            while (true) {
                ...
                let smallest = i;
                if (l < this.arr.length && this.arr[l].dist < this.arr[smallest].dist)
                    smallest = l;
                if (r < this.arr.length && this.arr[r].dist < this.arr[smallest].dist)
                    smallest = r;
                if (smallest === i)
                    break;
...
class NetworkTopology {
    ...
    rebuild(nodes, links) {
        ...
        for (const lk of links) {
            if (!this.adjacency.has(lk.a))
                this.adjacency.set(lk.a, []);
            if (!this.adjacency.has(lk.b))
                this.adjacency.set(lk.b, []);
    ...
    getNode(id) { var _a; return (_a = this.nodeMap.get(id)) !== null && _a !== void 0 ? _a : null; }
    ...
    neighbors(id) { var _a; return (_a = this.adjacency.get(id)) !== null && _a !== void 0 ? _a : []; }
    ...
    shortestPath(src, dst, isNodeLive, weightCtx, policy, excludeLinkKeys) {
        ...
        if (!this.hasNode(src) || !this.hasNode(dst))
            return null;
        if (isNodeLive && (!isNodeLive(src) || !isNodeLive(dst)))
            return null;
        ...
        while (heap.size > 0) {
            ...
            if (visited.has(u))
                continue; // stale heap entry — atla
            visited.add(u);
            if (u === dst)
                break;
        ...
        if (!dist.has(dst) || !isFinite(dist.get(dst)))
            return null;
        const path = [];
        let cur = dst;
        while (prev.has(cur)) {
...
class DualClockScheduler {
    ...
    reportOccupancy(occupancy) {
        ...
        if (occupancy < this.OCCUPANCY_WAKE_THRESHOLD)
            return false;
        ...
        const loop = (now) => {
            ...
            for (const [name, sys] of this.systems) {
                ...
                if (elapsed >= sys.tickMs) {
                    ...
                    catch (e) { /* bir sistemin hatası diğerlerini durdurmasın */ }
                }
            }
...
class ChiralAcousticEngine {
    constructor() {
        this.ctx = null;
        this.carrierPrime = null;
        this.gainPrime = null;
        this.modulatorPrime = null;
        this.modGainPrime = null;
        this.carrierMirror = null;
        this.gainMirror = null;
        this.modulatorMirror = null;
        this.modGainMirror = null;
        this.running = false;
    ...
    start({ carrierFreq, modFreq, modDepth }) {
        ...
        this.carrierPrime.start();
        this.modulatorPrime.start();
        this.carrierMirror.start();
        this.modulatorMirror.start();
        this.running = true;
        return true;
    }
    ...
    updateFromTelemetry(qber, chiralityDelta) {
        ...
        const modDepth = 5 + (chiralityDelta !== null && chiralityDelta !== void 0 ? chiralityDelta : 0) * 15;
        const modFreq = 2 + (chiralityDelta !== null && chiralityDelta !== void 0 ? chiralityDelta : 0) * 3;
        [this.carrierPrime, this.carrierMirror].forEach(osc => {
            if (osc)
                osc.frequency.linearRampToValueAtTime(carrierFreq, t + 0.15);
        });
        if (this.modulatorPrime)
            this.modulatorPrime.frequency.linearRampToValueAtTime(modFreq, t + 0.15);
        if (this.modulatorMirror)
            this.modulatorMirror.frequency.linearRampToValueAtTime(modFreq * 0.5, t + 0.15);
        if (this.modGainPrime)
            this.modGainPrime.gain.linearRampToValueAtTime(modDepth, t + 0.15);
        if (this.modGainMirror)
            this.modGainMirror.gain.linearRampToValueAtTime(modDepth, t + 0.15);
    }
    ...
    async suspend() { if (this.ctx && this.ctx.state === "running")
        await this.ctx.suspend(); }
    async resume() { if (this.ctx && this.ctx.state === "suspended")
    ...
    if (typeof navigator !== "undefined" && navigator.locks && navigator.locks.request) {
        try {
            ...
            return await navigator.locks.request("photonnet-master-universe", { ifAvailable: true }, async (lock) => {
                if (!lock)
                    return false; // başka sekme zaten Master
                onAcquired === null || onAcquired === void 0 ? void 0 : onAcquired();
...
function releaseMasterLock() {
    if (onReleased_resolve) {
        onReleased_resolve();
        onReleased_resolve = null;
    }
}
// ══════════════════════════════════════════════════════════════
...
function createClockDriftDetector(onDrift, intervalMs = 1000) {
    ...
    const timer = setInterval(() => {
        ...
        expected += intervalMs;
        onDrift === null || onDrift === void 0 ? void 0 : onDrift({ driftMs: drift, intervalMs, timestamp: Date.now() });
        lastReal = real;
    }, intervalMs);
...
function buildReportHtml(entry) {
    ...
    const eventRows = (entry.eventLog || []).slice(0, 30).map(ev => `<tr><td>${ev.seg}</td><td>${ev.bit}</td><td>${ev.type}</td><td>${ev.km.toFixed(2)} km</td><td>${ev.text}</td></tr>`).join("");
...
<style>
  ...
  <p>Baz uzlaşma oranı: ${r.bb84MatchRate != null ? (r.bb84MatchRate * 100).toFixed(1) + "%" : "—"}<br/>
     ...
     Sifted key uzunluğu: ${(_a = r.siftedKeyLen) !== null && _a !== void 0 ? _a : "—"} bit<br/>
...
function explainEcc(corrected) {
    if (corrected <= 0)
        return null;
    if (corrected === 1)
        return `Hamming(7,4) kodu burada 1 bitlik hatayı tek-bit-düzeltme kapasitesiyle onardı.`;
...
function physicalSimulation(segs, bits, entanglementSeed, evesdrop, getNode, legaOpts) {
    ...
    for (let si = 0; si < segs.length; si++) {
        ...
        const rp = (_a = nd === null || nd === void 0 ? void 0 : nd.reps) !== null && _a !== void 0 ? _a : 0;
        let sl = 0, sf = 0;
        ...
        const legaCtx = legaOpts ? {
            linkLoad: (_e = ((_c = (_b = legaOpts.linkLoad) === null || _b === void 0 ? void 0 : _b[`${lk.a}-${lk.b}`]) !== null && _c !== void 0 ? _c : (_d = legaOpts.linkLoad) === null || _d === void 0 ? void 0 : _d[`${lk.b}-${lk.a}`])) !== null && _e !== void 0 ? _e : 0,
            historicalBer: (_f = legaOpts.historicalBer) !== null && _f !== void 0 ? _f : 0,
        ...
        const redundancy = (legaOpts === null || legaOpts === void 0 ? void 0 : legaOpts.dynamicFecEnabled)
        ...
        for (let i = 0; i < recv.length; i++) {
            ...
            r.evs.forEach(ev => {
                var _a;
                ec[ev.type] = ((_a = ec[ev.type]) !== null && _a !== void 0 ? _a : 0) + 1;
            ...
            if (r.ok && legaOpts && nd) {
                ...
                gateReport.checked++;
                gateReport.savedNs = ((_g = gateReport.savedNs) !== null && _g !== void 0 ? _g : 0) + (latencyNs.vonNeumann - latencyNs.pim);
                if (!gate.passed) {
                    if (gate.phase === 1)
                        gateReport.phase1Rejected++;
                    else
                        gateReport.phase2Rejected++;
                    ec.GATE_REJECT = ((_h = ec.GATE_REJECT) !== null && _h !== void 0 ? _h : 0) + 1;
                    eventLog.push({
                ...
                if (gate.softMatched) {
                    gateReport.softMatchedCount = ((_j = gateReport.softMatchedCount) !== null && _j !== void 0 ? _j : 0) + 1;
                    eventLog.push({
...
function Oscilloscope({ active, color = "#00d4ff" }) {
    ...
    return React.createElement("canvas", { ref: cvs, width: 220, height: 52, style: { width: "100%", height: 52, display: "block", borderRadius: 4 } });
}
// ── Spark line ────────────────────────────────────────────────
...
function BerGauge({ value }) {
    ...
    return (React.createElement("div", { style: { height: 3, background: "#0a1628", borderRadius: 2, overflow: "hidden", width: 40 } },
        React.createElement("div", { style: { height: "100%", width: `${pct}%`, background: c, borderRadius: 2,
                boxShadow: `0 0 4px ${c}`, transition: "width .6s" } })));
}
// ══════════════════════════════════════════════════════════════
// LEO QUANTUM LINK PANEL (görseldeki mobil arayüz)
// ══════════════════════════════════════════════════════════════
function LeoPanel({ sats, activeSat, setActiveSat, onSend, running, msg, setMsg, evesdrop, setEvesdrop, result, phase, timeline, onOpenTimeline, mirrorOn, setMirrorOn, mirrorResult, onCollapse, onInterfere, collapsedTo, entropyDelta, isOnline, observerFrame, atmosphericTurbidity, setAtmosphericTurbidity, turbulenceStrength, setTurbulenceStrength, pointingPrecision, setPointingPrecision, nightMode, setNightMode, paQueue }) {
    var _a;
    ...
    return (React.createElement("div", { style: {
        ...
        React.createElement("div", { style: {
            ...
            React.createElement("div", { style: {
                ...
                React.createElement("div", { style: { fontSize: 26, filter: "drop-shadow(0 0 10px #00d4ff88)",
                        animation: "satOrbit 8s linear infinite" } }, "\uD83D\uDEF0"),
                React.createElement("div", null,
                    React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: "#00d4ff", letterSpacing: ".05em" } }, "LEO Ka-Band Quantum Link"),
                    React.createElement("div", { style: { fontSize: 8, color: "#334155" } },
                        curSat.name,
                        " \u00B7 ",
                        curSat.elev.toFixed(0),
                        "\u00B0 \u00B7",
                        " ",
                        React.createElement("span", { style: { color: q > .7 ? "#10b981" : q > .4 ? "#f59e0b" : "#f43f5e", fontWeight: 700 } },
                            (q * 100).toFixed(0),
                            "% kalite")),
                    React.createElement("div", { style: { width: 100, height: 3, background: "#0a1628", borderRadius: 2, overflow: "hidden", marginTop: 3 } },
                        React.createElement("div", { style: { height: "100%", width: `${q * 100}%`, borderRadius: 2, transition: "width .8s",
                                background: q > .7 ? "linear-gradient(90deg,#10b981,#00d4ff)" : q > .4 ? "#f59e0b" : "#f43f5e",
                                boxShadow: `0 0 6px ${q > .7 ? "#00d4ff" : q > .4 ? "#f59e0b" : "#f43f5e"}` } })))),
            React.createElement("div", { style: { minWidth: 200, paddingRight: 16, borderRight: "1px solid #0a1e3a44", marginRight: 16, flexShrink: 0 } },
                React.createElement("div", { style: { fontSize: 8, color: "#475569", letterSpacing: ".08em", marginBottom: 4 } }, "MESSAGE"),
                React.createElement("input", { value: msg, onChange: e => setMsg(e.target.value.slice(0, 28)), disabled: running, placeholder: "Mesaj\u0131n\u0131z\u0131 girin\u2026", style: { width: "100%", padding: "7px 10px", borderRadius: 7,
                        background: "#020912", border: "1px solid #00d4ff22",
                        color: "#00d4ff", fontSize: 12, letterSpacing: ".03em",
                        fontFamily: "'Inter',system-ui,sans-serif",
                        boxShadow: "inset 0 0 10px #00d4ff08" } }),
                React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", marginTop: 2, fontFamily: "monospace" }, title: "Bu, girilen metnin ham karakter/bit say\u0131s\u0131d\u0131r \u2014 QKD performans \u00F6l\u00E7\u00FCt\u00FC DE\u011E\u0130LD\u0130R. Ger\u00E7ek anahtar \u00FCretim h\u0131z\u0131 (bit/saniye) i\u00E7in sonu\u00E7 panelindeki 'QKD' g\u00F6stergesine bak\u0131n." },
                    msg.length,
                    "/28 kr \u00B7 ",
                    msg.length * 8,
                    " bit (mesaj boyutu, QKD h\u0131z\u0131 de\u011Fil)")),
            React.createElement("div", { style: { paddingRight: 16, borderRight: "1px solid #0a1e3a44", marginRight: 16, flexShrink: 0 } },
                React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 5, marginBottom: 4 } },
                    React.createElement("span", { style: { fontSize: 8, color: "#475569", letterSpacing: ".08em" } }, "WAVELENGTH"),
                    React.createElement("span", { style: { fontSize: 6, color: "#334155" }, title: "\u00D6neri, ger\u00E7ek FSO literat\u00FCr\u00FCn\u00FCn N\u0130TEL\u0130KSEL y\u00F6nelimine dayan\u0131r \u2014 say\u0131sal katsay\u0131lar kalibre edilmemi\u015Ftir. M\u00FChendislik-s\u0131n\u0131f\u0131 do\u011Fruluk i\u00E7in NetSquid sim\u00FClasyonu ve ITU-R P.1621-2 link-b\u00FCt\u00E7esi standard\u0131yla do\u011Frulama gerekir." }, "(FSO-\u00F6nerisi, didaktik model \u2014 ITU-R do\u011Frulamas\u0131 yok)")),
                React.createElement("div", { style: { display: "flex", gap: 5 } }, Object.entries(WL).map(([nm, w]) => {
                    ...
                    return (React.createElement("button", { key: nm, onClick: () => setSelNm(+nm), title: `${w.label} · ${w.loss}dB/km (fiber) · FSO uygunluk: %${(atmoFactor * 100).toFixed(0)}`, style: {
                        ...
                        isRecommended && (React.createElement("div", { style: { position: "absolute", top: -7, left: "50%", transform: "translateX(-50%)",
                                fontSize: 8, color: "#22c55e" } }, "\u2B50")),
                        React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: active ? w.hex : "#334155", letterSpacing: ".02em" } }, nm),
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f" } },
                            w.loss,
                            "dB"),
                        React.createElement("div", { style: { fontSize: 6, color: atmoFactor > 0.7 ? "#22c55e" : atmoFactor > 0.4 ? "#f59e0b" : "#f43f5e" } },
                            "FSO %",
                            (atmoFactor * 100).toFixed(0))));
                })),
                React.createElement("div", { style: { marginTop: 5 } },
                    React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 2 } },
                        React.createElement("span", { style: { fontSize: 6, color: "#334155" } }, "ATMOSFER BULANIKLI\u011EI"),
                        React.createElement("span", { style: { fontSize: 6, color: "#00d4ff", fontFamily: "monospace" } },
                            (atmosphericTurbidity * 100).toFixed(0),
                            "%")),
                    React.createElement("input", { type: "range", min: 0, max: 1, step: 0.05, value: atmosphericTurbidity, onChange: e => setAtmosphericTurbidity(+e.target.value), style: { width: "100%", accentColor: "#22c55e", cursor: "pointer", height: 3 } }))),
            React.createElement("div", { style: { width: 150, paddingRight: 16, borderRight: "1px solid #0a1e3a44", marginRight: 16, flexShrink: 0 } },
                React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 4 } },
                    React.createElement("span", { style: { fontSize: 8, color: "#475569", letterSpacing: ".06em" } }, "MESAFE"),
                    React.createElement("span", { style: { fontSize: 9, color: "#00d4ff", fontFamily: "monospace", fontWeight: 700 } },
                        satDist,
                        "km")),
                React.createElement("input", { type: "range", min: 400, max: 1200, value: satDist, onChange: e => setSatDist(+e.target.value), disabled: running, style: { width: "100%", accentColor: "#00d4ff", cursor: "pointer", height: 4 } }),
                React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginTop: 2 } },
                    React.createElement("span", { style: { fontSize: 7, color: "#1e3a5f" } }, "LEO"),
                    React.createElement("span", { style: { fontSize: 7, color: "#1e3a5f" } }, "MEO"))),
            React.createElement("div", { style: { width: 190, paddingRight: 16, borderRight: "1px solid #0a1e3a44", marginRight: 16, flexShrink: 0 } },
                React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 5, marginBottom: 4 } },
                    React.createElement("span", { style: { fontSize: 8, color: "#475569", letterSpacing: ".08em" } }, "ATMOSFER\u0130K G\u00DCR\u00DCLT\u00DC"),
                    React.createElement("span", { style: { fontSize: 6, color: "#334155" }, title: "Hufnagel-Valley Cn\u00B2(h) + e\u011Fik-yol Rytov varyans\u0131 + Andrews-Phillips doygunluk d\u00FCzeltmesi. Kayd\u0131r\u0131c\u0131n\u0131n Cn\u00B2(0)'a haritalanmas\u0131 ve r\u00FCzgar h\u0131z\u0131 varsay\u0131lan\u0131 kalibre edilmemi\u015Ftir \u2014 didaktik model." }, "(scintillation, didaktik)")),
                React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 2 } },
                    React.createElement("span", { style: { fontSize: 6, color: "#334155" } }, "T\u00DCRB\u00DCLANS \u015E\u0130DDET\u0130"),
                    React.createElement("span", { style: { fontSize: 6, color: "#00d4ff", fontFamily: "monospace" } },
                        (turbulenceStrength * 100).toFixed(0),
                        "%")),
                React.createElement("input", { type: "range", min: 0, max: 1, step: 0.05, value: turbulenceStrength, onChange: e => setTurbulenceStrength(+e.target.value), style: { width: "100%", accentColor: "#f59e0b", cursor: "pointer", height: 3 } }),
                React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 2, marginTop: 4 } },
                    React.createElement("span", { style: { fontSize: 6, color: "#334155" } }, "\u0130ZLEME HASSAS\u0130YET\u0130"),
                    React.createElement("span", { style: { fontSize: 6, color: "#00d4ff", fontFamily: "monospace" } },
                        (pointingPrecision * 100).toFixed(0),
                        "%")),
                React.createElement("input", { type: "range", min: 0, max: 1, step: 0.05, value: pointingPrecision, onChange: e => setPointingPrecision(+e.target.value), title: "Uydu gimbal/ince-izleme sisteminin mekanik hassasiyeti \u2014 d\u00FC\u015F\u00FCkse \u0131\u015F\u0131n kaymas\u0131 (beam wander) ile birle\u015Ferek i\u015Faretleme kayb\u0131n\u0131 art\u0131r\u0131r (PointingBudget).", style: { width: "100%", accentColor: "#7c3aed", cursor: "pointer", height: 3 } }),
                React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 5 } },
                    React.createElement("span", { style: { fontSize: 6, color: "#334155" }, title: "G\u00FCnd\u00FCz g\u00F6ky\u00FCz\u00FC arka plan g\u00FCr\u00FClt\u00FCs\u00FC, gece de\u011Ferinden binlerce kat y\u00FCksektir \u2014 ger\u00E7ek uydu QKD sistemlerinin (Micius dahil) neredeyse tamam\u0131 bu y\u00FCzden yaln\u0131zca gece \u00E7al\u0131\u015F\u0131r (DetectorNoiseModel)." }, "OPERASYON ZAMANI"),
                    React.createElement("button", { onClick: () => setNightMode(v => !v), style: { fontSize: 7, fontWeight: 700, padding: "2px 8px", borderRadius: 4, cursor: "pointer",
                            border: `1px solid ${nightMode ? "#7c3aed66" : "#f59e0b66"}`,
                            background: nightMode ? "#7c3aed22" : "#f59e0b22",
                            color: nightMode ? "#a78bfa" : "#f59e0b" } }, nightMode ? "🌙 GECE" : "☀️ GÜNDÜZ")),
                React.createElement("div", { style: { marginTop: 4, fontSize: 6, color: "#1e3a5f", fontFamily: "monospace" }, title: "scintillationEngine'in son satellite-link-tick'te (900ms) hesaplad\u0131\u011F\u0131 anl\u0131k de\u011Ferler." },
                    "scint.idx=",
                    scintillationEngine.lastScintIndex.toFixed(2),
                    " (",
                    scintillationEngine.lastRegime,
                    ") \u00B7 fade\u00D7",
                    scintillationEngine.fadeFactor().toFixed(2))),
            React.createElement("div", { style: { minWidth: 150, paddingRight: 16, borderRight: "1px solid #0a1e3a44", marginRight: 16, flexShrink: 0 } },
                React.createElement("div", { style: { fontSize: 8, color: "#475569", letterSpacing: ".08em", marginBottom: 4 } }, "AKT\u0130F UYDU"),
                React.createElement("div", { style: { display: "flex", gap: 4 } }, sats.slice(0, 3).map(s => {
                    const sq = satQ(s.elev);
                    const ac = s.id === activeSat;
                    return (React.createElement("div", { key: s.id, onClick: () => !running && setActiveSat(s.id), title: `${s.name} · ${s.elev.toFixed(0)}° · ${(sq * 100).toFixed(0)}%`, style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 2,
                        ...
                        React.createElement("span", { style: { fontSize: 7, color: ac ? "#00d4ff" : "#475569", fontWeight: ac ? 700 : 400 } }, s.id.split("-")[1])));
                }))),
            React.createElement("div", { style: { display: "flex", gap: 14, paddingRight: 16, borderRight: "1px solid #0a1e3a44", marginRight: 16, flexShrink: 0 } },
                React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4 } },
                    React.createElement("div", { onClick: () => !running && setEvesdrop(v => !v), style: { width: 38, height: 20, borderRadius: 10, cursor: "pointer",
                            background: evesdrop ? "linear-gradient(90deg,#00d4ff,#7c3aed)" : "#0a1628",
                            border: `1px solid ${evesdrop ? "#00d4ff44" : "#1e3a5f"}`,
                            position: "relative", transition: "all .3s" } },
                        React.createElement("div", { style: { width: 14, height: 14, borderRadius: "50%", background: "#fff",
                                position: "absolute", top: 2, left: evesdrop ? 20 : 2, transition: "left .3s",
                                boxShadow: evesdrop ? "0 0 5px #00d4ff" : "0 1px 2px rgba(0,0,0,.4)" } })),
                    React.createElement("span", { style: { fontSize: 7, color: evesdrop ? "#00d4ff" : "#475569", whiteSpace: "nowrap" } }, "Eavesdrop")),
                React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4 } },
                    React.createElement("div", { onClick: () => !running && setMirrorOn(v => !v), style: { width: 38, height: 20, borderRadius: 10, cursor: "pointer",
                    ...
                    React.createElement("span", { style: { fontSize: 7, color: mirrorOn ? "#a855f7" : "#475569", whiteSpace: "nowrap" } }, "\uD83E\uDE9E Mirror"))),
            running && ACTION_LABEL[phase] && (React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 7,
                    background: "#020912", border: "1px solid #00d4ff22",
                    borderRadius: 7, padding: "6px 12px", marginRight: 16,
                    animation: "fadeUp .25s ease", flexShrink: 0 } },
                React.createElement("span", { style: { display: "inline-block", animation: "spin 1s linear infinite", fontSize: 12 } }, "\u25CE"),
                React.createElement("span", { style: { fontSize: 10, color: "#00d4ff", fontWeight: 600, whiteSpace: "nowrap" } }, ACTION_LABEL[phase]))),
            React.createElement("div", { style: { marginLeft: "auto", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 } },
                result && result.routeHealth && !result.routeHealth.healthy && !running && (React.createElement("div", { style: { fontSize: 8, color: "#f59e0b", fontFamily: "monospace",
                        background: "#2a1a05", border: "1px solid #f59e0b44", borderRadius: 6,
                        padding: "5px 9px", maxWidth: 280, textAlign: "right", lineHeight: 1.4 } },
                    "\u26A0 Son rota fiziksel olarak ge\u00E7ilemez s\u0131n\u0131rdayd\u0131 (u\u00E7tan-uca ge\u00E7irgenlik %",
                    (result.routeHealth.cumulativeTransmittance * 100).toFixed(3),
                    ") \u2014 tekrar g\u00F6ndermeden \u00F6nce daha k\u0131sa bir mesafe/hedef se\u00E7in veya tekrarlay\u0131c\u0131 say\u0131s\u0131n\u0131 art\u0131r\u0131n.")),
                React.createElement("button", { onClick: handleSend, disabled: running || !msg.trim(), style: {
                    ...
                    ? React.createElement(React.Fragment, null,
                        React.createElement("span", { style: { display: "inline-block", animation: "spin 1s linear infinite", fontSize: 13 } }, "\u25CE"),
                        " ",
                        phase,
                        "\u2026")
                    : React.createElement(React.Fragment, null, "\uD83D\uDEF0 G\u00D6NDER")))),
        ((mirrorOn && mirrorResult) || (showResult && result && !running)) && (React.createElement("div", { style: {
                display: "flex", gap: 14, padding: "0 14px 12px",
                animation: "fadeUp .3s ease", flexWrap: "wrap",
            } },
            mirrorOn && mirrorResult && (React.createElement("div", { style: { flex: "1 1 300px", minWidth: 280,
                    background: "#1a0a2e33", border: "1px solid #a855f733",
                    borderRadius: 8, padding: "10px 12px" } },
                React.createElement("div", { style: { fontSize: 8, color: "#a855f7", marginBottom: 5, letterSpacing: ".06em", fontWeight: 700 } }, "\uD83E\uDE9E AYNA EVREN SONUCU"),
                mirrorResult.chirality && (React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap",
                        marginBottom: 6, paddingBottom: 6, borderBottom: "1px solid #a855f722" } },
                    React.createElement("span", { style: { fontSize: 8, color: mirrorResult.chirality.symmetryRatio > 0.5 ? "#22d3ee" : "#f43f5e",
                            fontWeight: 700, letterSpacing: ".02em" } },
                        "\u269B ",
                        mirrorResult.chirality.chiralityState),
                    React.createElement("span", { style: { fontSize: 8, color: "#94a3b8", fontFamily: "monospace" } },
                        mirrorResult.chirality.conjugateCount,
                        " e\u015Flenik / ",
                        mirrorResult.chirality.phaseDeltaCount,
                        " faz-fark\u0131"),
                    ((_a = mirrorResult.errorCorrelation) === null || _a === void 0 ? void 0 : _a.correlation) != null && (React.createElement("span", { style: { fontSize: 8, color: "#a855f7", fontFamily: "monospace" } },
                        "r=",
                        mirrorResult.errorCorrelation.correlation.toFixed(2))),
                    observerFrame && observerFrame.observerFrame !== "SÜPERPOZİSYON" && (React.createElement("span", { style: { fontSize: 8, color: "#00d4ff", fontFamily: "monospace" } },
                        "\uD83D\uDC41 ",
                        observerFrame.observerFrame)))),
                React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" } },
                    React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: mirrorResult.success ? "#22d3ee" : "#f43f5e",
                            fontFamily: "monospace" } },
                        "\"",
                        mirrorResult.decoded,
                        "\""),
                    React.createElement("div", { style: { fontSize: 10, color: "#94a3b8", fontFamily: "monospace" } },
                        "BER ",
                        mirrorResult.er,
                        "%"),
                    entropyDelta && (React.createElement("div", { style: { fontSize: 9, color: "#94a3b8" } },
                        "Entropi fark\u0131:",
                        " ",
                        React.createElement("span", { style: { color: "#a855f7", fontWeight: 700, fontFamily: "monospace" } },
                            entropyDelta.delta.toFixed(3),
                            " bit"))),
                    React.createElement("div", { style: { display: "flex", gap: 6, marginLeft: "auto" } },
                        React.createElement("button", { onClick: () => onCollapse("prime"), disabled: !!collapsedTo, style: { padding: "5px 10px", borderRadius: 5, cursor: collapsedTo ? "not-allowed" : "pointer",
                                border: `1px solid ${collapsedTo === "prime" ? "#00d4ff77" : "#1e3a5f"}`,
                                background: collapsedTo === "prime" ? "#00d4ff11" : "transparent",
                                color: collapsedTo === "prime" ? "#00d4ff" : "#475569", fontSize: 8, fontWeight: 700, whiteSpace: "nowrap" } }, "ASAL SE\u00C7"),
                        React.createElement("button", { onClick: () => onCollapse("mirror"), disabled: !!collapsedTo, style: { padding: "5px 10px", borderRadius: 5, cursor: collapsedTo ? "not-allowed" : "pointer",
                                border: `1px solid ${collapsedTo === "mirror" ? "#a855f777" : "#1e3a5f"}`,
                                background: collapsedTo === "mirror" ? "#a855f711" : "transparent",
                                color: collapsedTo === "mirror" ? "#a855f7" : "#475569", fontSize: 8, fontWeight: 700, whiteSpace: "nowrap" } }, "AYNA SE\u00C7"),
                        React.createElement("button", { onClick: onInterfere, style: { padding: "5px 10px", borderRadius: 5, cursor: "pointer",
                                border: "1px solid #ec489944", background: "#ec489911", color: "#f472b6",
                                fontSize: 8, fontWeight: 700, whiteSpace: "nowrap" } }, "\u26A1 INTERFERENCE"))))),
            showResult && result && !running && (React.createElement("div", { style: { flex: "1 1 340px", minWidth: 300,
                ...
                React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" } },
                    React.createElement("div", { style: { fontSize: 22 } }, result.success ? "✅" : "⚠️"),
                    React.createElement("div", null,
                        React.createElement("div", { style: { fontSize: 11, fontWeight: 800, letterSpacing: ".04em",
                                color: result.success ? "#10b981" : "#f59e0b" } }, result.success ? "İletildi" : "Kısmi İletim"),
                        React.createElement("div", { style: { fontSize: 8, color: "#64748b" } },
                            result.sat,
                            " \u00FCzerinden")),
                    React.createElement("div", { style: { background: "#020912", borderRadius: 6, padding: "5px 9px" } },
                        React.createElement("div", { style: { fontSize: 6, color: "#334155" } }, "G\u00D6NDER\u0130LEN"),
                        React.createElement("div", { style: { fontSize: 10, color: "#00d4ff", fontWeight: 700, fontFamily: "monospace" } },
                            "\"",
                            result.original,
                            "\"")),
                    React.createElement("div", { style: { background: "#020912", borderRadius: 6, padding: "5px 9px" } },
                        React.createElement("div", { style: { fontSize: 6, color: "#334155" } }, "ALINAN"),
                        React.createElement("div", { style: { fontSize: 10, fontWeight: 700, fontFamily: "monospace",
                                color: result.success ? "#10b981" : "#f59e0b" } },
                            "\"",
                            result.decoded,
                            "\"")),
                    React.createElement("div", { style: { display: "flex", gap: 10, fontSize: 9, color: "#475569", fontFamily: "monospace" } },
                        ...
                        React.createElement("span", null,
                            result.totalKm,
                            "km"),
                        React.createElement("span", { style: { color: +result.er > 10 ? "#f43f5e" : "#10b981" } },
                            "BER ",
                            result.er,
                            "%"),
                        result.bb84MatchRate != null && (React.createElement("span", { style: { color: "#7c3aed" } },
                            "BB84 ",
                            (result.bb84MatchRate * 100).toFixed(0),
                            "%")),
                        result.rawKeyRate && (React.createElement("span", { style: { color: "#06b6d4" }, title: "Micius uydusu ger\u00E7ek performans verisine (80MHz kaynak, en-iyi-durum ~1.7Mbit/s) \u00F6l\u00E7eklenmi\u015F ham anahtar \u00FCretim h\u0131z\u0131" },
                            "QKD ",
                            result.rawKeyRate.label)),
                        result.securityProof && (React.createElement("span", { style: { color: result.securityProof.secure ? "#10b981" : "#f43f5e" }, title: result.securityProof.secure
                    ...
                    React.createElement("button", { onClick: onOpenTimeline, style: {
                        ...
                        ")")),
                result.securityProof && (React.createElement("div", { style: { marginTop: 8, paddingTop: 8, borderTop: "1px solid #ffffff0f",
                        display: "flex", gap: 14, flexWrap: "wrap", fontSize: 8, color: "#64748b", fontFamily: "monospace" } },
                    React.createElement("span", { style: { color: "#94a3b8", fontWeight: 700 } }, "\uD83D\uDD10 SONLU-ANAHTAR KANITI (GLLP/Serfling):"),
                    React.createElement("span", null,
                        "n=",
                        result.securityProof.n,
                        "bit"),
                    React.createElement("span", null,
                        "Q_bit=",
                        result.securityProof.qBit != null ? (result.securityProof.qBit * 100).toFixed(2) + "%" : "—"),
                    React.createElement("span", null,
                        "Q_ph\u2264",
                        (result.securityProof.qPhUpper * 100).toFixed(2),
                        "%"),
                    React.createElement("span", { title: "Sonsuz-\u00F6rneklem (asimptotik) GLLP anahtar oran\u0131 \u2014 kar\u015F\u0131la\u015Ft\u0131rma i\u00E7in" },
                        "R_\u221E=",
                        (result.securityProof.asymptoticRate * 100).toFixed(1),
                        "%"),
                    React.createElement("span", { style: { color: result.securityProof.secure ? "#10b981" : "#f43f5e", fontWeight: 700 } },
                        "\u2113=",
                        result.securityProof.ell,
                        "bit ",
                        result.securityProof.secure ? "✓ KANITLANMIŞ GÜVENLİ" : "✗ GÜVENLİ ANAHTAR YOK"),
                    !result.securityProof.secure && (React.createElement("span", { style: { color: "#f43f5e" } },
                        "(",
                        result.securityProof.reason,
                        ")")),
                    result.classicalAuth && (React.createElement("span", { title: "Wegman-Carter MAC ile kimliklendirilen parametre-kestirim duyurusu + anahtar havuzu geri besleme durumu", style: { color: result.classicalAuth.ok ? "#a78bfa" : "#f43f5e" } },
                        "\uD83D\uDD0F klasik-kanal auth: ",
                        result.classicalAuth.ok ? `✓ (havuz ${result.classicalAuth.poolRemaining}bit)` : `✗ ${result.classicalAuth.reason}`)))),
                result.keyPoolStatus && !result.keyPoolStatus.finalized && (React.createElement("div", { style: { marginTop: 6, display: "flex", alignItems: "center", gap: 8, fontSize: 8, color: "#64748b", fontFamily: "monospace" }, title: "Bu rotadan (kaynak-hedef \u00E7ifti) biriken sifted-key havuzu \u2014 e\u015Fi\u011Fe ula\u015F\u0131nca GER\u00C7EK n/k (Parametre Kestirimi) ayr\u0131m\u0131yla yeniden, daha s\u0131k\u0131 bir sonlu-anahtar kan\u0131t\u0131 \u00FCretilir" },
                    React.createElement("span", { style: { color: "#94a3b8", fontWeight: 700 } }, "\uD83E\uDEA3 ASENKRON HAVUZ:"),
                    React.createElement("span", null,
                        result.keyPoolStatus.poolProgress,
                        "/",
                        result.keyPoolStatus.poolThreshold,
                        "bit"),
                    React.createElement("div", { style: { flex: "0 1 140px", height: 4, background: "#ffffff14", borderRadius: 2, overflow: "hidden" } },
                        React.createElement("div", { style: { width: `${Math.min(100, (result.keyPoolStatus.poolProgress / result.keyPoolStatus.poolThreshold) * 100)}%`, height: "100%", background: "#06b6d4" } })),
                    React.createElement("span", { style: { color: "#475569" } }, "blok tamamlan\u0131nca ger\u00E7ek n/k ayr\u0131m\u0131yla yeniden kan\u0131tlanacak"))),
                result.keyPoolStatus && result.keyPoolStatus.finalized && (() => {
                    const f = result.keyPoolStatus.finalized;
                    return (React.createElement("div", { style: { marginTop: 6, paddingTop: 6, borderTop: "1px solid #ffffff0f",
                            display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center", fontSize: 8, color: "#64748b", fontFamily: "monospace",
                            background: f.proof.secure ? "#052e1622" : "#3f151522", borderRadius: 6, padding: "4px 8px" }, title: "ProductionSecurityAudit.audit() \u2014 HAM test verisinden yeniden hesaplanan, ba\u011F\u0131ms\u0131z denetlenebilir QBER/Gain/\u2113/\u03B5 \u2014 Madde 3'\u00FCn per-mesaj basitle\u015Ftirmesinden daha s\u0131k\u0131" },
                        React.createElement("span", { style: { color: f.proof.secure ? "#10b981" : "#f43f5e", fontWeight: 700 } },
                            "\uD83E\uDEA3\u2713 BLOK #",
                            f.blockIndex,
                            " TAMAMLANDI (",
                            f.routeKey,
                        ...
                        React.createElement("span", null,
                            "Q_test=",
                            (f.qEstimated * 100).toFixed(2),
                            "%"),
                        f.gain != null && React.createElement("span", { title: "Gain: kaynak darbesi ba\u015F\u0131na nihai kullan\u0131labilir bit oran\u0131" },
                            "Gain=",
                            (f.gain * 100).toFixed(1),
                            "%"),
                        f.epsilon && React.createElement("span", { title: "Composable \u03B5-security: \u03B5_PE+\u03B5_cor+\u03B5_PA (union bound)" },
                            "\u03B5_toplam=",
                            f.epsilon.total.toExponential(1)),
                        f.reconciliation && (React.createElement("span", { title: `Brassard & Salvail (1993) çok-geçişli Cascade protokolü, GERÇEKTEN çalıştırıldı ve anahtar örneklemi üzerinde ölçüldü — leakEC artık teorik tahmin değil.${f.reconciliationComparison && f.reconciliationComparison.ldpc ? ` (Karşılaştırma: LDPC ${f.reconciliationComparison.ldpc.leakedBits}bit sızıntı, ${f.reconciliationComparison.ldpc.converged ? "yakınsadı" : "yakınsamadı"})` : ""}`, style: { color: f.reconciliation.converged ? "#a78bfa" : "#f43f5e" } },
                            "\uD83D\uDD27 EC(",
                            f.reconciliation.protocol,
                            ")=",
                            f.reconciliation.leakedBits,
                            "bit ",
                            f.reconciliation.converged ? "✓" : `✗ ${f.reconciliation.residualErrors}bit residual`)),
                        React.createElement("span", { style: { color: f.proof.secure ? "#10b981" : "#f43f5e", fontWeight: 700 } },
                            "\u2113=",
                            f.proof.ell,
                            "bit ",
                            f.proof.secure ? "✓ GERÇEK n/k İLE KANITLANMIŞ" : "✗ GÜVENLİ ANAHTAR YOK"),
                        f.keyDelivery && (React.createElement("span", { title: `ETSI GS QKD 014 KME teslim deposuna kaydedildi — bb84/etsi014_kme_server.js bu anahtarı SAE'lere (IBM Quantum Network, VPN, TLS tünelleri) enc_keys/dec_keys uç noktalarıyla servis edebilir. key_ID=${f.keyDelivery.key_ID}`, style: { color: "#22d3ee" } },
                            "\uD83D\uDD10 KME: ",
                            f.keyDelivery.key_ID.slice(0, 8),
                            "\u2026 (rotada ",
                            f.keyDelivery.storedForRoute,
                            " anahtar haz\u0131r)")),
                        React.createElement("button", { onClick: () => {
                                ...
                                a.href = url;
                                a.download = `photonnet_production_blocks_${Date.now()}.json`;
                                document.body.appendChild(a);
                                a.click();
                                document.body.removeChild(a);
                                URL.revokeObjectURL(url);
                            }, title: "productionBlocks'un TAMAMINI (t\u00FCm rotalar, HAM test kay\u0131tlar\u0131 dahil) JSON olarak indir \u2014 bb84/production_security_audit.js ile PhotonNet'ten BA\u011EIMSIZ olarak yeniden do\u011Frulanabilir", style: { marginLeft: "auto", padding: "3px 8px", borderRadius: 5, border: "1px solid #06b6d455",
                                background: "#06b6d422", color: "#06b6d4", fontSize: 8, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" } }, "\uD83D\uDCE4 Denetim Verisi (JSON)"),
                        React.createElement("button", { onClick: () => {
                                ...
                                const a = document.createElement("a");
                                a.href = url;
                                a.download = `photonnet_kme_keystore_${Date.now()}.json`;
                                document.body.appendChild(a);
                                a.click();
                                document.body.removeChild(a);
                                URL.revokeObjectURL(url);
                            }, title: "KeyDeliveryStore'daki T\u00DCM teslim edilebilir G\u00DCVENL\u0130 anahtarlar\u0131 (GER\u00C7EK anahtar materyali \u2014 hassas!) bb84/etsi014_kme_server.js'in --keystore= ile do\u011Frudan i\u00E7e aktarabilece\u011Fi ETSI GS QKD 014 bi\u00E7iminde d\u0131\u015Fa aktar\u0131r", style: { padding: "3px 8px", borderRadius: 5, border: "1px solid #22d3ee55",
                                background: "#22d3ee22", color: "#22d3ee", fontSize: 8, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" } }, "\uD83D\uDD10 KME Anahtar Deposu (JSON)")));
                })(),
                paQueue && paQueue.length > 0 && (React.createElement("div", { style: { marginTop: 6, paddingTop: 6, borderTop: "1px solid #ffffff0f", display: "flex", flexDirection: "column", gap: 4 } }, paQueue.map(e => (React.createElement("div", { key: `${e.routeKey}-${e.blockIndex}`, style: { display: "flex", gap: 8, alignItems: "center", fontSize: 8, color: "#fbbf24", fontFamily: "monospace",
                        background: "#78350f22", borderRadius: 6, padding: "4px 8px" }, title: "Toeplitz gizlilik y\u00FCkseltme, b\u00FCy\u00FCkl\u00FC\u011F\u00FC nedeniyle (n\u00B7ell e\u015Fi\u011Fi a\u015F\u0131ld\u0131) Web Worker havuzunda ARKA PLANDA hesaplan\u0131yor \u2014 taray\u0131c\u0131 aray\u00FCz\u00FC BLOKLANMAZ, tamamlan\u0131nca ayr\u0131ca loglanacak" },
                    ...
                    React.createElement("span", null,
                        "\u2113=",
                        e.ell,
                        "bit gizlilik y\u00FCkseltme Web Worker havuzunda \u00E7al\u0131\u015F\u0131yor\u2026"),
                    React.createElement("span", { style: { color: "#a8a29e" } },
                        "(",
                        ((Date.now() - e.startedAt) / 1000).toFixed(0),
                        "s)"))))))))))));
}
// ══════════════════════════════════════════════════════════════
// ANA BİLEŞEN
...
function PhotonNet() {
    ...
    useEffect(() => { var _a; lastRouteDistanceRef.current = (_a = result === null || result === void 0 ? void 0 : result.totalKm) !== null && _a !== void 0 ? _a : null; }, [result]);
    ...
    useEffect(() => { var _a; quantumStateLogRef.current = (_a = result === null || result === void 0 ? void 0 : result.quantumStateLog) !== null && _a !== void 0 ? _a : []; }, [result]);
    // ══════════════════════════════════════════════════════════
    ...
    useEffect(() => {
        ...
        if (sp.src && nodes.find(n => n.id === sp.src))
            setSrc(sp.src);
        if (sp.dst && nodes.find(n => n.id === sp.dst))
            setDst(sp.dst);
        if (sp.msg)
            setMsg(decodeURIComponent(sp.msg));
        if (sp.ecc != null)
            setEcc(sp.ecc === "1" || sp.ecc === "true");
        if (sp.evesdrop != null)
            setEvesdrop(sp.evesdrop === "1" || sp.evesdrop === "true");
        if (sp.seed) {
            const parsedSeed = parseInt(sp.seed, 10);
            if (!isNaN(parsedSeed))
                setPendingSeedFromUrl(parsedSeed >>> 0);
        }
    ...
    useEffect(() => {
        ...
        if (typeof navigator !== "undefined" && "serviceWorker" in navigator) {
            (_b = (_a = navigator.serviceWorker).getRegistrations) === null || _b === void 0 ? void 0 : _b.call(_a).then(regs => {
                if (regs && regs.length)
                    addLog(`PWA: ${regs.length} service worker kaydı aktif`, "SYS");
            }).catch(() => { });
        }
    }, []);
    ...
    useEffect(() => {
        ...
        bc.onmessage = (e) => {
            var _a;
            const m = e.data;
            if (!isValidBcPacket(m) || m.userId === myUserId)
                return;
            ...
            if (m.type === BC_MSG_TYPES.TELEMETRY) {
                ...
                if (tp.raw_syndrome != null) {
                    const localSyndrome = ((_a = quantumStateLogRef.current) === null || _a === void 0 ? void 0 : _a.length)
                    ...
                    addLog(`◈ SENDROM SİMETRİSİ: yerel=${localSyndrome.toString(2).padStart(3, "0")} `
                        + `uzak=${remoteSyndrome.toString(2).padStart(3, "0")} XOR=${xorSymmetry} `
                        + `${isComplementary ? "(TAM TÜMLEYEN — faz koruması kararlı)" : ""}`, isComplementary ? "OK" : "INFO");
        ...
        const cleanup = setInterval(() => {
            setRemoteUsers(prev => {
                const n = { ...prev };
                Object.keys(n).forEach(k => { if (Date.now() - n[k].lastSeen > 10000)
                    delete n[k]; });
                return n;
            });
        }, 5000);
        ...
        rafEngine.register("satellite-link-tick", 900, () => {
            ...
            setLinkLoad(prev => {
                ...
                LINKS.forEach(lk => {
                    var _a;
                    const k = `${lk.a}-${lk.b}`;
                    const live = Math.random() < .2 ? Math.random() * .9 : ((_a = prev[k]) !== null && _a !== void 0 ? _a : 0) * .9 + Math.random() * .05;
                    ...
                    const hist = timelineRef.current.filter(t => t.segs && t.segs.some(s => s && ((s.a === lk.a && s.b === lk.b) || (s.a === lk.b && s.b === lk.a))));
    ...
    useEffect(() => {
        ...
        const watchdogIo = { setNodes, setNodeHealth, setActiveAnomalies, addLog: (t, ty) => { var _a; return (_a = addLogRef.current) === null || _a === void 0 ? void 0 : _a.call(addLogRef, t, ty); }, setWatchdogStats,
        ...
        rafEngine.register("watchdog-vitals-tick", 1200, () => {
            ...
            setNodeHealth(prev => {
                ...
                nodesRef.current.forEach(n => {
                    if (!n.on || nodeWatchdog.isBusy(n.id) || nodeTrendActiveRef.current === n.id)
                        return;
            ...
            if (Math.random() < WATCHDOG_ORGANIC_ANOMALY_CHANCE) {
                ...
                if (pool.length) {
                    ...
                    (_a = addLogRef.current) === null || _a === void 0 ? void 0 : _a.call(addLogRef, `🛡️ WATCHDOG: pasif arka-plan taramasında ${victim.id} düğümünde anomali sinyali yakalandı`, "WARN");
    ...
    useEffect(() => {
        ...
        if (watchdogLogStreakRef.current === streakKey)
            return; // bu seri zaten işlendi
        ...
        if (commonFaultEdge) {
            ...
            if (!alreadyQuarantined && !linkOutageController.running) {
                addLog(`🛡️ WATCHDOG: log akışında anormal örüntü tespit edildi (ardışık ${lastThree.length} başarısız iletim, ${fSrc}→${fDst}) — ortak payda bir düğüm değil, `
                    ...
                    + `HAT karantinaya alınıyor, yönlendirme motoru alternatif fiziksel yol arayacak`, "WARN");
                ...
                linkOutageController.injectOutage({ linkKey: linkKeyAB, durationMs }, setLinkDown).then(() => {
                    ...
                    (_a = addLogRef.current) === null || _a === void 0 ? void 0 : _a.call(addLogRef, `✅ WATCHDOG: ${commonFaultEdge.a}-${commonFaultEdge.b} hattı karantinadan çıkarıldı, yeniden değerlendirmeye açıldı`, "OK");
                }).catch(() => {
                    setActiveLinkQuarantines(prev => { const n = { ...prev }; delete n[linkKeyAB]; return n; });
                });
        ...
        const culprit = candidates[Math.floor(candidates.length / 2)]
            || ((_a = nodesRef.current.filter(n => n.on && n.id !== fSrc && n.id !== fDst && !nodeWatchdog.isBusy(n.id))[0]) === null || _a === void 0 ? void 0 : _a.id);
        if (!culprit || nodeWatchdog.isBusy(culprit))
            return;
    ...
    useEffect(() => {
        ...
        rafEngine.register("predictive-telemetry-tick", PREDICTIVE_TICK_MS, () => {
            ...
            BACKBONE_LINKS.forEach(l => {
                var _a, _b;
                const key = `${l.a}-${l.b}`;
                const load = (_b = (_a = linkLoadRef.current[key]) !== null && _a !== void 0 ? _a : linkLoadRef.current[`${l.b}-${l.a}`]) !== null && _b !== void 0 ? _b : 0;
                ...
                if (res)
                    insights.push({ ...res, kind: "link", label: `${l.a}-${l.b}` });
            });
            nodesRef.current.forEach(n => {
                var _a, _b;
                ...
                if (!h)
                    return;
                predictiveEngine.ingest(`node-cpu:${n.id}`, ((_a = h.cpu) !== null && _a !== void 0 ? _a : 0) / 100, now);
                predictiveEngine.ingest(`node-mem:${n.id}`, ((_b = h.mem) !== null && _b !== void 0 ? _b : 0) / 100, now);
                ...
                if (cpuRes)
                    insights.push({ ...cpuRes, kind: "node", label: `${n.id} (CPU)`, nodeId: n.id, metric: "CPU" });
                if (memRes)
                    insights.push({ ...memRes, kind: "node", label: `${n.id} (BELLEK)`, nodeId: n.id, metric: "BELLEK" });
            });
                ...
                if (ins.level === "WARNING" || ins.level === "CRITICAL") {
                    if (!wasAlerted) {
                        ...
                        const etaTxt = ins.etaMs != null
                            ...
                            : "yakın vadede";
                        (_a = addLogRef.current) === null || _a === void 0 ? void 0 : _a.call(addLogRef, `🔮 TAHMİN: ${ins.label} için ${etaTxt} darboğaz/gecikme öngörülüyor `
                            + `(risk %${(ins.risk * 100).toFixed(0)}, ivme ${ins.acceleration >= 0 ? "+" : ""}${ins.acceleration.toFixed(3)}/sn²)`, "WARN");
                    ...
                    if (ins.kind === "node" && ins.level === "CRITICAL"
                        ...
                        nodeWatchdog.triggerPreventiveMaintenance({ nodeId: ins.nodeId, reason: `artan ${ins.metric} trendi (risk %${(ins.risk * 100).toFixed(0)})` }, { setNodes, setActivePreventive, addLog: (t, ty) => { var _a; return (_a = addLogRef.current) === null || _a === void 0 ? void 0 : _a.call(addLogRef, t, ty); }, setPredictiveStats,
                            openDrillDown: watchdogAutoOpenDrill, onHealed: watchdogOnHealed });
                    }
                }
                else if (ins.level === "SAFE") {
                    if (wasAlerted)
                        predictiveAlertedRef.current.delete(ins.key); // risk gerçekten düştü — tekrar uyarılabilir
                    if (ins.kind === "node")
                        preventiveTriggeredRef.current.delete(ins.nodeId); // tekrar bakım tetiklenebilir
                }
            });
        }, "micro");
        rafEngine.start();
    ...
    useEffect(() => {
        rafEngine.register("noise-gate-calibration-tick", 2200, () => {
            ...
            if (tripping && !snspdTrippedRef.current) {
                ...
                (_a = addLogRef.current) === null || _a === void 0 ? void 0 : _a.call(addLogRef, `🌡️ SNSPD: sıcaklık sürüklenmesi kritik eşiği aştı (sapma ${noiseGateMiddleware.tempDriftMk.toFixed(2)}mK) — `
                    + `ham karanlık sayım oranı ${result.rawDarkRateHz.toFixed(0)}Hz'e çıktı, Gürültü Kapısı kalibrasyonu devreye giriyor`, "WARN");
            }
            else if (!tripping && snspdTrippedRef.current) {
                snspdTrippedRef.current = false;
                (_b = addLogRef.current) === null || _b === void 0 ? void 0 : _b.call(addLogRef, `🌡️ SNSPD: sıcaklık sapması nominal aralığa döndü (${noiseGateMiddleware.tempDriftMk.toFixed(2)}mK)`, "OK");
            }
            const narrowed = result.gateRatio < 0.999;
            if (narrowed && !gateNarrowedRef.current) {
                gateNarrowedRef.current = true;
                (_c = addLogRef.current) === null || _c === void 0 ? void 0 : _c.call(addLogRef, `🔬 GÜRÜLTÜ KAPISI: zaman-korelasyon penceresi %${(result.gateRatio * 100).toFixed(0)}'e daraltıldı — `
                    + `kalibre edilen karanlık oranı hedef bütçeyi aştığı için otonom olarak sıkılaştırıldı`, "WARN");
            }
            else if (!narrowed && gateNarrowedRef.current) {
                gateNarrowedRef.current = false;
                (_d = addLogRef.current) === null || _d === void 0 ? void 0 : _d.call(addLogRef, `🔬 GÜRÜLTÜ KAPISI: pencere nominal genişliğe geri gevşetildi — gürültü bütçesi normale döndü`, "OK");
            }
        }, "micro");
        rafEngine.start();
    ...
    useEffect(() => {
        rafEngine.register("coefficient-evolution", 4000, () => {
            ...
            if (!calm)
                return; // yoğun ağda evrim durur — hesaplama bütçesi gerçek trafiğe ayrılır
                ...
                ? loads.map(l => {
                    ...
                    return ({
                        ...
                        historicalBer: berHistoryRef.current.length
                            ? ((_a = berHistoryRef.current[berHistoryRef.current.length - 1]) !== null && _a !== void 0 ? _a : 0) / 100
                            : 0,
                    });
                })
            ...
            if (champion) {
                ...
                if (improved) {
                    (_a = addLogRef.current) === null || _a === void 0 ? void 0 : _a.call(addLogRef, `🧬 MAKRO→MİKRO ENJEKSİYON: epoch ${coeffEvolution.generation} → yeni kalibrasyon α=${lega.alpha.toFixed(3)} β=${lega.beta.toFixed(3)} (fitness=${champion.fitness.toFixed(3)}) — donanım bir sonraki kalibrasyona kadar bu sınırlarla çalışacak`, "SYS");
                    (_b = addLogRef.current) === null || _b === void 0 ? void 0 : _b.call(addLogRef, `🎯 DÜĞÜM REDDİ EĞİTİMİ: bu şampiyonun ölçülen gerçek ret oranı %${(championRejectionRate * 100).toFixed(1)} (ceza ağırlığı=${nodeRejectionPenaltyWeight.toFixed(2)}) — nesiller boyunca izlenmeli`, championRejectionRate < 0.3 ? "OK" : "WARN");
                    if (coeffEvolution._lastInjectionAffectedCells > 0) {
                        (_c = addLogRef.current) === null || _c === void 0 ? void 0 : _c.call(addLogRef, `🔧 REZONANS SENKRONU: ${coeffEvolution._lastInjectionAffectedCells} hücrenin Aşama-1 rezonans profili (NodeTransitGate) yeni kalibrasyonla hizalandı — kapıda "eski profil" reddi önlendi`, "OK");
                    }
                }
            }
        }, "macro");
    ...
    useEffect(() => {
        rafEngine.register("corridor-sync", 500, () => {
            ...
            setActiveCorridorNodes(prev => {
                if (prev.length === active.length && prev.every((p, i) => { var _a; return p.nodeId === ((_a = active[i]) === null || _a === void 0 ? void 0 : _a.nodeId); }))
                    return prev;
                return active;
            });
        }, "micro");
    ...
    useEffect(() => {
        rafEngine.register("chaos-sweep", 350, () => {
            ...
            if (result.orphanCount > 0) {
                ...
                (_a = addLogRef.current) === null || _a === void 0 ? void 0 : _a.call(addLogRef, `🛡 KAOS ENGELLEYİCİ: ${result.orphanCount} hayalet paket tespit edildi → anti-paketlerle nötrleştirildi `
                    + `(o anki makro kalibrasyon: epoch ${cal.epoch}, α=${cal.alpha.toFixed(2)} β=${cal.beta.toFixed(2)})`, "WARN");
            }
            ...
            if (wokeEarly) {
                (_b = addLogRef.current) === null || _b === void 0 ? void 0 : _b.call(addLogRef, `⏰ MAKRO SAAT ERKEN UYANDIRILDI: bellek doluluğu %${(occupancy * 100).toFixed(0)} `
                    + `≥ %${(rafEngine.OCCUPANCY_WAKE_THRESHOLD * 100).toFixed(0)} eşiği — Genetik Algoritma normal döngüsünü beklemeden tetiklenecek`, "WARN");
            }
            ...
            const packetsForFilter = ghostPktsRef.current.map(g => {
                ...
                return ({
                    id: g.id,
                    bornAt: (_a = g.bornAt) !== null && _a !== void 0 ? _a : Date.now(),
            ...
            setGhostPkts(prev => prev
                ...
                .map(g => { var _a; return ({ ...g, signalStrength: (_a = landing.strengths[g.id]) !== null && _a !== void 0 ? _a : 1.0 }); }));
            if (landing.evictIds.length > 0) {
                if (landing.mode === "emergency") {
                    (_c = addLogRef.current) === null || _c === void 0 ? void 0 : _c.call(addLogRef, `⚠ SOFT-LANDING (ACİL): hücre doluluğu %${(occupancy * 100).toFixed(0)} — ${landing.evictIds.length} paket `
                        + `hızlı sönümleme eğrisini tamamlayıp tahliye edildi (~${softLandingFilter.EMERGENCY_DECAY_END_MS}ms, sahiplik gözetilmedi)`, "ERR");
                }
                else {
                    (_d = addLogRef.current) === null || _d === void 0 ? void 0 : _d.call(addLogRef, `SOFT-LANDING (nazik): ${landing.evictIds.length} paket yumuşak sönümleme eğrisini tamamlayıp tahliye edildi `
                        + `(~${softLandingFilter.GRACEFUL_DECAY_END_MS}ms)`, "INFO");
                }
            }
            ...
            setAntiPackets(prev => {
                ...
                if (prev.length === active.length && active.length === 0)
                    return prev;
                return active;
            });
        }, "micro");
    ...
    useEffect(() => { if (logRef.current)
        logRef.current.scrollTop = logRef.current.scrollHeight; }, [log]);
    ...
    useEffect(() => {
        keyPoolBuffer.onBlockPrivacyAmplified = (info) => {
            ...
            if (info.error) {
                (_a = addLogRef.current) === null || _a === void 0 ? void 0 : _a.call(addLogRef, `❌ BLOK #${info.blockIndex} (${info.routeKey}) GİZLİLİK YÜKSELTME HATASI (Worker havuzu): ${info.error}`, "ERR");
            }
            else {
                ...
                (_b = addLogRef.current) === null || _b === void 0 ? void 0 : _b.call(addLogRef, `🔒 BLOK #${info.blockIndex} (${info.routeKey}) GİZLİLİK YÜKSELTME TAMAMLANDI (arka planda, Web Worker havuzu): ℓ=${info.finalKeyBitsLen}bit güvenli anahtar üretildi${kmeStr}`, "OK");
            }
        };
    ...
    const hwConnect = useCallback(async () => {
        ...
        try {
            ...
            if (hwEventSourceRef.current) {
                hwEventSourceRef.current.close();
            }
    ...
    useEffect(() => {
        return () => { if (hwEventSourceRef.current)
            hwEventSourceRef.current.close(); };
    }, []);
    // ══════════════════════════════════════════════════════════
    ...
    const getLinkXY = lk => { const a = getNode(lk.a), b = getNode(lk.b); if (!a || !b)
        return null; return { x1: a.x, y1: a.y, x2: b.x, y2: b.y }; };
    ...
    async function animPkt(pathArr, color, label) {
        for (let i = 0; i < pathArr.length - 1; i++) {
            const nA = getNode(pathArr[i].node), nB = getNode(pathArr[i + 1].node);
            if (!nA || !nB)
                continue;
    ...
    async function transmit() {
        ...
        if (q < .3)
            addLog("UYDU KALİTESİ DÜŞÜK — alternatif değerlendiriliyor", "WARN");
        ...
        if (evesdrop)
            addLog("BB84: DİNLEME MODU AKTİF", "WARN");
        // DİNAMİK YÜK DENGELEME / AKILLI ROTALAMA — bkz. LinkOutageController
        ...
        if (activeCrisis) {
            ...
            if (baselineUsesCrisisLink && !usesCrisisLink) {
                ...
                addLog(`🔀 ROTA OPTİMİZE EDİLDİ: ${activeCrisis.a}-${activeCrisis.b} hattı ${activeCrisis.mode === "outage" ? "KOPUK" : "AŞIRI YOĞUN"} — trafik `
                    + `${viaNodes.length ? viaNodes.join("→") : "doğrudan alternatif hat"} üzerinden yönlendirildi (yeni rota: ${p.map(x => x.node).join("→")}, ${totalKm}km)`, "WARN");
            }
            ...
            if (riskyLinkEntries.length) {
                ...
                if (baselineUsesRisky && !usesRisky) {
                    addLog(`🔮 ÖNLEYİCİ YÖNLENDİRME: öngörülen risk taşıyan hat(lar) proaktif olarak atlandı — trafik `
                        + `${p.map(x => x.node).join("→")} üzerinden yönlendirildi (kriz OLUŞMADAN ÖNCE önlem alındı)`, "WARN");
                }
            }
        }
        ...
        if (!health.healthy) {
            const reasons = [];
            if (health.tooManyHops)
                reasons.push(`${health.hopCount} hop (sağlıklı sınır: 6)`);
            if (health.criticallyLowTransmittance)
                reasons.push(`uçtan-uca geçirgenlik %${(health.cumulativeTransmittance * 100).toFixed(3)}`);
        ...
        if (usingTrustedChain) {
            ...
            addLog(`BB84 ZİNCİR: ort. QBER=${(qber * 100).toFixed(1)}% · en-kötü-hop QBER=${(qkdChain.worstHop.qber * 100).toFixed(1)}% ${eavesdropDetected ? "— DİNLEME İSTATİSTİKSEL OLARAK TESPİT EDİLDİ (en az bir hop eşiği aştı)" : "(tüm hoplar güvenli aralıkta)"}`, eavesdropDetected ? "ERR" : "OK");
            ...
            addLog(`BB84: QBER=${(qber * 100).toFixed(1)}% ${eavesdropDetected ? "— DİNLEME İSTATİSTİKSEL OLARAK TESPİT EDİLDİ" : "(güvenli aralıkta)"}`, eavesdropDetected ? "ERR" : "OK");
        }
        ...
        addLog(`OTP: mesaj sifted key ile şifrelendi (${otpKeyBits.length}bit anahtar) → deşifre bütünlüğü ${otpIntegrityOk ? "✓" : "✗"}`, otpIntegrityOk ? "OK" : "ERR");
        // ══════════════════════════════════════════════════════════
        ...
        const atmosphericConditions = {
            ...
            rangeKm: totalKm,
            elevationDeg: (_a = curSatForTx === null || curSatForTx === void 0 ? void 0 : curSatForTx.elev) !== null && _a !== void 0 ? _a : 45,
            turbulenceStrength,
            pointingPrecision,
        };
        ...
        const simResult = physicalSimulation(segs, bits, entanglementSeed, evesdrop, getNode, { linkLoad, historicalBer: avgHistoricalBer, opllEnabled: opllOn, atmosphericConditions, dynamicFecEnabled: true });
        ...
        segReport.forEach((sr, si) => {
            ...
            if (sr.usingMsgRelay) {
                addLog(`  ↳ 🔗 GÜVENİLİR-DÜĞÜM RÖLE ZİNCİRİ (mesaj kanalı): ${sr.link.km}km tek link tek-atış EDFA telafisi için çok uzun `
                    + `(bkz. propPhotonRelayChain — analog amplifikasyonun %5 kalıntı-kayıp tabanı bu mesafede yetersiz) — `
                    + `${sr.msgRelayHops} hop × ${(sr.link.km / sr.msgRelayHops).toFixed(0)}km'e bölündü, her hop KENDİ bağımsız fotonuyla ölç-ve-yeniden-gönder`, "SYS");
            }
            if (sr.fecRedundancy > 1) {
                addLog(`  ↳ 🔁 DİNAMİK FEC: kanal zayıf (T=${sr.transmitPct}%) — bit başına ×${sr.fecRedundancy} foton çoğullama devreye girdi`
                    + `${sr.fecRescued ? `, ${sr.fecRescued} bit ek kopyalarla kurtarıldı` : ""}`
                    + `${sr.fecExhausted ? `, ${sr.fecExhausted} bit TÜM kopyalara rağmen yine kayıp` : ""}`, sr.fecExhausted ? "WARN" : "OK");
            }
            if (sr.lost)
                addLog(`  ↳ ${sr.lost} foton kayıp @ ${sr.link.a}→${sr.link.b}`, "ERR");
            if (sr.flipped)
                addLog(`  ↳ ${sr.flipped} bit bozuldu @ ${sr.link.a}→${sr.link.b}`, "WARN");
            if (sr.absorptionBarrierDetected) {
                addLog(`⚠ ANİ SOĞURULMA BARİYERİ: ${sr.link.a}→${sr.link.b} hattında λ=${sr.link.nm}nm için `
                    + `${sr.maxConsecutiveAbsorb} ardışık ABSORB olayı — alıcı düğümde (${sr.link.b}) veri boşluğu oluştu, ECC bu boşluğu telafi etmeye çalışıyor`, "ERR");
            }
        ...
        if (gateReport && gateReport.checked > 0) {
            addLog(`GATE: ${gateReport.checked} foton düğüm triyajından geçti → `
                + `Faz1-red:${gateReport.phase1Rejected} Faz2-red:${gateReport.phase2Rejected} Faz3-geçiş:${gateReport.phase3Passed}`, gateReport.phase3Passed >= gateReport.checked * 0.7 ? "OK" : "WARN");
        ...
        if (evesdrop && ec.EAVES)
            addLog("BB84: DİNLEME TESPİT EDİLDİ!", "ERR");
        // EK 24: Ses efekti — kullanıcı ayarı açıksa gerçek Web Audio tonu çalınır
        if (soundOn) {
            try {
                success ? playSuccessChime() : playErrorGlitch();
            }
            catch { }
        }
        ...
        const res = {
            ...
            legaSnapshot: segs[0] ? lega.computeThresholds(segs[0].nm, segs[0].km, { linkLoad: (_b = linkLoad[`${segs[0].a}-${segs[0].b}`]) !== null && _b !== void 0 ? _b : 0, historicalBer: avgHistoricalBer }) : null, // LEGA: bu iletim için hesaplanan gerçek dinamik eşikler
            ...
            classicalAuth: { ok: authResult.ok, tag: authResult.tag, poolRemaining: (_c = authResult.poolRemaining) !== null && _c !== void 0 ? _c : classicalAuthChannel.pool.length, reason: (_d = authResult.reason) !== null && _d !== void 0 ? _d : null }, // Wegman-Carter kimlik doğrulama sonucu
        ...
        const persistAndBroadcast = () => {
            ...
            if (bcRef.current) {
                bcRef.current.postMessage(buildBcPacket(BC_MSG_TYPES.TRANSMIT_EVENT, myUserId, buildTransmitPayload(src, dst, success)));
                ...
                if (res.quantumStateLog && res.quantumStateLog.length) {
                    bcRef.current.postMessage(buildBcPacket(BC_MSG_TYPES.TELEMETRY, myUserId, buildTelemetryPayload({
            ...
            if (chiralAudioOn && res.qber != null) {
                chiralEngine.updateFromTelemetry(res.qber, ((_a = res.chirality) === null || _a === void 0 ? void 0 : _a.symmetryRatio) != null ? (1 - res.chirality.symmetryRatio) : 0);
            }
            // Görsel WDM animasyonu arka planda oynatılır — sonucu beklemeden gösterir
            Promise.all([1550, 1310, 850].map((nm, i) => delay(i * 200).then(() => animPkt(p, WL[nm].hex, `${nm}`))));
    ...
    function retroCauseTimeline(entryId) {
        ...
        if (!entry) {
            addLog(`RETRO-CAUSE HATA: kayıt bulunamadı (${entryId})`, "ERR");
            return;
        }
        ...
        addLog(`⏪ RETRO-CAUSE: "${entry.msg}" alternatif kuantum tesadüfüyle yeniden çalıştırıldı → `
            + `"${forkResult.decoded}" ${diverged ? "(orijinalden FARKLI — dallanma gerçekleşti)" : "(orijinalle aynı sonuç)"} `
            + `QBER=${(forkResult.qber * 100).toFixed(1)}%`, diverged ? "WARN" : "OK");
    }
    // ══════════════════════════════════════════════════════════════
    ...
    async function runMirrorUniverse(entanglementSeed, p, segs, msgSnapshot, eccSnapshot, evesdropSnapshot) {
        ...
        if (result && result.quantumStateLog && mirror.quantumStateLog) {
            ...
            addLog(`⚛ KİRALİTE: ${chirality.chiralityState} (${chirality.conjugateCount} eşlenik / ${chirality.phaseDeltaCount} faz-farkı blok)`, chirality.symmetryRatio > 0.5 ? "OK" : "WARN");
        ...
        addLog(`🪞 AYNA EVREN: BER=${mirror.er}% success=${mirror.success} (seed=${(entanglementSeed ^ 0xFFFFFFFF) >>> 0})`, mirror.success ? "OK" : "WARN");
    ...
    function interfereUniverses() {
        if (!result || !mirrorResult) {
            addLog("INTERFERENCE: her iki evren de henüz hesaplanmadı", "WARN");
            return null;
        }
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (a === b) {
                ...
                interfered.push(Math.random() < 0.5 ? 0 : 1);
                annihilated++;
            } // zıt faz → sönümlenir (rastgele çöker)
        }
    ...
    async function replayTimeline(entryId) {
        const entry = timeline.find(t => t.id === entryId);
        if (!entry) {
            addLog(`REPLAY HATA: kayıt bulunamadı (${entryId})`, "ERR");
            return;
        }
        if (replaying.includes(entryId))
        ...
        try {
            ...
            addLog(`⧉ RE-RUN: BER=${recomputed.er}% success=${recomputed.success} `
                + `${matchesOriginal ? "(orijinalle birebir eşleşti ✓)" : "(orijinalden FARKLI — seed/algoritma tutarsızlığı!)"}`, matchesOriginal ? "OK" : "ERR");
        }
        catch (e) {
            ...
            for (let si = 0; si < segs.length; si++) {
                ...
                for (let s = 0; s <= steps; s++) {
                    ...
                    setGhostPkts(prev => {
                        ...
                        const collidingOthers = next.filter(g => g.id !== gid && g.sourceId !== entryId && Math.hypot(g.x - gx, g.y - gy) < 14);
                        if (collidingOthers.length > 0)
                            setInterference(v => v + 1);
    ...
    function entangleCompare(idA, idB) {
        ...
        const eventDiff = [...allTypes].map(type => {
            ...
            return ({
                type,
                a: (_a = a.result.ec[type]) !== null && _a !== void 0 ? _a : 0,
                b: (_b = b.result.ec[type]) !== null && _b !== void 0 ? _b : 0,
            });
        });
    ...
    function collapseSuperposition(spId, chosenMemberId) {
        setSuperpositions(sp => sp.map(s => s.id === spId ? { ...s, collapsedTo: chosenMemberId } : s));
    ...
    useEffect(() => {
        if (tab !== "GRAPH") {
            rafEngine.unregister("force-directed-graph");
            return;
        }
        ...
        rafEngine.register("force-directed-graph", 60, () => {
            ...
            ids.forEach(id => {
                ...
                fx += (400 - next[id].x) * 0.003;
                fy += (250 - next[id].y) * 0.003;
                next[id] = {
    ...
    async function triggerBurstTest(profile = "spike") {
        ...
        try {
            await burstTrafficGenerator.generateBurst({ profile, peak: 0.95, durationMs: profile === "sawtooth" ? 6000 : 3000, linkKeys: ["BURST-TEST"] }, setLinkLoad, (value) => {
    ...
    async function injectCrisis() {
        if (!crisisLinkSel) {
            addLog("KRİZ ENJEKSİYONU: önce bir hat seçin", "WARN");
            return;
        }
        if (activeCrisis) {
            addLog("KRİZ ENJEKSİYONU: zaten aktif bir kriz var — önce iptal edin", "WARN");
            return;
        }
        const [a, b] = crisisLinkSel.split("-");
        const link = links.find(l => (l.a === a && l.b === b) || (l.a === b && l.b === a));
        if (!link) {
            addLog("KRİZ ENJEKSİYONU: hat bulunamadı", "ERR");
            return;
        }
        ...
        try {
            if (crisisMode === "outage") {
                ...
                await burstTrafficGenerator.generateBurst({ profile: "plateau", peak: 0.98, durationMs, linkKeys: [crisisLinkSel] }, setLinkLoad);
    ...
    function cancelCrisis() {
        if (!activeCrisis)
            return;
        if (activeCrisis.mode === "outage")
            linkOutageController.cancel();
        else
            burstTrafficGenerator.cancel();
    ...
    async function injectAnomaly() {
        if (!anomalyNodeSel) {
            addLog("WATCHDOG DEMO: önce bir düğüm seçin", "WARN");
            return;
        }
        ...
        if (nodeWatchdog.isBusy(anomalyNodeSel)) {
            addLog("WATCHDOG DEMO: bu düğüm zaten meşgul (aktif anomali veya proaktif bakım)", "WARN");
            return;
        }
        await nodeWatchdog.triggerAnomaly({ nodeId: anomalyNodeSel, type: anomalyTypeSel }, { setNodes, setNodeHealth, setActiveAnomalies, addLog, setWatchdogStats,
            openDrillDown: watchdogAutoOpenDrill, onHealed: watchdogOnHealed });
    }
    ...
    async function injectLinkTrend() {
        if (!trendLinkSel) {
            addLog("TREND ENJEKSİYONU: önce bir hat seçin", "WARN");
            return;
        }
        if (burstTrafficGenerator.running) {
            addLog("TREND ENJEKSİYONU: BurstTrafficGenerator zaten meşgul", "WARN");
            return;
        }
        const [a, b] = trendLinkSel.split("-");
        const link = links.find(l => (l.a === a && l.b === b) || (l.a === b && l.b === a));
        if (!link) {
            addLog("TREND ENJEKSİYONU: hat bulunamadı", "ERR");
            return;
        }
        ...
        try {
            await burstTrafficGenerator.generateBurst({ profile: "plateau", peak: 0.95, durationMs: 28000, linkKeys: [trendLinkSel] }, setLinkLoad);
    ...
    async function injectNodeTrend() {
        if (!trendNodeSel) {
            addLog("TREND ENJEKSİYONU: önce bir düğüm seçin", "WARN");
            return;
        }
        if (trendNodeSel === src || trendNodeSel === dst) {
            addLog("TREND ENJEKSİYONU: aktif kaynak/hedef düğüm seçilemez", "WARN");
            return;
        }
        if (metricTrendInjector.running) {
            addLog("TREND ENJEKSİYONU: bir düğüm trendi zaten aktif", "WARN");
            return;
        }
        ...
        try {
            await metricTrendInjector.run({ peak: 0.97, durationMs: 28000, baseline: 0.15 }, (v) => setNodeHealth(prev => ({ ...prev, [trendNodeSel]: {
                    ...(prev[trendNodeSel] || {}),
    ...
    function injectThermalFluctuation() {
        ...
        addLog(`🌡️ ISIL DALGALANMA ENJEKTE EDİLDİ: SNSPD bias noktası zorlandı (${before.toFixed(2)}mK → ${noiseGateMiddleware.tempDriftMk.toFixed(2)}mK) `
            + `— kalibrasyon turlarını ve 🔬 GÜRÜLTÜ sekmesini izleyin, sistem otonom olarak toparlanacak`, "SYS");
    }
    function toggleFiberLeak() {
        ...
        addLog(next
            ...
            : "💧 FİBER SIZINTISI GİDERİLDİ: sızıntı arka planı kaldırıldı", "SYS");
    }
    // ══════════════════════════════════════════════════════════
    // FAZ 3: MAKRO/MİKRO GÖRÜNÜM GEÇİŞLERİ (Hierarchical Drill-Down).
    ...
    function buildDrillData(nodeId) {
        ...
        if (node.reps > 0)
            submodules.splice(2, 0, { name: "EDFA Optik Amplifikatör", detail: `×${node.reps} tekrarlayıcı katmanı` });
        ...
        const sockets = connectedLinks.map(l => {
            ...
            const load = (_b = (_a = linkLoad[kAB]) !== null && _a !== void 0 ? _a : linkLoad[kBA]) !== null && _b !== void 0 ? _b : 0;
        ...
        const threads = Array.from({ length: poolSize }, (_, i) => {
            if ((anomaly === null || anomaly === void 0 ? void 0 : anomaly.type) === "lockup")
                return { id: i, state: "BLOCKED" };
            if ((anomaly === null || anomaly === void 0 ? void 0 : anomaly.type) === "unresponsive")
                return { id: i, state: "NO_RESPONSE" };
    ...
    const pathKm = path ? path.slice(1).reduce((s, p) => { var _a, _b; return s + ((_b = (_a = p.link) === null || _a === void 0 ? void 0 : _a.km) !== null && _b !== void 0 ? _b : 0); }, 0) : 0;
    ...
    return (React.createElement("div", { style: { height: "100vh", background: "#020409", color: "#e2e8f0",
  ...
  `),
        React.createElement("div", { style: { height: 46, background: "#030610", borderBottom: "1px solid #0a162888",
                display: "flex", alignItems: "center", padding: "0 14px", gap: 10, flexShrink: 0 } },
            React.createElement("div", { style: { width: 26, height: 26, borderRadius: 4,
                    background: "linear-gradient(135deg,#00d4ff22,#7c3aed22)",
                    border: "1px solid #00d4ff33",
                    display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13,
                    boxShadow: "0 0 10px #00d4ff22", flexShrink: 0 } }, "\u269B"),
            React.createElement("div", { style: { flexShrink: 0 } },
                React.createElement("div", { style: { fontSize: 12, fontWeight: 700, letterSpacing: ".14em",
                        color: "#00d4ff", textShadow: "0 0 10px #00d4ff55" } }, "PHOTONNET"),
                React.createElement("div", { style: { fontSize: 8, color: "#1e3a5f", letterSpacing: ".16em", marginTop: -1 } }, "QUANTUM OPTICAL NETWORK v2.5")),
            [
            ...
            ].map(s => (React.createElement("div", { key: s.l, style: { display: "flex", alignItems: "center", gap: 5,
                    background: "#0a1628", border: `1px solid ${s.c}22`,
                    borderRadius: 3, padding: "3px 8px", flexShrink: 0 } },
                React.createElement("div", { style: { width: 5, height: 5, borderRadius: "50%", background: s.c,
                        boxShadow: `0 0 5px ${s.c}`,
                        animation: s.blink ? "blink 2s ease-in-out infinite" : "none" } }),
                React.createElement("span", { style: { fontSize: 7, color: "#334155", letterSpacing: ".1em" } }, s.l),
                React.createElement("span", { style: { fontSize: 9, color: s.c, fontWeight: 700, letterSpacing: ".04em" } }, s.v)))),
            React.createElement("div", { style: { display: "flex", gap: 2, marginLeft: "auto" } }, [["MAP", "🌐 AĞ"], ["SAT", "🛰 UYDU"], ["TRF", "📊 TRAFİK"], ["TML", "⧉ ZAMAN ÇİZ."], ["GRAPH", "🕸 GRAPH"], ["WDG", "🛡 BEKÇİ"], ["PRED", "🔮 TAHMİN"], ["NGM", "🔬 GÜRÜLTÜ"], ["HW", "🔧 DONANIM"], ["LOG", "📋 KAYIT"]].map(([k, l]) => (React.createElement("button", { key: k, className: `ptab ${tab === k ? "on" : ""}`, onClick: () => setTab(k) }, l)))),
            React.createElement("button", { onClick: () => setSoundOn(v => !v), style: { background: "none", border: "none", cursor: "pointer", fontSize: 13,
                    marginLeft: 8, opacity: soundOn ? 1 : .35, flexShrink: 0 }, title: soundOn ? "Sesi kapat" : "Sesi aç" }, soundOn ? "🔊" : "🔇"),
            React.createElement("button", { onClick: toggleChiralAudio, style: { background: chiralAudioOn ? "#a855f722" : "none",
                    ...
                    display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }, title: "Kiral Akustik Motor \u2014 QBER/kiralite ile canl\u0131 FM ses" },
                "\uD83C\uDFB5 ",
                chiralAudioOn ? "KİRAL SES AÇIK" : "Kiral Ses"),
            React.createElement("button", { onClick: () => setOpllOn(v => !v), style: { background: opllOn ? "#00d4ff22" : "none",
                    border: `1px solid ${opllOn ? "#00d4ff55" : "transparent"}`,
                    borderRadius: 4, cursor: "pointer", fontSize: 11, padding: "3px 8px",
                    marginLeft: 6, color: opllOn ? "#00d4ff" : "#334155",
                    display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }, title: "Optik Faz Kilitli D\u00F6ng\u00FC (OPLL) \u2014 kilitlenen kanallarda faz kaymas\u0131 olas\u0131l\u0131\u011F\u0131n\u0131 bast\u0131r\u0131r" },
                "\u2307 ",
                opllOn ? "OPLL KİLİTLİ" : "OPLL"),
            opllOn && opticalPLL.isCrisis().inCrisis && (React.createElement("div", { title: "Katsay\u0131 enjeksiyon h\u0131z\u0131, fiziksel faz kilitlenmesinden daha agresif \u2014 ECC bu ko\u015Fulda otomatik bypass edilecek", style: { display: "flex", alignItems: "center", gap: 5, marginLeft: 6, flexShrink: 0,
                    fontSize: 9, fontFamily: "monospace", color: "#f43f5e" } },
                React.createElement("div", { style: { width: 5, height: 5, borderRadius: "50%", background: "#f43f5e",
                        boxShadow: "0 0 5px #f43f5e", animation: "blink 0.5s ease-in-out infinite" } }),
                "\u26A0 FAZ KR\u0130Z\u0130")),
            clockDrift && Math.abs(clockDrift.driftMs) > 50 && (React.createElement("div", { title: "Zaman Dilatasyonu (throttling sapmas\u0131)", style: { fontSize: 9, color: Math.abs(clockDrift.driftMs) > 150 ? "#f43f5e" : "#f59e0b",
                    fontFamily: "monospace", marginLeft: 6, flexShrink: 0 } },
                "\u23F1 ",
                clockDrift.driftMs > 0 ? "+" : "",
                clockDrift.driftMs.toFixed(0),
                "ms")),
            React.createElement("div", { title: evolutionActive ? "MAKRO ZAMAN: ağ sakin — epoch bazlı evrim çalışıyor (4sn periyot)" : "MAKRO ZAMAN: ağ yoğun — evrim duraklatıldı, hesaplama bütçesi trafiğe ayrıldı", style: { display: "flex", alignItems: "center", gap: 5, marginLeft: 8, flexShrink: 0,
                ...
                React.createElement("span", { style: { opacity: .6, fontSize: 7 } }, "MAKRO"),
                " N",
                evolutionGen,
                " \u03B1=",
                liveCoeffs.alpha.toFixed(2),
                " \u03B2=",
                liveCoeffs.beta.toFixed(2)),
            lastNodeRejectionRate != null && (React.createElement("div", { title: `Düğüm Reddi Cezası (ağırlık=${nodeRejectionPenaltyWeight.toFixed(2)}) — genetik algoritmanın probeGate ile ölçtüğü gerçek ret oranı`, style: { display: "flex", alignItems: "center", gap: 5, marginLeft: 8, flexShrink: 0,
                    fontSize: 9, fontFamily: "monospace", color: lastNodeRejectionRate < 0.3 ? "#22c55e" : "#f59e0b" } },
                "\uD83C\uDFAF RET %",
                (lastNodeRejectionRate * 100).toFixed(0))),
            React.createElement("div", { title: "M\u0130KRO ZAMAN: Kristal Bellek + Kuantum \u00D6n-Y\u00F6nlendirme + Negatif Sinyal \u2014 donan\u0131m-seviyesi deterministik, rAF native h\u0131z\u0131nda", style: { display: "flex", alignItems: "center", gap: 5, marginLeft: 8, flexShrink: 0,
                    fontSize: 9, fontFamily: "monospace", color: "#00d4ff" } },
                React.createElement("div", { style: { width: 5, height: 5, borderRadius: "50%", background: "#00d4ff",
                        boxShadow: "0 0 5px #00d4ff", animation: "blink 0.6s ease-in-out infinite" } }),
                React.createElement("span", { style: { opacity: .6, fontSize: 7 } }, "M\u0130KRO"),
                " ",
                rafEngine.systemsByTier("micro").length,
                " sistem"),
            (() => {
                const occ = rafEngine.occupancyStats();
                const pct = occ.lastOccupancy * 100;
                const color = pct >= 92 ? "#f43f5e" : pct >= 88 ? "#f59e0b" : "#334155";
                return (React.createElement("div", { title: `Bellek Haritası Doluluğu — %${occ.threshold * 100} eşiğinde makro saat erken uyanır (${occ.earlyWakeCount} kez tetiklendi)`, style: { display: "flex", alignItems: "center", gap: 5, marginLeft: 8, flexShrink: 0,
                        fontSize: 9, fontFamily: "monospace", color } },
                    "\uD83E\uDDE0 %",
                    pct.toFixed(0)));
            })(),
            softLandingMode === "emergency" && (React.createElement("div", { title: "Soft-Landing Filtresi AC\u0130L modda \u2014 %92 \u00E7\u00F6k\u00FC\u015F metri\u011Fi a\u015F\u0131ld\u0131, paketler sahiplik durumuna bak\u0131lmaks\u0131z\u0131n s\u00F6n\u00FCmleniyor", style: { display: "flex", alignItems: "center", gap: 5, marginLeft: 8, flexShrink: 0,
                    fontSize: 9, fontFamily: "monospace", color: "#f43f5e" } },
                React.createElement("div", { style: { width: 5, height: 5, borderRadius: "50%", background: "#f43f5e",
                        boxShadow: "0 0 5px #f43f5e", animation: "blink 0.4s ease-in-out infinite" } }),
                "\u26D1 SOFT-LANDING: AC\u0130L")),
            React.createElement("button", { onClick: () => triggerBurstTest("spike"), title: "Burst Traffic Generator \u2014 %95 tepe y\u00FCkl\u00FC yapay patlama tetikler (Dual-Clock erken-uyanma + Soft-Landing acil modunu test eder)", style: { background: burstActive ? "#f59e0b22" : "none",
            ...
            chaosStats.antiPacketsFired > 0 && (React.createElement("div", { title: "Kaos Engelleyici \u2014 n\u00F6trle\u015Ftirilen hayalet paketler", style: { display: "flex", alignItems: "center", gap: 5, marginLeft: 8, flexShrink: 0,
                    fontSize: 9, fontFamily: "monospace", color: "#f43f5e" } },
                "\uD83D\uDEE1 \u2298",
                chaosStats.antiPacketsFired)),
            React.createElement("div", { style: { fontSize: 9, color: "#1e3a5f", fontFamily: "monospace", flexShrink: 0, marginLeft: 8 } }, new Date().toLocaleTimeString("tr", { hour12: false }))),
        audioBlocked && (React.createElement("div", { onClick: retryAudioUnlock, style: {
            ...
            React.createElement("span", { style: { fontSize: 10, color: "#f59e0b", fontWeight: 600 } }, "Taray\u0131c\u0131 AudioContext'i otomatik ba\u015Flatmay\u0131 engelledi (autoplay policy)."),
            React.createElement("span", { style: { fontSize: 10, color: "#94a3b8" } }, "Kiral akustik motoru ba\u015Flatmak i\u00E7in buraya t\u0131klay\u0131n."),
            React.createElement("span", { style: { marginLeft: "auto", fontSize: 9, color: "#f59e0b",
                    border: "1px solid #f59e0b55", borderRadius: 5, padding: "3px 10px" } }, "\uD83D\uDD13 Kilidi A\u00E7"))),
        React.createElement("div", { style: { height: 36, background: "#030610", borderBottom: "1px solid #0a162855",
            ...
            ].map((m, i) => (React.createElement("div", { key: i, style: { display: "flex", flexDirection: "column", padding: "0 12px",
                    borderRight: "1px solid #0a162888", justifyContent: "center", height: "100%", flexShrink: 0 } },
                React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em" } }, m.l),
                React.createElement("div", { style: { fontSize: 10, color: m.c, fontWeight: 700, letterSpacing: ".04em" } }, m.v)))),
            React.createElement("div", { style: { padding: "0 12px", minWidth: 120, display: "flex", flexDirection: "column", justifyContent: "center" } },
                React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 2 } }, "THROUGHPUT"),
                React.createElement(Spark, { data: throughput, color: "#00d4ff", h: 16 }))),
        React.createElement(LeoPanel, { sats: sats, activeSat: activeSat, setActiveSat: setActiveSat, onSend: transmit, running: running, msg: msg, setMsg: setMsg, evesdrop: evesdrop, setEvesdrop: setEvesdrop, result: result, paQueue: paQueue, phase: phase, timeline: timeline, onOpenTimeline: () => setTab("TML"), mirrorOn: mirrorOn, setMirrorOn: setMirrorOn, mirrorResult: mirrorResult, onCollapse: collapseUniverse, onInterfere: interfereUniverses, collapsedTo: collapsedTo, entropyDelta: entropyDelta, isOnline: isOnline, observerFrame: observerFrame, atmosphericTurbidity: atmosphericTurbidity, setAtmosphericTurbidity: setAtmosphericTurbidity, turbulenceStrength: turbulenceStrength, setTurbulenceStrength: setTurbulenceStrength, pointingPrecision: pointingPrecision, setPointingPrecision: setPointingPrecision, nightMode: nightMode, setNightMode: setNightMode }),
        React.createElement("div", { style: { display: "flex", flex: 1, minHeight: 0 } },
            React.createElement("div", { style: { width: 180, flexShrink: 0, background: "#030610",
                    borderRight: "1px solid #0a162888",
                    display: "flex", flexDirection: "column", overflowY: "auto" } },
                React.createElement("div", { style: { padding: "10px 10px", borderBottom: "1px solid #0a162888" } },
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 6 } }, "S\u0130NYAL MON\u0130T\u00D6R"),
                    React.createElement(Oscilloscope, { active: running, color: "#00d4ff" }),
                    React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginTop: 3 } },
                        React.createElement("span", { style: { fontSize: 7, color: "#1e3a5f" } }, "\u03BB=1550nm"),
                        React.createElement("span", { style: { fontSize: 7, color: running ? "#00d4ff" : "#1e3a5f",
                                animation: running ? "blink 1s ease-in-out infinite" : "none" } }, running ? "● ACTIVE" : "○ IDLE"))),
                React.createElement("div", { style: { padding: "10px", borderBottom: "1px solid #0a162888" } },
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 6 } }, "KAYNAK / HEDEF"),
                    [["TX", src, setSrc, "dst"], [" RX", dst, setDst, "src"]].map(([l, val, set, excl]) => (React.createElement("div", { key: l, style: { marginBottom: 6 } },
                        React.createElement("div", { style: { fontSize: 7, color: "#334155", marginBottom: 2, display: "flex", gap: 5, alignItems: "center" } },
                            React.createElement("div", { style: { width: 4, height: 4, borderRadius: 1,
                                    background: l.includes("TX") ? "#00d4ff" : "#10b981" } }),
                            l),
                        React.createElement("select", { value: val, disabled: running, onChange: e => { set(e.target.value); setPath(null); setResult(null); }, style: { width: "100%", padding: "4px 6px", fontSize: 10 } }, online.filter(n => n.id !== (excl === "dst" ? dst : src))
                            .map(n => React.createElement("option", { key: n.id, value: n.id },
                            n.id,
                            " \u2014 ",
                            n.label))))))),
                React.createElement("div", { style: { padding: "8px 10px", borderBottom: "1px solid #0a162888" } },
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 6 } }, "PROTOKOL"),
                    [
                        { k: "ecc", s: ecc, set: setEcc, on: "HAMMING ECC", off: "ECC KAPALI", c: "#10b981" },
                    ].map(o => (React.createElement("label", { key: o.k, style: { display: "flex", alignItems: "center", gap: 6, cursor: "pointer",
                            background: o.s ? o.c + "11" : "transparent",
                            border: `1px solid ${o.s ? o.c + "33" : "#0a1628"}`,
                            borderRadius: 3, padding: "5px 7px", transition: "all .2s" } },
                        React.createElement("input", { type: "checkbox", checked: o.s, disabled: running, onChange: e => o.set(e.target.checked), style: { width: 11, height: 11, accentColor: o.c } }),
                        React.createElement("span", { style: { fontSize: 8, color: o.s ? o.c : "#334155", fontWeight: o.s ? 700 : 400 } }, o.s ? o.on : o.off))))),
                React.createElement("div", { style: { padding: "8px 10px", flex: 1 } },
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 6 } }, "D\u00DC\u011E\u00DCM MATR\u0130S\u0130"),
                    nodes.map(n => {
                        const locked = n.id === src || n.id === dst;
                        const nc = { hub: "#00d4ff", intl: "#f59e0b", node: "#10b981" }[n.type] || "#10b981";
                        return (React.createElement("div", { key: n.id, className: "nrow", onClick: () => { toggleNode(n.id); setSelNode(n.id === selNode ? null : n.id); } },
                            React.createElement("div", { style: { width: 5, height: 5, borderRadius: 1, flexShrink: 0,
                                    background: n.on ? nc : "#1e293b", opacity: n.on ? 1 : .4 } }),
                            React.createElement("span", { style: { fontSize: 8, color: n.on ? (pathIds.has(n.id) ? nc : "#475569") : "#1e293b",
                                    flex: 1, letterSpacing: ".02em" } }, n.id),
                            React.createElement("span", { style: { fontSize: 7, color: locked ? "#7c3aed" : n.on ? "#1e3a5f" : "#0f172a" } }, locked ? (n.id === src ? "TX" : "RX") : n.on ? "ON" : "OFF"),
                            React.createElement("span", { title: "Mikro katmana zoom-in", onClick: (e) => { e.stopPropagation(); openDrillDown(n.id); }, style: { fontSize: 8, cursor: "zoom-in", opacity: .5, padding: "0 2px" }, onMouseEnter: e => e.currentTarget.style.opacity = 1, onMouseLeave: e => e.currentTarget.style.opacity = .5 }, "\uD83D\uDD0D")));
                    }))),
            React.createElement("div", { style: { flex: 1, display: "flex", flexDirection: "column", minWidth: 0,
                ...
                React.createElement("svg", { style: { position: "absolute", top: 0, left: 0, width: "100%", height: "100%",
                        pointerEvents: "none", zIndex: 0 }, viewBox: "0 0 800 520", preserveAspectRatio: "xMidYMid slice" }, stars.map((s, i) => (React.createElement("circle", { key: i, cx: s.x * 8, cy: s.y * 5.2, r: s.r, fill: "white", opacity: s.o, style: { animation: `blink ${2 + s.r * 2}s ease-in-out infinite`, animationDelay: `${s.o * 5}s` } })))),
                React.createElement("div", { style: { position: "absolute", top: 0, left: 0, width: "100%", height: "100%",
                        pointerEvents: "none", zIndex: 0, overflow: "hidden" } }, cosmicParticles.map((p, i) => (React.createElement("div", { key: i, style: {
                ...
                tab === "MAP" && (React.createElement("div", { style: { flex: 1, position: "relative", zIndex: 1 } },
                    activeFork && (React.createElement("div", { style: { position: "absolute", top: 10, left: "50%", transform: "translateX(-50%)",
                        ...
                        React.createElement("span", null,
                            "\u23EA FORK: \"",
                            activeFork.result.decoded,
                            "\""),
                        React.createElement("span", { style: { color: "#4a1a35" } }, "|"),
                        React.createElement("span", { style: { fontFamily: "monospace" } },
                            "QBER ",
                            (activeFork.result.qber * 100).toFixed(1),
                            "%"),
                        React.createElement("span", { style: { color: "#4a1a35" } }, "|"),
                        React.createElement("span", { style: { fontFamily: "monospace" } },
                            "seed=",
                            activeFork.retroCausalitySeed),
                        React.createElement("button", { onClick: () => setActiveForkId(null), style: { background: "none", border: "none", color: "#f472b6", cursor: "pointer",
                                fontSize: 11, padding: 0, marginLeft: 4 } }, "\u2715"))),
                    path && (React.createElement("div", { style: { position: "absolute", bottom: 10, left: "50%", transform: "translateX(-50%)",
                        ...
                        React.createElement("span", { style: { color: "#1e3a5f" } }, "|"),
                        React.createElement("span", { style: { fontFamily: "monospace" } },
                            pathKm,
                            "km"),
                        React.createElement("span", { style: { color: "#1e3a5f" } }, "|"),
                        React.createElement("span", null,
                            path.length - 1,
                            " HOP"))),
                    React.createElement("svg", { width: "100%", height: "100%", viewBox: "0 0 1000 520", style: { display: "block" } },
                        React.createElement("defs", null,
                            React.createElement("filter", { id: "gl" },
                                React.createElement("feGaussianBlur", { stdDeviation: "2.5", result: "b" }),
                                React.createElement("feMerge", null,
                                    React.createElement("feMergeNode", { in: "b" }),
                                    React.createElement("feMergeNode", { in: "SourceGraphic" }))),
                            React.createElement("filter", { id: "gl2" },
                                React.createElement("feGaussianBlur", { stdDeviation: "7", result: "b" }),
                                React.createElement("feMerge", null,
                                    React.createElement("feMergeNode", { in: "b" }),
                                    React.createElement("feMergeNode", { in: "SourceGraphic" })))),
                        Array.from({ length: 25 }, (_, i) => (React.createElement("line", { key: `v${i}`, x1: i * 40, y1: 0, x2: i * 40, y2: 520, stroke: "#00d4ff", strokeWidth: .3, strokeOpacity: .03 }))),
                        Array.from({ length: 13 }, (_, i) => (React.createElement("line", { key: `h${i}`, x1: 0, y1: i * 40, x2: 1000, y2: i * 40, stroke: "#00d4ff", strokeWidth: .3, strokeOpacity: .03 }))),
                        memoryTraces.map(mt => {
                            ...
                            return (React.createElement("g", { key: mt.id, opacity: opacity }, mt.path.slice(1).map((p, i) => {
                                const lk = p.link;
                                if (!lk)
                                    return null;
                                const co = getLinkXY(lk);
                                if (!co)
                                    return null;
                                return (React.createElement("line", { key: i, ...co, stroke: color, strokeWidth: 2, strokeLinecap: "round", strokeDasharray: "2 6" }));
                            })));
                        }),
                        links.map((lk, i) => {
                            const co = getLinkXY(lk);
                            if (!co)
                                return null;
                            const na = getNode(lk.a), nb = getNode(lk.b);
                            const active = (na === null || na === void 0 ? void 0 : na.on) && (nb === null || nb === void 0 ? void 0 : nb.on);
                            ...
                            const isH = hLink === i;
                            return (React.createElement("g", { key: i, onMouseEnter: () => setHLink(i), onMouseLeave: () => setHLink(null), style: { cursor: "pointer" } },
                                React.createElement("line", { ...co, stroke: "transparent", strokeWidth: 16 }),
                                active && load > .25 && (React.createElement("line", { ...co, stroke: wc.hex, strokeWidth: load * 6, strokeOpacity: load * .1, style: { animation: "glow 2s ease-in-out infinite" } })),
                                onP && React.createElement("line", { ...co, stroke: wc.hex, strokeWidth: 8, strokeOpacity: .1, filter: "url(#gl2)" }),
                                React.createElement("line", { ...co, stroke: !active ? "#0a1628" : onP ? wc.hex : isH ? "#1e3a5f33" : "#0a1628", strokeWidth: onP ? 2 : 1, strokeOpacity: active ? 1 : .2, strokeDasharray: onP ? "10 5" : load > .6 ? "3 8" : "none", style: onP ? { animation: "flow .7s linear infinite" } : {} }),
                                onFork && active && (React.createElement(React.Fragment, null,
                                    React.createElement("line", { ...co, stroke: "#ec4899", strokeWidth: 5, strokeOpacity: .15, filter: "url(#gl2)" }),
                                    React.createElement("line", { ...co, stroke: "#ec4899", strokeWidth: 2.5, strokeOpacity: .9, strokeDasharray: "3 4", strokeLinecap: "round", style: { animation: "flow .5s linear infinite reverse" } }))),
                                (isH || onP) && active && (React.createElement("text", { x: (co.x1 + co.x2) / 2, y: (co.y1 + co.y2) / 2 - 7, textAnchor: "middle", fill: wc.hex, fontSize: 7, fontFamily: "monospace", filter: "url(#gl)" },
                                    lk.km,
                                    "km\u00B7\u03BB",
                                    lk.nm))));
                        }),
                        nodes.map(n => {
                            ...
                            return (React.createElement("g", { key: n.id },
                                corridorInfo && (React.createElement("circle", { cx: n.x, cy: n.y, r: r + 8, fill: "none", stroke: "#22d3ee", strokeWidth: 1.2, strokeOpacity: 0.5 + (1 - corridorInfo.discount) * 0.5, strokeDasharray: "4 3", style: { animation: "blink 0.9s ease-in-out infinite" } })),
                                mirrorOn && (React.createElement("circle", { cx: mirrorX, cy: mirrorY, r: r * 0.6, fill: "none", stroke: "#a855f7", strokeWidth: 0.8, strokeOpacity: 0.25, strokeDasharray: "2 3" })),
                                React.createElement("g", { onClick: () => { toggleNode(n.id); setSelNode(n.id === selNode ? null : n.id); }, onMouseEnter: () => setHNode(n.id), onMouseLeave: () => setHNode(null), style: { cursor: "pointer" } },
                                    (onP || isSrc || isDst) && n.on && (React.createElement("circle", { cx: n.x, cy: n.y, r: r + 12, fill: baseC, opacity: .05, filter: "url(#gl2)" })),
                                    (onP || isH) && n.on && (React.createElement("circle", { cx: n.x, cy: n.y, r: r + 4, fill: "none", stroke: baseC, strokeWidth: .6, strokeOpacity: .2 })),
                                    React.createElement("circle", { cx: n.x, cy: n.y, r: r, fill: n.on ? baseC + "14" : "#020409", stroke: !n.on ? "#0a1628" : onP ? baseC : isH ? baseC + "66" : baseC + "33", strokeWidth: onP ? 1.5 : .8, filter: onP ? "url(#gl)" : "none" }),
                                    React.createElement("circle", { cx: n.x, cy: n.y, r: n.type === "hub" ? 4 : 3, fill: n.on ? baseC : "#0a1628", style: onP ? { animation: "blink 1.2s ease-in-out infinite" } : {} }),
                                    n.on && n.reps > 0 && (React.createElement("text", { x: n.x + r + 3, y: n.y - r + 3, fill: "#1e3a5f", fontSize: 6, fontFamily: "monospace" },
                                        "\u00D7",
                                        n.reps)),
                                    React.createElement("text", { x: n.x, y: n.y + r + 11, textAnchor: "middle", fill: n.on ? (onP ? baseC : "#334155") : "#1e293b", fontSize: 8, fontWeight: onP ? 700 : 400, fontFamily: "monospace" }, n.id),
                                    (isSrc || isDst) && (React.createElement("text", { x: n.x, y: n.y - r - 7, textAnchor: "middle", fill: isSrc ? "#00d4ff" : "#10b981", fontSize: 6, fontWeight: 700, fontFamily: "monospace", letterSpacing: ".1em" }, isSrc ? "ALICE" : "BOB")),
                                    !n.on && (React.createElement("text", { x: n.x, y: n.y + 4, textAnchor: "middle", fill: "#1e3a5f", fontSize: 8, fontWeight: 700 }, "\u2715"))),
                                isH && (React.createElement("g", { onClick: (e) => { e.stopPropagation(); openDrillDown(n.id); }, style: { cursor: "zoom-in" } },
                                    React.createElement("circle", { cx: n.x + r + 9, cy: n.y - r - 9, r: 7.5, fill: "#030610ee", stroke: "#00d4ff", strokeWidth: 1 }),
                                    React.createElement("text", { x: n.x + r + 9, y: n.y - r - 6, textAnchor: "middle", fontSize: 8 }, "\uD83D\uDD0D")))));
                        }),
                        pkts.map(p => (React.createElement("g", { key: p.id },
                            React.createElement("circle", { cx: p.x, cy: p.y, r: 10, fill: p.color, opacity: .1, filter: "url(#gl2)" }),
                            React.createElement("circle", { cx: p.x, cy: p.y, r: 5, fill: p.color, opacity: .9, style: { animation: "pkt .4s ease-in-out infinite" } }),
                            React.createElement("circle", { cx: p.x, cy: p.y, r: 2, fill: "#fff", opacity: .9 }),
                            React.createElement("text", { x: p.x, y: p.y - 13, textAnchor: "middle", fill: p.color, fontSize: 7, fontFamily: "monospace", fontWeight: 700 },
                                "\u03BB",
                                p.label)))),
                        ghostPkts.map(g => {
                            var _a;
                            const strength = (_a = g.signalStrength) !== null && _a !== void 0 ? _a : 1.0;
                            ...
                            return (React.createElement("g", { key: g.id, opacity: 0.45 * strength },
                                React.createElement("circle", { cx: g.x, cy: g.y, r: 7, fill: dampenColor, opacity: .15, filter: "url(#gl2)" }),
                                React.createElement("circle", { cx: g.x, cy: g.y, r: 3.5 * Math.max(0.4, strength), fill: "none", stroke: dampenColor, strokeWidth: dampening ? 0.7 : 1 })));
                        }),
                        antiPackets.map(a => (React.createElement("g", { key: a.id, opacity: 0.85 },
                            React.createElement("circle", { cx: a.x, cy: a.y, r: 10, fill: "none", stroke: "#f43f5e", strokeWidth: 1.5, strokeOpacity: 0.7, style: { animation: "pkt .26s ease-out" } }),
                            React.createElement("circle", { cx: a.x, cy: a.y, r: 3, fill: "#f43f5e", opacity: .6 }),
                            React.createElement("text", { x: a.x, y: a.y - 14, textAnchor: "middle", fill: "#f43f5e", fontSize: 7, fontFamily: "monospace", fontWeight: 700 }, "\u2298")))),
                        mirrorPkts.map(g => (React.createElement("g", { key: g.id, opacity: 0.7 },
                            React.createElement("circle", { cx: g.x, cy: g.y, r: 9, fill: "#a855f7", opacity: .18, filter: "url(#gl2)" }),
                            React.createElement("circle", { cx: g.x, cy: g.y, r: 4.5, fill: "#a855f7", opacity: .85, style: { animation: "pkt .4s ease-in-out infinite" } }),
                            React.createElement("circle", { cx: g.x, cy: g.y, r: 1.5, fill: "#fff", opacity: .9 }),
                            React.createElement("text", { x: g.x, y: g.y - 13, textAnchor: "middle", fill: "#a855f7", fontSize: 7, fontFamily: "monospace", fontWeight: 700 }, "\uD83E\uDE9E")))),
                        selNode && (() => {
                            const n = getNode(selNode);
                            if (!n)
                                return null;
                            return (React.createElement("g", null,
                                React.createElement("rect", { x: n.x + 18, y: n.y - 30, width: 120, height: 80, rx: 4, fill: "#030610", stroke: "#00d4ff22", strokeWidth: 1 }),
                                React.createElement("text", { x: n.x + 24, y: n.y - 16, fill: "#00d4ff", fontSize: 9, fontWeight: 700 }, n.id),
                                [`${n.label}`, `${n.type.toUpperCase()} R×${n.reps}`,
                                    `${n.lat}°N ${n.lon}°E`, n.on ? "ONLINE" : "OFFLINE"].map((t, i) => (React.createElement("text", { key: i, x: n.x + 24, y: n.y + i * 11, fill: i === 3 ? (n.on ? "#10b981" : "#f43f5e") : "#475569", fontSize: 7 }, t)))));
                        })()))),
                tab === "SAT" && (React.createElement("div", { style: { flex: 1, overflowY: "auto", padding: 14, zIndex: 1, position: "relative" } },
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 } }, sats.map(s => {
                        const q = satQ(s.elev);
                        const ac = s.id === activeSat;
                        return (React.createElement("div", { key: s.id, onClick: () => setActiveSat(s.id), style: { background: ac ? "#0a1628" : "#030610",
                            ...
                            React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 7, marginBottom: 7 } },
                                ...
                                React.createElement("span", { style: { fontSize: 9, color: ac ? "#00d4ff" : "#334155", fontWeight: ac ? 700 : 400, letterSpacing: ".06em" } }, s.name),
                                React.createElement("span", { style: { marginLeft: "auto", fontSize: 7, color: "#1e3a5f",
                                        background: "#0a1628", padding: "1px 5px", borderRadius: 2 } }, s.status)),
                            React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, marginBottom: 7 } }, [["İRTİFA", `${s.alt}km`], ["YÜKSELTİ", `${s.elev.toFixed(1)}°`],
                                ["BOYLAM", `${s.lon.toFixed(1)}°E`], ["KALİTE", `${(q * 100).toFixed(0)}%`]
                            ].map(([l, v]) => (React.createElement("div", { key: l, style: { background: "#020409", borderRadius: 3, padding: "4px 6px" } },
                                React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", marginBottom: 1 } }, l),
                                React.createElement("div", { style: { fontSize: 10, fontWeight: 700,
                                        color: l === "KALİTE" ? (q > .7 ? "#10b981" : q > .4 ? "#f59e0b" : "#f43f5e") : "#64748b"
                                    } }, v))))),
                            React.createElement(BerGauge, { value: q * 100 })));
                    })),
                    React.createElement("div", { style: { background: "#030610", border: "1px solid #10b98122",
                        ...
                        React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 7, marginBottom: 7 } },
                            ...
                            React.createElement("span", { style: { fontSize: 9, color: "#10b981", fontWeight: 700, letterSpacing: ".08em" } }, "ISP BA\u011EIMSIZ M\u0130MAR\u0130")),
                        React.createElement("div", { style: { fontSize: 9, color: "#334155", lineHeight: 1.9 } },
                            "PhotonNet veriyi do\u011Frudan LEO uydu \u00FCzerinden iletir.",
                            React.createElement("br", null),
                            "Yerel ISP altyap\u0131s\u0131na BA\u011ELI DE\u011E\u0130LD\u0130R.",
                            React.createElement("br", null),
                            "\u0130nternet kesintisi \u00B7 DNS blokaj\u0131 \u00B7 trafik y\u00F6netimi",
                            React.createElement("br", null),
                            "bu a\u011F\u0131 ETK\u0130LEMEZ.")))),
                tab === "TRF" && (React.createElement("div", { style: { flex: 1, overflowY: "auto", padding: 14, zIndex: 1, position: "relative" } },
                    React.createElement("div", { style: { background: "#0a0d1f",
                        ...
                        React.createElement("div", { style: { fontSize: 10, color: "#00d4ff", fontWeight: 700, letterSpacing: ".08em", marginBottom: 8,
                                display: "flex", alignItems: "center", gap: 6 } }, "\uD83C\uDF10 D\u0130NAM\u0130K Y\u00DCK DENGELEME \u2014 A\u011E KR\u0130Z S\u0130M\u00DCLASYONU"),
                        React.createElement("div", { style: { fontSize: 8, color: "#64748b", marginBottom: 10, lineHeight: 1.5 } }, "Stratejik bir hatta (\u00F6rn. okyanus-\u00F6tesi omurga) yapay bir kriz enjekte edin \u2014 y\u00F6nlendirme motoru (Dijkstra + EdgeWeightPolicy) bunu bir sonraki G\u00D6NDER'de otomatik alg\u0131lay\u0131p trafi\u011Fi en optimize alternatif rotaya kayd\u0131r\u0131r."),
                        !activeCrisis ? (React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap" } },
                            React.createElement("div", { style: { flex: "1 1 220px", minWidth: 180 } },
                                React.createElement("div", { style: { fontSize: 7, color: "#334155", marginBottom: 3 } }, "HAT SE\u00C7 (omurga, >1500km)"),
                                React.createElement("select", { value: crisisLinkSel, disabled: crisisBusy, onChange: e => setCrisisLinkSel(e.target.value), style: { width: "100%", padding: "5px 6px", fontSize: 9 } },
                                    React.createElement("option", { value: "" }, "\u2014 hat se\u00E7in \u2014"),
                                    [...links].filter(l => l.km > 1500).sort((a, b) => b.km - a.km).map(l => (React.createElement("option", { key: `${l.a}-${l.b}`, value: `${l.a}-${l.b}` },
                                        l.a,
                                        " \u2192 ",
                                        l.b,
                                        " (",
                            ...
                            React.createElement("div", { style: { flex: "0 0 auto" } },
                                React.createElement("div", { style: { fontSize: 7, color: "#334155", marginBottom: 3 } }, "KR\u0130Z T\u00DCR\u00DC"),
                                React.createElement("div", { style: { display: "flex", gap: 4 } }, [["outage", "🔴 KOPMA"], ["congestion", "🟠 YOĞUNLUK"]].map(([k, l]) => (React.createElement("button", { key: k, disabled: crisisBusy, onClick: () => setCrisisMode(k), style: { padding: "5px 9px", fontSize: 8, borderRadius: 4, cursor: "pointer",
                                        ...
                                        color: crisisMode === k ? (k === "outage" ? "#f43f5e" : "#f59e0b") : "#334155", fontWeight: 700 } }, l))))),
                            React.createElement("button", { onClick: injectCrisis, disabled: crisisBusy || !crisisLinkSel, style: { padding: "6px 14px", fontSize: 9, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                                    background: "linear-gradient(135deg,#f43f5e,#f59e0b)", border: "none", color: "#fff",
                                    opacity: crisisBusy || !crisisLinkSel ? 0.4 : 1 } }, "\u26A1 KR\u0130Z\u0130 ENJEKTE ET"))) : (React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" } },
                            React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6,
                                ...
                                React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: activeCrisis.mode === "outage" ? "#f43f5e" : "#f59e0b" } },
                                    activeCrisis.a,
                                    "-",
                                    activeCrisis.b,
                                    " ",
                                    activeCrisis.mode === "outage" ? "KOPUK" : "AŞIRI YOĞUN")),
                            React.createElement("span", { style: { fontSize: 8, color: "#64748b" } }, "Bir sonraki G\u00D6NDER, alternatif rotay\u0131 otomatik dener."),
                            React.createElement("button", { onClick: cancelCrisis, style: { padding: "5px 11px", fontSize: 8, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                                    background: "none", border: "1px solid #33415577", color: "#94a3b8" } }, "\u2715 \u0130PTAL ET / HEMEN ONAR")))),
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 7, marginBottom: 12 } }, [
                    ...
                    ].map(m => (React.createElement("div", { key: m.l, style: { background: "#030610", border: "1px solid #0a162888",
                            borderRadius: 4, padding: "9px 10px" } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 3 } }, m.l),
                        React.createElement("div", { style: { fontSize: 18, fontWeight: 700, color: m.c, fontFamily: "monospace" } }, m.v))))),
                    React.createElement("div", { style: { background: "#030610", border: "1px solid #0a162888",
                            borderRadius: 4, padding: "9px 10px", marginBottom: 10 } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 5 } }, "BER TAR\u0130HSEL"),
                        React.createElement(Spark, { data: berHistory, color: "#00d4ff", h: 38 })),
                    traffic.map((t, i) => (React.createElement("div", { key: i, style: { background: "#030610",
                            border: `1px solid ${t.success ? "#10b98122" : "#f43f5e22"}`,
                            borderRadius: 4, padding: "8px 10px", marginBottom: 5 } },
                        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 4 } },
                            React.createElement("span", { style: { fontSize: 9, color: "#00d4ff", fontWeight: 700 } },
                                t.src,
                                " \u2192 ",
                                t.dst),
                            React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center" } },
                                React.createElement("span", { style: { fontSize: 7, color: "#1e3a5f", fontFamily: "monospace" } }, t.time),
                                React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: t.success ? "#10b981" : "#f43f5e" } }, t.success ? "✓ OK" : "✗ ERR"))),
                        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5, marginBottom: 4 } }, [["TX", `"${t.original}"`, "#00d4ff"], ["RX", `"${t.decoded}"`, t.success ? "#10b981" : "#f59e0b"]].map(([l, v, c]) => (React.createElement("div", { key: l, style: { background: "#020409", borderRadius: 3, padding: "3px 6px" } },
                            React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f" } }, l),
                            React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: c, fontFamily: "monospace" } }, v))))),
                        React.createElement("div", { style: { display: "flex", gap: 10, fontSize: 8, color: "#1e3a5f", fontFamily: "monospace" } },
                            React.createElement("span", null,
                                t.totalKm,
                                "km"),
                            React.createElement("span", null,
                                t.hops,
                                "hop"),
                            React.createElement("span", { style: { color: +t.er > 10 ? "#f43f5e" : "#10b981" } },
                                "BER:",
                                t.er,
                                "%"),
                            t.corrected > 0 && React.createElement("span", { style: { color: "#10b981" } },
                                "ECC+",
                                t.corrected),
                            React.createElement("span", { style: { marginLeft: "auto" } },
                                "\uD83D\uDEF0 ",
                                t.sat))))))),
                tab === "TML" && (React.createElement("div", { style: { flex: 1, overflowY: "auto", padding: 14, zIndex: 1, position: "relative" } },
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 7, marginBottom: 12 } }, [
                        ...
                        { l: "ENTANGLE SEÇİMİ", v: `${entangleA ? 1 : 0}+${entangleB ? 1 : 0}/2`, c: "#f59e0b" },
                            ...
                            borderRadius: 4, padding: "9px 10px" } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 3 } }, m.l),
                        React.createElement("div", { style: { fontSize: 17, fontWeight: 700, color: m.c, fontFamily: "monospace" } }, m.v))))),
                    interference > 0 && (React.createElement("div", { style: { background: "#3f151533", border: "1px solid #f43f5e44",
                            borderRadius: 6, padding: "8px 12px", marginBottom: 12, fontSize: 9, color: "#f43f5e" } },
                        "\u26A1 ",
                        interference,
                        " \u00E7ak\u0131\u015Fma tespit edildi \u2014 birden fazla replay ayn\u0131 rotada ayn\u0131 anda hayalet paket ta\u015F\u0131yor, bu fiziksel olarak faz kaymas\u0131 (PHASE) olas\u0131l\u0131\u011F\u0131n\u0131 art\u0131r\u0131r.")),
                    React.createElement("div", { style: { background: "#0a0d1f", border: "1px solid #22c55e22", borderRadius: 8,
                            padding: "13px 15px", marginBottom: 14 } },
                        React.createElement("div", { style: { fontSize: 10, color: "#22c55e", fontWeight: 700, letterSpacing: ".08em", marginBottom: 10 } }, "\uD83C\uDFAF D\u00DC\u011E\u00DCM REDD\u0130 CEZASI \u2014 CoefficientEvolutionEngine E\u011Fitimi"),
                        React.createElement("div", { style: { fontSize: 8, color: "#64748b", marginBottom: 6, lineHeight: 1.5 } }, "Fitness fonksiyonuna eklenen ceza terimi: her genom, izole bir probe NodeTransitGate \u00FCzerinde GER\u00C7EKTEN test edilir \u2014 \u00F6l\u00E7\u00FClen ret oran\u0131 \u00D7 bu a\u011F\u0131rl\u0131kla fitness'ten d\u00FC\u015F\u00FCl\u00FCr. Motorun kendi kendini e\u011Fitmesini a\u015Fa\u011F\u0131daki nesil ge\u00E7mi\u015Finden izleyebilirsiniz."),
                        React.createElement("div", { style: { fontSize: 7, color: "#475569", marginBottom: 10, lineHeight: 1.6, fontStyle: "italic",
                                background: "#050a05", borderRadius: 5, padding: "6px 8px", border: "1px solid #22c55e18" } }, "\u26A0 Ger\u00E7ek\u00E7ilik s\u0131n\u0131r\u0131: pop\u00FClasyon (24) ve probe foton say\u0131s\u0131 (8) istatistiksel anlaml\u0131l\u0131k i\u00E7in d\u00FC\u015F\u00FCkt\u00FCr; ceza a\u011F\u0131rl\u0131\u011F\u0131 (varsay\u0131lan 0.4) deneysel kalibrasyondan gelmez. En kritik s\u0131n\u0131r: \u00FCretilen katsay\u0131lar YALNIZCA LEGA'n\u0131n kendi form\u00FCl\u00FCyle puanlan\u0131r \u2014 ger\u00E7ekten g\u00F6nderilmi\u015F fotonlara/\u00F6l\u00E7\u00FClm\u00FC\u015F BER'e kar\u015F\u0131 ASLA do\u011Frulanmaz. Bu sistem kendi s\u0131nav\u0131n\u0131 kendi haz\u0131rlay\u0131p kendi cevapl\u0131yor; ilkeler a\u00E7\u0131s\u0131ndan bir \"genetik algoritma\"dan \u00E7ok, yerel bir tepe-t\u0131rmanma (hill-climbing) sezgiseline daha yak\u0131nd\u0131r."),
                        React.createElement("div", { style: { marginBottom: 10 } },
                            React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 4 } },
                                React.createElement("span", { style: { fontSize: 8, color: "#475569", letterSpacing: ".06em" } }, "CEZA A\u011EIRLI\u011EI"),
                                React.createElement("span", { style: { fontSize: 9, color: "#22c55e", fontFamily: "monospace", fontWeight: 700 } }, nodeRejectionPenaltyWeight.toFixed(2))),
                            React.createElement("input", { type: "range", min: 0, max: 1, step: 0.05, value: nodeRejectionPenaltyWeight, onChange: e => setNodeRejectionPenaltyWeight(+e.target.value), style: { width: "100%", accentColor: "#22c55e", cursor: "pointer", height: 4 } }),
                            React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginTop: 2 } },
                                React.createElement("span", { style: { fontSize: 7, color: "#1e3a5f" } }, "0 = etkisiz"),
                                React.createElement("span", { style: { fontSize: 7, color: "#1e3a5f" } }, "1 = bask\u0131n"))),
                        lastNodeRejectionRate != null && (React.createElement("div", { style: { background: "#020409", borderRadius: 6, padding: "8px 10px", marginBottom: 10,
                                display: "flex", alignItems: "center", gap: 10 } },
                            React.createElement("div", null,
                                React.createElement("div", { style: { fontSize: 7, color: "#334155" } }, "SON \u00D6L\u00C7\u00DCLEN RET ORANI"),
                                React.createElement("div", { style: { fontSize: 14, fontWeight: 700, fontFamily: "monospace",
                                        color: lastNodeRejectionRate < 0.3 ? "#22c55e" : "#f59e0b" } },
                                    "%",
                                    (lastNodeRejectionRate * 100).toFixed(1))),
                            React.createElement("div", { style: { flex: 1, height: 6, background: "#0a1628", borderRadius: 3, overflow: "hidden" } },
                                React.createElement("div", { style: { height: "100%", width: `${lastNodeRejectionRate * 100}%`, borderRadius: 3,
                                        background: lastNodeRejectionRate < 0.3 ? "#22c55e" : "#f59e0b", transition: "width .4s" } })))),
                        coeffEvolution.history.length > 0 && (React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 8, color: "#1e3a5f", letterSpacing: ".08em", marginBottom: 6 } },
                                "NES\u0130L GE\u00C7M\u0130\u015E\u0130 (son ",
                                Math.min(8, coeffEvolution.history.length),
                                ")"),
                            React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 3 } }, coeffEvolution.history.slice(-8).reverse().map(h => (React.createElement("div", { key: h.generation, style: { display: "flex", gap: 8, alignItems: "center",
                                    fontSize: 8, fontFamily: "monospace", color: "#64748b" } },
                                React.createElement("span", { style: { minWidth: 34, color: "#334155" } },
                                    "N",
                                    h.generation),
                                React.createElement("span", null,
                                    "\u03B1=",
                                ...
                                React.createElement("span", null,
                                    "\u03B2=",
                                    h.beta.toFixed(3)),
                                React.createElement("span", { style: { marginLeft: "auto", color: h.fitness > 0 ? "#22c55e" : "#f59e0b" } },
                                    "fitness=",
                                    h.fitness.toFixed(3))))))))),
                    (() => {
                        const mw = analyzeManyWorlds(timeline);
                        if (!mw)
                            return (React.createElement("div", { style: { background: "#030610", border: "1px solid #0a162888", borderRadius: 8,
                                    padding: "12px 14px", marginBottom: 14, fontSize: 10, color: "#334155", textAlign: "center" } }, "Many-Worlds analizi i\u00E7in en az 1 iletim gerekli."));
                        return (React.createElement("div", { style: { background: "#0a0d1f", border: "1px solid #00d4ff22", borderRadius: 8,
                                padding: "13px 15px", marginBottom: 14 } },
                            React.createElement("div", { style: { fontSize: 10, color: "#00d4ff", fontWeight: 700, letterSpacing: ".08em", marginBottom: 10 } },
                                "\uD83C\uDF0C MANY-WORLDS \u0130STAT\u0130ST\u0130KLER\u0130 (",
                                mw.totalRecords,
                                " ger\u00E7eklik)"),
                            React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 7, marginBottom: 10 } }, [
                            ...
                            ].map(s => (React.createElement("div", { key: s.l, style: { background: "#020409", borderRadius: 5, padding: "7px 9px" } },
                                React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", marginBottom: 2 } }, s.l),
                                React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: s.c, fontFamily: "monospace" } }, s.v))))),
                            React.createElement("div", { style: { fontSize: 8, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 6 } }, "GLOBAL OLAY DA\u011EILIMI H\u0130STOGRAMI"),
                            React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 4, marginBottom: 10 } }, Object.entries(mw.globalEc).sort((a, b) => b[1] - a[1]).map(([type, count]) => {
                                const m = ERR[type] || { c: "#64748b", l: type, i: "·" };
                                const total = Object.values(mw.globalEc).reduce((a, v) => a + v, 0);
                                return (React.createElement("div", { key: type, style: { display: "flex", alignItems: "center", gap: 8, fontSize: 9 } },
                                    React.createElement("span", { style: { color: m.c, minWidth: 100 } },
                                        m.i,
                                        " ",
                                        m.l),
                                    React.createElement("div", { style: { flex: 1, height: 6, background: "#0a1628", borderRadius: 3, overflow: "hidden" } },
                                        React.createElement("div", { style: { height: "100%", width: `${(count / total) * 100}%`,
                                                background: m.c, borderRadius: 3 } })),
                                    React.createElement("span", { style: { color: m.c, fontFamily: "monospace", minWidth: 28, textAlign: "right" } }, count)));
                            })),
                            mw.nmRanking.length > 0 && (React.createElement("div", { style: { background: "#020409", borderRadius: 6, padding: "9px 11px" } },
                                React.createElement("div", { style: { fontSize: 9, color: "#94a3b8", marginBottom: 6 } },
                                    "\uD83D\uDCA1 Bu a\u011Fda en kararl\u0131 dalga boyu:",
                                    " ",
                                    React.createElement("span", { style: { color: "#10b981", fontWeight: 700, fontFamily: "monospace" } },
                                        mw.mostStableWavelength,
                                        "nm"),
                                    " ",
                                    "(ort. BER ",
                                    mw.nmRanking[0].avgBer.toFixed(2),
                                    "%)"),
                                mw.nmRanking.length > 1 && (React.createElement("div", { style: { fontSize: 9, color: "#94a3b8" } },
                                    "En az kararl\u0131:",
                                    " ",
                                    React.createElement("span", { style: { color: "#f43f5e", fontWeight: 700, fontFamily: "monospace" } },
                                        mw.leastStableWavelength,
                                        "nm"),
                                    " ",
                                    "(ort. BER ",
                                    mw.nmRanking[mw.nmRanking.length - 1].avgBer.toFixed(2),
                                    "%)")),
                                React.createElement("div", { style: { marginTop: 7, display: "flex", flexDirection: "column", gap: 3 } }, mw.nmRanking.map(r => (React.createElement("div", { key: r.nm, style: { display: "flex", alignItems: "center", gap: 7, fontSize: 8 } },
                                    React.createElement("span", { style: { color: (WL[r.nm] || {}).hex || "#64748b", minWidth: 50, fontFamily: "monospace" } },
                                        r.nm,
                                        "nm"),
                                    React.createElement("div", { style: { flex: 1, height: 4, background: "#0a1628", borderRadius: 2 } },
                                    ...
                                    React.createElement("span", { style: { color: "#475569", fontFamily: "monospace", minWidth: 50, textAlign: "right" } },
                                        r.avgBer.toFixed(1),
                                        "% BER")))))))));
                    })(),
                    entangleA && entangleB && entangleA !== entangleB && (() => {
                        ...
                        if (!cmp)
                            return null;
                        return (React.createElement("div", { style: { background: "#0a0d1f", border: "1px solid #7c3aed44",
                                borderRadius: 8, padding: "12px 14px", marginBottom: 14 } },
                            React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 } },
                                React.createElement("span", { style: { fontSize: 10, color: "#a78bfa", fontWeight: 700, letterSpacing: ".08em" } }, "\u29C9 ENTANGLED KAR\u015EILA\u015ETIRMA"),
                                React.createElement("button", { className: "nbtn", onClick: () => { setEntangleA(null); setEntangleB(null); }, style: { fontSize: 9, color: "#334155" } }, "TEM\u0130ZLE \u2715")),
                            React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 } }, [cmp.a, cmp.b].map((e, idx) => (React.createElement("div", { key: e.id, style: { background: "#020409", borderRadius: 6, padding: "8px 10px" } },
                                React.createElement("div", { style: { fontSize: 8, color: idx === 0 ? "#00d4ff" : "#22d3ee", marginBottom: 4, fontFamily: "monospace" } },
                                    idx === 0 ? "A" : "B",
                                    " \u00B7 ",
                                    e.time),
                                React.createElement("div", { style: { fontSize: 11, color: "#94a3b8", fontFamily: "monospace", marginBottom: 3 } },
                                    e.src,
                                    "\u2192",
                                    e.dst,
                                    " \u00B7 \"",
                                    e.msg,
                                    "\""),
                                React.createElement("div", { style: { fontSize: 9, color: e.result.success ? "#10b981" : "#f59e0b" } },
                                    "BER ",
                                    e.result.er,
                                    "% \u00B7 seed=",
                                    e.seed))))),
                            React.createElement("div", { style: { fontSize: 10, color: "#94a3b8", marginBottom: 8 } },
                                "BER fark\u0131 (A\u2212B): ",
                                React.createElement("span", { style: {
                                    ...
                                    cmp.berDiff,
                                    "%")),
                            React.createElement("div", { style: { fontSize: 8, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 6 } }, "OLAY DA\u011EILIMI FARKI"),
                            cmp.eventDiff.map(d => {
                                const m = ERR[d.type] || { c: "#64748b", l: d.type, i: "·" };
                                return (React.createElement("div", { key: d.type, style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 4, fontSize: 9 } },
                                    React.createElement("span", { style: { color: m.c, minWidth: 90 } },
                                        m.i,
                                        " ",
                                        m.l),
                                    React.createElement("span", { style: { color: "#00d4ff", fontFamily: "monospace" } },
                                        "A:",
                                        d.a),
                                    React.createElement("span", { style: { color: "#22d3ee", fontFamily: "monospace" } },
                                        "B:",
                                        d.b),
                                    React.createElement("div", { style: { flex: 1, height: 2, background: "#0a1628", borderRadius: 1, position: "relative" } },
                                        React.createElement("div", { style: { position: "absolute", left: 0, height: "100%",
                                                width: `${Math.min(100, (d.a / (d.a + d.b || 1)) * 100)}%`,
                                                background: "#00d4ff", borderRadius: 1 } }))));
                            }),
                            React.createElement("button", { onClick: () => { replayTimeline(entangleA); replayTimeline(entangleB); }, style: { width: "100%", marginTop: 10, padding: "8px", borderRadius: 6,
                                    border: "1px solid #7c3aed55", background: "#7c3aed11", color: "#a78bfa",
                                    fontSize: 10, fontWeight: 700, letterSpacing: ".06em", cursor: "pointer" } }, "\u25B6\u25B6 \u0130K\u0130S\u0130N\u0130 AYNI ANDA REPLAY ET (interference testi)")));
                    })(),
                    timeline.length === 0 ? (React.createElement("div", { style: { color: "#1e3a5f", fontSize: 12, textAlign: "center", marginTop: 60 } },
                        "Hen\u00FCz entangle edilmi\u015F bir iletim yok.",
                        React.createElement("br", null),
                        React.createElement("span", { style: { fontSize: 10 } }, "Sol panelden \"\u29C9 ENTANGLE\" butonuna basarak iletimleri kaydedin."))) : timeline.map(entry => {
                        var _a;
                        const isReplaying = replaying.includes(entry.id);
                        const isA = entangleA === entry.id, isB = entangleB === entry.id;
                        return (React.createElement("div", { key: entry.id, style: {
                                background: "#030610",
                            ...
                            React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 } },
                                React.createElement("span", { style: { fontSize: 10, color: "#00d4ff", fontWeight: 700, fontFamily: "monospace" } },
                                    entry.src,
                                    " \u2192 ",
                                    entry.dst),
                                React.createElement("div", { style: { display: "flex", gap: 6, alignItems: "center" } },
                                    React.createElement("span", { style: { fontSize: 7, color: "#1e3a5f", fontFamily: "monospace" } }, entry.time),
                                    React.createElement("span", { style: { fontSize: 9, fontWeight: 700,
                                            color: entry.result.success ? "#10b981" : "#f43f5e" } }, entry.result.success ? "✓" : "✗"))),
                            React.createElement("div", { style: { fontSize: 10, color: "#64748b", fontFamily: "monospace", marginBottom: 4 } },
                                "\"",
                                entry.msg,
                                "\" \u00B7 seed=",
                                entry.seed,
                                ...
                                entry.result.er,
                                "%"),
                            entry.result.qber != null && (React.createElement("div", { style: { fontSize: 9, color: entry.result.eavesdropDetected ? "#f43f5e" : "#7c3aed",
                                    fontFamily: "monospace", marginBottom: 6 } },
                                "QBER ",
                                (entry.result.qber * 100).toFixed(1),
                                "% \u00B7 sifted key ",
                                entry.result.siftedKeyLen,
                                "bit",
                            ...
                            entry.result.legaSnapshot && (React.createElement("div", { style: { fontSize: 8, color: "#f59e0b", fontFamily: "monospace", marginBottom: 6, opacity: .85 }, title: "Bu katsay\u0131lar ger\u00E7ek fiber optik \u00F6l\u00E7\u00FCm verisinden t\u00FCretilmemi\u015Ftir \u2014 sezgisel/didaktik bir davran\u0131\u015F modelidir" },
                                "\uD83C\uDF21\uFE0F LEGA: scatter-\u00F6l\u00FCm ",
                                (entry.result.legaSnapshot.scatterDeathProb * 100).toFixed(0),
                                "% \u00B7 absorb-bias \u00D7",
                                entry.result.legaSnapshot.absorbBias.toFixed(2),
                                " \u00B7 decohere-bias \u00D7",
                                entry.result.legaSnapshot.decohereBias.toFixed(2),
                                React.createElement("span", { style: { opacity: .55, marginLeft: 5 } }, "(kalibre edilmemi\u015F model)"))),
                            entry.result.gateReport && entry.result.gateReport.checked > 0 && (React.createElement("div", { style: { marginBottom: 6 } },
                                React.createElement("div", { style: { fontSize: 8, color: "#fb923c", fontFamily: "monospace", marginBottom: 3 } },
                                    "\u26D4 NodeTransitGate: ",
                                    entry.result.gateReport.checked,
                                    " foton triyaj edildi"),
                                React.createElement("div", { style: { display: "flex", height: 4, borderRadius: 2, overflow: "hidden", background: "#0a1628" } },
                                    React.createElement("div", { style: { width: `${(entry.result.gateReport.phase1Rejected / entry.result.gateReport.checked) * 100}%`, background: "#f43f5e" }, title: "Faz 1: Rezonans reddi" }),
                                    React.createElement("div", { style: { width: `${(entry.result.gateReport.phase2Rejected / entry.result.gateReport.checked) * 100}%`, background: "#f59e0b" }, title: "Faz 2: Genlik reddi" }),
                                    React.createElement("div", { style: { width: `${(entry.result.gateReport.phase3Passed / entry.result.gateReport.checked) * 100}%`, background: "#10b981" }, title: "Faz 3: Transit ge\u00E7i\u015F" })),
                                React.createElement("div", { style: { display: "flex", gap: 8, fontSize: 7, color: "#475569", marginTop: 2, fontFamily: "monospace" } },
                                    React.createElement("span", { style: { color: "#f43f5e" } },
                                        "F1:",
                                        entry.result.gateReport.phase1Rejected),
                                    React.createElement("span", { style: { color: "#f59e0b" } },
                                        "F2:",
                                        entry.result.gateReport.phase2Rejected),
                                    React.createElement("span", { style: { color: "#10b981" } },
                                        "F3:",
                                        entry.result.gateReport.phase3Passed),
                                    entry.result.gateReport.softMatchedCount > 0 && (React.createElement("span", { style: { color: "#22c55e" } },
                                        "\uD83D\uDFE2 Yumu\u015Fak:",
                                        entry.result.gateReport.softMatchedCount))))),
                            entry.result.pimSummary && (React.createElement("div", { style: { fontSize: 8, color: "#22d3ee", fontFamily: "monospace", marginBottom: 6,
                                ...
                                React.createElement("div", { style: { marginBottom: 2 } },
                                    ...
                                    " ",
                                    entry.result.pimSummary.decisions,
                                    " karar h\u00FCcre-i\u00E7i al\u0131nd\u0131"),
                                React.createElement("div", { style: { color: "#64748b" } },
                                    "von-Neumann ~",
                                    entry.result.pimSummary.totalVonNeumannNs.toFixed(0),
                                    "ns vs PIM ~",
                                    entry.result.pimSummary.totalPimNs.toFixed(0),
                                    "ns \u2192",
                                    " ",
                                    React.createElement("span", { style: { color: "#22d3ee" } },
                                        "\u00D7",
                                        entry.result.pimSummary.avgSpeedup.toFixed(0),
                                        " h\u0131zlanma"),
                                    " ",
                                    "(",
                                    entry.result.pimSummary.cellCount,
                                    " aktif h\u00FCcre)"))),
                            entry.result.opllSummary && (React.createElement("div", { style: { fontSize: 8, color: "#00d4ff", fontFamily: "monospace", marginBottom: 6,
                                    background: "#0a1628", borderRadius: 5, padding: "5px 7px" } },
                                React.createElement("div", null,
                                    "\u2307 OPLL: ",
                                    entry.result.opllSummary.lockedChannels,
                                    "/",
                                    entry.result.opllSummary.totalChannels,
                                    " kanal kilitli"),
                                entry.result.opllSummary.channels.length > 0 && (React.createElement("div", { style: { color: "#64748b", marginTop: 2, display: "flex", flexDirection: "column", gap: 1 } }, entry.result.opllSummary.channels.slice(0, 4).map(ch => (React.createElement("div", { key: ch.key },
                                    ch.key,
                                    ": ",
                                    React.createElement("span", { style: { color: ch.locked ? "#10b981" : "#f59e0b" } }, ch.locked ? "KİLİTLİ" : `kilitleniyor (${ch.lockStreak}/5)`)))))))),
                            entry.result.eccBypassed && (React.createElement("div", { style: { fontSize: 8, color: "#f43f5e", fontFamily: "monospace", marginBottom: 6,
                                    background: "#3f1515", borderRadius: 5, padding: "5px 7px", border: "1px solid #f43f5e44" } }, "\u26A0 FAZ KARARLILI\u011EI KR\u0130Z\u0130: ECC katman\u0131 bu iletimde bypass edildi (katsay\u0131 enjeksiyon h\u0131z\u0131 fiziksel kilitlenmeyi ge\u00E7ti)")),
                            entry.result.routeHealth && !entry.result.routeHealth.healthy && (React.createElement("div", { style: { fontSize: 8, color: "#f59e0b", fontFamily: "monospace", marginBottom: 6,
                                    background: "#2a1a05", borderRadius: 5, padding: "5px 7px", border: "1px solid #f59e0b44" } },
                                React.createElement("div", null,
                                    "\u26A0 ROTA SA\u011ELI\u011EI D\u00DC\u015E\u00DCK: ",
                                    entry.result.routeHealth.hopCount,
                                    " hop, u\u00E7tan-uca ge\u00E7irgenlik %",
                                    (entry.result.routeHealth.cumulativeTransmittance * 100).toFixed(3)),
                                parseFloat(entry.result.rawErrorAccumulation) > 100 && (React.createElement("div", { style: { color: "#78716c", marginTop: 2 } },
                                    "Ham hata birikimi: %",
                                    entry.result.rawErrorAccumulation,
                                    " (g\u00F6sterilen BER %100'de tavanland\u0131)")))),
                            entry.result.corridorSnapshot && (React.createElement("div", { style: { fontSize: 8, color: "#a3e635", fontFamily: "monospace", marginBottom: 6 } },
                                "\u26A1 Feedforward Tahmin: ",
                                entry.result.corridorSnapshot.nodesOpened,
                                " d\u00FC\u011F\u00FCm yola \u00E7\u0131kmadan \u00F6nce a\u00E7\u0131ld\u0131 (vakum koridoru, ort. e\u015Fik \u00D7",
                                entry.result.corridorSnapshot.avgDiscount.toFixed(2),
                                ")")),
                            entry.result.quantumStateLog && entry.result.quantumStateLog.length > 0 && (React.createElement("details", { style: { marginBottom: 7 } },
                                React.createElement("summary", { style: { fontSize: 8, color: "#00d4ff", cursor: "pointer", letterSpacing: ".06em" }, title: "Hamming sendromlar\u0131n\u0131n kuantum durum notasyonuna e\u015Flenmesi bir ANALOJ\u0130D\u0130R \u2014 ger\u00E7ek bir kuantum \u00F6l\u00E7\u00FCm\u00FCn\u00FC temsil etmez" },
                                    "\u269B ",
                                    entry.result.quantumStateLog.length,
                                    " kuantum durum imzas\u0131 \u2014 telemetri g\u00FCnl\u00FC\u011F\u00FC",
                                    React.createElement("span", { style: { opacity: .5, marginLeft: 5 } }, "(analoji, ger\u00E7ek \u00F6l\u00E7\u00FCm de\u011Fil)")),
                                React.createElement("div", { style: { marginTop: 5, display: "flex", flexDirection: "column", gap: 2,
                                        maxHeight: 140, overflowY: "auto" } }, entry.result.quantumStateLog.slice(0, 20).map((qs, i) => (React.createElement("div", { key: i, style: { display: "flex", alignItems: "center", gap: 6,
                                        fontSize: 7, fontFamily: "monospace",
                                        color: qs.corrected ? "#f59e0b" : "#334155",
                                        paddingLeft: 8, borderLeft: `2px solid ${qs.corrected ? "#f59e0b44" : "#0a162844"}` } },
                                    React.createElement("span", { style: { minWidth: 34 } },
                                        "#",
                                        qs.blockIdx),
                                    React.createElement("span", { style: { flex: 1, color: qs.corrected ? "#00d4ff" : "#475569" } }, qs.stateLabel),
                                    React.createElement("span", { style: { color: "#1e3a5f" } }, qs.collapseSignature))))))),
                            entry.result.chirality && (React.createElement("details", { style: { marginBottom: 7 } },
                                React.createElement("summary", { style: { fontSize: 8, color: "#a855f7", cursor: "pointer", letterSpacing: ".06em" } },
                                    "\u269B Kiralite simetrisi: ",
                                    entry.result.chirality.chiralityState),
                                React.createElement("div", { style: { marginTop: 5, display: "flex", flexDirection: "column", gap: 2,
                                        maxHeight: 140, overflowY: "auto" } },
                                    React.createElement("div", { style: { fontSize: 8, color: "#94a3b8", marginBottom: 4 } },
                                        entry.result.chirality.conjugateCount,
                                        " e\u015Flenik durum \u00B7",
                                        " ",
                                        entry.result.chirality.phaseDeltaCount,
                                        " faz-fark\u0131 blo\u011Fu",
                                        ((_a = entry.result.errorCorrelation) === null || _a === void 0 ? void 0 : _a.correlation) != null &&
                                            ` · hata korelasyonu r=${entry.result.errorCorrelation.correlation.toFixed(3)}`),
                                    entry.result.chirality.symmetryLog.slice(0, 15).map((s, i) => (React.createElement("div", { key: i, style: { fontSize: 7, fontFamily: "monospace", lineHeight: 1.5,
                                            color: s.isConjugate ? "#22d3ee" : "#f43f5e",
                                            paddingLeft: 8, borderLeft: `2px solid ${s.isConjugate ? "#22d3ee44" : "#f43f5e44"}` } }, s.label)))))),
                            entry.eventLog && entry.eventLog.length > 0 && (React.createElement("details", { style: { marginBottom: 7 } },
                                React.createElement("summary", { style: { fontSize: 8, color: "#334155", cursor: "pointer", letterSpacing: ".06em" } },
                                    entry.eventLog.length,
                                    " fiziksel olay \u2014 detay i\u00E7in a\u00E7"),
                                React.createElement("div", { style: { marginTop: 5, display: "flex", flexDirection: "column", gap: 3 } },
                                    entry.eventLog.slice(0, 8).map((ev, i) => {
                                        const edu = EDU_REFERENCES[ev.type];
                                        return (React.createElement("div", { key: i },
                                            React.createElement("div", { style: { fontSize: 8, color: (ERR[ev.type] || { c: "#64748b" }).c,
                                                    lineHeight: 1.5, paddingLeft: 8, borderLeft: `2px solid ${(ERR[ev.type] || { c: "#334155" }).c}44` } }, ev.text),
                                            edu && (React.createElement("details", { style: { marginLeft: 8, marginTop: 2 } },
                                                React.createElement("summary", { style: { fontSize: 7, color: "#334155", cursor: "pointer" } }, "\uD83D\uDCD6 Learn More \u2014 bilimsel ba\u011Flam ve kaynaklar"),
                                                React.createElement("div", { style: { fontSize: 7, color: "#64748b", lineHeight: 1.6, marginTop: 3, paddingLeft: 6 } },
                                                    edu.context,
                                                    React.createElement("div", { style: { marginTop: 4, display: "flex", flexDirection: "column", gap: 2 } }, edu.links.map((l, li) => (React.createElement("a", { key: li, href: l.url, target: "_blank", rel: "noopener noreferrer", style: { color: "#00d4ff", textDecoration: "none", fontSize: 7 } },
                                                        "\u2192 ",
                                                        l.label)))))))));
                                    }),
                                    entry.result.eccExplain && (React.createElement("div", { style: { fontSize: 8, color: "#10b981", lineHeight: 1.5, paddingLeft: 8,
                                            borderLeft: "2px solid #10b98144" } }, entry.result.eccExplain))))),
                            React.createElement("div", { style: { display: "flex", gap: 6 } },
                                React.createElement("button", { onClick: () => replayTimeline(entry.id), disabled: isReplaying, style: { flex: 1, padding: "6px", borderRadius: 5, cursor: isReplaying ? "not-allowed" : "pointer",
                                        border: "1px solid #a78bfa44", background: isReplaying ? "#0a1628" : "#a78bfa11",
                                        color: isReplaying ? "#334155" : "#a78bfa", fontSize: 9, fontWeight: 700, letterSpacing: ".04em" } }, isReplaying ? "◎ OYNATILIYOR…" : "▶ REPLAY"),
                                React.createElement("button", { onClick: () => {
                                        if (entangleA === entry.id)
                                            setEntangleA(null);
                                        else if (entangleB === entry.id)
                                            setEntangleB(null);
                                        else if (!entangleA)
                                            setEntangleA(entry.id);
                                        else if (!entangleB)
                                            setEntangleB(entry.id);
                                        else {
                                            setEntangleA(entry.id);
                                            setEntangleB(null);
                                        }
                                        ...
                                        color: (isA || isB) ? "#00d4ff" : "#334155", fontSize: 9, fontWeight: 700, letterSpacing: ".04em" } }, isA ? "⧉ A SEÇİLİ" : isB ? "⧉ B SEÇİLİ" : "⧉ SEÇ"),
                                React.createElement("button", { onClick: () => toggleGraphSelection(entry.id), style: { flex: 1, padding: "6px", borderRadius: 5, cursor: "pointer",
                                        ...
                                        color: graphSelection.includes(entry.id) ? "#f59e0b" : "#334155", fontSize: 9, fontWeight: 700, letterSpacing: ".04em" } }, graphSelection.includes(entry.id) ? "🕸 GRAFİKTE ✓" : "🕸 GRAFİĞE EKLE")),
                            React.createElement("div", { style: { display: "flex", gap: 6, marginTop: 6 } },
                                React.createElement("button", { onClick: () => openReportInNewTab(entry), style: { flex: 1, padding: "6px", borderRadius: 5, cursor: "pointer",
                                        border: "1px solid #0a1628", background: "transparent",
                                        color: "#475569", fontSize: 8, fontWeight: 600 } }, "\uD83D\uDCC4 Rapor (PDF i\u00E7in Ctrl+P)"),
                                React.createElement("button", { onClick: () => {
                                        var _a;
                                        ...
                                        (_a = navigator.clipboard) === null || _a === void 0 ? void 0 : _a.writeText(url).then(() => {
                                        ...
                                        border: "1px solid #0a1628", background: "transparent",
                                        color: "#475569", fontSize: 8, fontWeight: 600 } }, "\uD83D\uDD17 Payla\u015F")),
                            React.createElement("button", { onClick: () => retroCauseTimeline(entry.id), style: { width: "100%", marginTop: 6, padding: "7px", borderRadius: 6, cursor: "pointer",
                                    border: "1px solid #ec489944", background: "#ec489911", color: "#f472b6",
                                    fontSize: 9, fontWeight: 700, letterSpacing: ".05em",
                                    display: "flex", alignItems: "center", justifyContent: "center", gap: 6 } }, "\u23EA RETRO-CAUSE \u2014 Alternatif Kuantum Tesad\u00FCf\u00FCyle Yeniden \u00C7al\u0131\u015Ft\u0131r"),
                            (() => {
                                const myForks = forkedEntries.filter(f => f.sourceEntryId === entry.id);
                                if (myForks.length === 0)
                                    return null;
                                return (React.createElement("details", { style: { marginTop: 6 } },
                                    React.createElement("summary", { style: { fontSize: 8, color: "#f472b6", cursor: "pointer", letterSpacing: ".06em" } },
                                        "\u23EA ",
                                        myForks.length,
                                        " fork \u00FCretildi \u2014 g\u00F6r\u00FCnt\u00FClemek i\u00E7in a\u00E7"),
                                    React.createElement("div", { style: { marginTop: 5, display: "flex", flexDirection: "column", gap: 4 } }, myForks.map(fk => {
                                        const diverged = fk.result.decoded !== entry.result.decoded;
                                        const isActive = activeForkId === fk.id;
                                        return (React.createElement("div", { key: fk.id, onClick: () => setActiveForkId(isActive ? null : fk.id), style: {
                                                display: "flex", alignItems: "center", gap: 8, cursor: "pointer",
                                                padding: "5px 8px", borderRadius: 5,
                                                background: isActive ? "#ec489922" : "#020409",
                                                border: `1px solid ${isActive ? "#ec4899aa" : diverged ? "#f59e0b33" : "#10b98133"}`,
                                            } },
                                            React.createElement("span", { style: { fontSize: 8, color: diverged ? "#f59e0b" : "#10b981" } }, diverged ? "⚡" : "≈"),
                                            React.createElement("span", { style: { fontSize: 9, fontFamily: "monospace", color: "#94a3b8", flex: 1 } },
                                                "\"",
                                                fk.result.decoded,
                                                "\""),
                                            React.createElement("span", { style: { fontSize: 8, fontFamily: "monospace", color: "#64748b" } },
                                                "QBER ",
                                                (fk.result.qber * 100).toFixed(1),
                                                "%"),
                                            React.createElement("span", { style: { fontSize: 7, color: isActive ? "#ec4899" : "#334155" } }, isActive ? "HARİTADA ✓" : "göster")));
                                    }))));
                            })()));
                    }))),
                tab === "GRAPH" && (React.createElement("div", { style: { flex: 1, display: "flex", flexDirection: "column", zIndex: 1, position: "relative", overflow: "hidden" } },
                    React.createElement("div", { style: { padding: "10px 14px", borderBottom: "1px solid #0a162888",
                            display: "flex", alignItems: "center", gap: 10, flexShrink: 0 } },
                        React.createElement("span", { style: { fontSize: 9, color: "#1e3a5f", letterSpacing: ".1em" } },
                            "SE\u00C7\u0130L\u0130: ",
                            graphSelection.length,
                            " kay\u0131t"),
                        React.createElement("button", { onClick: createSuperposition, disabled: graphSelection.length < 2, style: { padding: "6px 14px", borderRadius: 6, cursor: graphSelection.length < 2 ? "not-allowed" : "pointer",
                                border: "1px solid #f59e0b55", background: graphSelection.length < 2 ? "#0a1628" : "#f59e0b11",
                                color: graphSelection.length < 2 ? "#334155" : "#f59e0b", fontSize: 9, fontWeight: 700, letterSpacing: ".04em" } }, "\u29C9 ENTANGLE \u2014 S\u00FCperpozisyon Olu\u015Ftur"),
                        graphSelection.length > 0 && (React.createElement("button", { onClick: () => setGraphSelection([]), style: { padding: "6px 10px", borderRadius: 6, cursor: "pointer",
                                border: "1px solid #0a1628", background: "transparent",
                                color: "#334155", fontSize: 9 } }, "Se\u00E7imi Temizle")),
                        React.createElement("span", { style: { marginLeft: "auto", fontSize: 8, color: "#1e3a5f" } },
                            superpositions.length,
                            " s\u00FCperpozisyon \u00B7 ",
                            entangleEdges.length,
                            " entangle kenar\u0131")),
                    React.createElement("div", { style: { flex: 1, display: "flex", minHeight: 0 } },
                        React.createElement("div", { style: { flex: 1, position: "relative" } }, timeline.length === 0 ? (React.createElement("div", { style: { position: "absolute", inset: 0, display: "flex", alignItems: "center",
                                justifyContent: "center", color: "#1e3a5f", fontSize: 12, textAlign: "center" } },
                            "Grafik i\u00E7in hen\u00FCz kay\u0131t yok.",
                            React.createElement("br", null),
                            React.createElement("span", { style: { fontSize: 10 } }, "Zaman \u00C7izelgesi sekmesinden iletim kaydedin."))) : (React.createElement("svg", { width: "100%", height: "100%", viewBox: "0 0 800 500", style: { display: "block" } },
                            React.createElement("defs", null,
                                React.createElement("filter", { id: "graphGlow" },
                            ...
                            entangleEdges.map((e, i) => {
                                ...
                                return (React.createElement("line", { key: i, x1: pa.x, y1: pa.y, x2: pb.x, y2: pb.y, stroke: collapsed ? "#334155" : "#f59e0b", strokeWidth: collapsed ? 0.6 : 1.4, strokeOpacity: collapsed ? 0.25 : 0.55, strokeDasharray: collapsed ? "none" : "5 4", style: collapsed ? {} : { animation: "flow .8s linear infinite" } }));
                            }),
                            timeline.map(t => {
                                ...
                                return (React.createElement("g", { key: t.id, onClick: () => toggleGraphSelection(t.id), style: { cursor: "pointer" } },
                                    selected && (React.createElement("circle", { cx: pos.x, cy: pos.y, r: 16, fill: "#f59e0b", opacity: .15, filter: "url(#graphGlow)" })),
                                    inSuperposition && (React.createElement("circle", { cx: pos.x, cy: pos.y, r: 13, fill: "none", stroke: "#f59e0b", strokeWidth: 1, strokeOpacity: .4, strokeDasharray: "3 2", style: { animation: "blink 1.5s ease-in-out infinite" } })),
                                    React.createElement("circle", { cx: pos.x, cy: pos.y, r: 9, fill: color + "22", stroke: selected ? "#f59e0b" : color, strokeWidth: selected ? 2 : 1.2 }),
                                    React.createElement("circle", { cx: pos.x, cy: pos.y, r: 3, fill: color }),
                                    React.createElement("text", { x: pos.x, y: pos.y + 18, textAnchor: "middle", fill: "#475569", fontSize: 7, fontFamily: "monospace" },
                                        t.src,
                                        "\u2192",
                                        t.dst)));
                            })))),
                        React.createElement("div", { style: { width: 240, flexShrink: 0, borderLeft: "1px solid #0a162888",
                                overflowY: "auto", padding: 12 } },
                            React.createElement("div", { style: { fontSize: 8, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 8 } }, "S\u00DCPERPOZ\u0130SYONLAR"),
                            superpositions.length === 0 ? (React.createElement("div", { style: { color: "#1e3a5f", fontSize: 10, textAlign: "center", marginTop: 30 } },
                                "Grafikte 2+ kay\u0131t se\u00E7ip",
                                React.createElement("br", null),
                                "\"Entangle\" ile olu\u015Fturun.")) : superpositions.map(sp => {
                                const members = sp.memberIds.map(id => timeline.find(t => t.id === id)).filter(Boolean);
                                return (React.createElement("div", { key: sp.id, style: { background: "#030610",
                                        border: `1px solid ${sp.collapsedTo ? "#33415555" : "#f59e0b44"}`,
                                        borderRadius: 7, padding: "9px 10px", marginBottom: 8 } },
                                    React.createElement("div", { style: { fontSize: 9, color: sp.collapsedTo ? "#334155" : "#f59e0b",
                                            fontWeight: 700, marginBottom: 6 } },
                                        sp.collapsedTo ? "⚛ ÇÖKMÜŞ" : "◈ SÜPERPOZİSYON",
                                        " (",
                                        members.length,
                                        ")"),
                                    members.map(m => (React.createElement("div", { key: m.id, style: { display: "flex", alignItems: "center", gap: 6,
                                            marginBottom: 4, fontSize: 9,
                                            opacity: sp.collapsedTo && sp.collapsedTo !== m.id ? 0.35 : 1 } },
                                        React.createElement("span", { style: { color: m.result.success ? "#22d3ee" : "#f43f5e" } }, sp.collapsedTo === m.id ? "●" : "○"),
                                        React.createElement("span", { style: { color: "#94a3b8", flex: 1, fontFamily: "monospace" } },
                                            "\"",
                                            m.msg.slice(0, 14),
                                            "\""),
                                        !sp.collapsedTo && (React.createElement("button", { onClick: () => collapseSuperposition(sp.id, m.id), style: { fontSize: 7, padding: "2px 6px", borderRadius: 3, cursor: "pointer",
                                                border: "1px solid #00d4ff44", background: "transparent", color: "#00d4ff" } }, "SE\u00C7"))))),
                                    !sp.collapsedTo && (React.createElement("button", { onClick: () => interfereSuperposition(sp.id), style: { width: "100%", marginTop: 6, padding: "6px", borderRadius: 5, cursor: "pointer",
                                            border: "1px solid #ec489944", background: "#ec489911", color: "#f472b6",
                                            fontSize: 8, fontWeight: 700, letterSpacing: ".04em" } }, "\u26A1 INTERFERENCE \u2014 Yeni Dalga Fonksiyonu \u00DCret"))));
                            }))))),
                tab === "WDG" && (React.createElement("div", { style: { flex: 1, overflowY: "auto", padding: 14, zIndex: 1, position: "relative" } },
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 10 } }, "KEND\u0130 KEND\u0130N\u0130 \u0130Y\u0130LE\u015ET\u0130RME \u2014 OTONOM A\u011E BEK\u00C7\u0130S\u0130 (Watchdog)"),
                    React.createElement("div", { style: { fontSize: 9, color: "#475569", lineHeight: 1.5, marginBottom: 14,
                            background: "#020409", border: "1px solid #0a1628", borderRadius: 5, padding: "8px 10px" } },
                        "Bu katman, y\u00F6nlendirme motorunun \u00FCretti\u011Fi loglar\u0131 ve d\u00FC\u011F\u00FCmlerin sentetik ya\u015Famsal bulgular\u0131n\u0131 (CPU/bellek/heartbeat) arka planda pasif olarak izler. Bir ",
                        React.createElement("b", { style: { color: "#f43f5e" } }, "thread kilitlenmesi"),
                        ",",
                        " ",
                        React.createElement("b", { style: { color: "#f59e0b" } }, "bellek s\u0131z\u0131nt\u0131s\u0131"),
                        " veya",
                        " ",
                        React.createElement("b", { style: { color: "#f43f5e" } }, "yan\u0131t vermeyen d\u00FC\u011F\u00FCm"),
                        " tespit edildi\u011Finde, Watchdog d\u00FC\u011F\u00FCm\u00FC otomatik olarak karantinaya al\u0131r (y\u00F6nlendirme motoru zaten bu alana sayg\u0131 g\u00F6sterir \u2014 bkz. D\u0130NAM\u0130K Y\u00DCK DENGELEME), asenkron olarak \"yeniden ba\u015Flat\u0131r\" ve insan m\u00FCdahalesi olmadan a\u011Fa geri entegre eder.",
                        " ",
                        React.createElement("b", { style: { color: "#a855f7" } }, "HAT BAZLI SU\u00C7LAMA:"),
                        " son 3 ba\u015Far\u0131s\u0131zl\u0131\u011F\u0131n ortak paydas\u0131 bir d\u00FC\u011F\u00FCm de\u011Fil de HEP AYNI fiziksel segment ise (\u00F6rn. tekrarlayan bir AN\u0130 SO\u011EURULMA BAR\u0130YER\u0130), Watchdog art\u0131k rotan\u0131n ortas\u0131ndaki ilgisiz bir d\u00FC\u011F\u00FCm\u00FC \"kovalamak\" yerine do\u011Frudan o HATTI karantinaya al\u0131r \u2014 y\u00F6nlendirme motoru ba\u015Fka bir fiziksel yol arar."),
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 7, marginBottom: 12 } }, [
                        ...
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 3 } }, m.l),
                        React.createElement("div", { style: { fontSize: 18, fontWeight: 700, color: m.c, fontFamily: "monospace" } }, m.v))))),
                    React.createElement("div", { style: { background: "#0a0d1f", border: "1px solid #00d4ff22", borderRadius: 8, padding: "13px 15px", marginBottom: 12 } },
                        React.createElement("div", { style: { fontSize: 10, color: "#00d4ff", fontWeight: 700, letterSpacing: ".08em", marginBottom: 8 } }, "\u26A1 ANOMAL\u0130 ENJEKS\u0130YONU (DEMO)"),
                        React.createElement("div", { style: { fontSize: 8, color: "#64748b", marginBottom: 10, lineHeight: 1.5 } }, "Bir d\u00FC\u011F\u00FCmde yapay olarak bir ar\u0131za t\u00FCr\u00FC tetikleyin \u2014 Watchdog'un otonom tespit \u2192 karantina \u2192 iyile\u015Ftirme d\u00F6ng\u00FCs\u00FCn\u00FC canl\u0131 izleyin. Aktif kaynak/hedef d\u00FC\u011F\u00FCm se\u00E7ilemez."),
                        React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap" } },
                            React.createElement("div", { style: { flex: "1 1 180px", minWidth: 150 } },
                                React.createElement("div", { style: { fontSize: 7, color: "#334155", marginBottom: 3 } }, "D\u00DC\u011E\u00DCM SE\u00C7"),
                                React.createElement("select", { value: anomalyNodeSel, onChange: e => setAnomalyNodeSel(e.target.value), style: { width: "100%", padding: "5px 6px", fontSize: 9 } },
                                    React.createElement("option", { value: "" }, "\u2014 d\u00FC\u011F\u00FCm se\u00E7in \u2014"),
                                    nodes.filter(n => n.id !== src && n.id !== dst).map(n => (React.createElement("option", { key: n.id, value: n.id },
                                        n.id,
                                        " \u2014 ",
                                        n.label,
                                        n.on ? "" : " (offline)"))))),
                            React.createElement("div", { style: { flex: "0 0 auto" } },
                                React.createElement("div", { style: { fontSize: 7, color: "#334155", marginBottom: 3 } }, "ANOMAL\u0130 T\u00DCR\u00DC"),
                                React.createElement("div", { style: { display: "flex", gap: 4 } }, [["lockup", "🔒 KİLİTLENME"], ["leak", "💧 BELLEK SIZINTISI"], ["unresponsive", "📡 YANITSIZ"]].map(([k, l]) => (React.createElement("button", { key: k, onClick: () => setAnomalyTypeSel(k), style: { padding: "5px 9px", fontSize: 8, borderRadius: 4, cursor: "pointer",
                                        background: anomalyTypeSel === k ? "#f43f5e22" : "transparent",
                                        border: `1px solid ${anomalyTypeSel === k ? "#f43f5e77" : "#0a1628"}`,
                                        color: anomalyTypeSel === k ? "#f43f5e" : "#334155", fontWeight: 700 } }, l))))),
                            React.createElement("button", { onClick: injectAnomaly, disabled: !anomalyNodeSel, style: { padding: "6px 14px", fontSize: 9, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                                    background: "linear-gradient(135deg,#f43f5e,#7c3aed)", border: "none", color: "#fff",
                                    opacity: !anomalyNodeSel ? 0.4 : 1 } }, "\u26A1 ANOMAL\u0130Y\u0130 TET\u0130KLE"))),
                    Object.keys(activeAnomalies).length > 0 && (React.createElement("div", { style: { marginBottom: 12 } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 6 } }, "AKT\u0130F ANOMAL\u0130LER"),
                        Object.entries(activeAnomalies).map(([nodeId, a]) => (React.createElement("div", { key: nodeId, style: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap",
                            ...
                            React.createElement("div", { style: { width: 6, height: 6, borderRadius: "50%",
                                    ...
                                    animation: "blink 0.6s ease-in-out infinite" } }),
                            React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: a.phase === "healing" ? "#10b981" : "#f43f5e" } },
                                nodeId,
                                " \u2014 ",
                                a.type === "lockup" ? "THREAD KİLİTLENMESİ" : a.type === "leak" ? "BELLEK SIZINTISI" : "YANITSIZ"),
                            React.createElement("span", { style: { fontSize: 8, color: "#64748b" } }, a.phase === "healing" ? "🔧 asenkron olarak yeniden başlatılıyor..." : "🔴 karantinada — tanılanıyor"),
                            React.createElement("button", { onClick: () => forceHealAnomaly(nodeId), style: { marginLeft: "auto", padding: "4px 10px", fontSize: 8, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                                    background: "none", border: "1px solid #33415577", color: "#94a3b8" } }, "\u2715 HEMEN \u0130Y\u0130LE\u015ET\u0130R")))))),
                    Object.keys(activeLinkQuarantines).length > 0 && (React.createElement("div", { style: { marginBottom: 12 } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 6 } }, "\uD83D\uDD17 AKT\u0130F HAT KARANT\u0130NALARI (Link-Based Blaming)"),
                        Object.entries(activeLinkQuarantines).map(([linkKey, q]) => (React.createElement("div", { key: linkKey, style: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap",
                                background: "#2a123f33", border: "1px solid #a855f744",
                                borderRadius: 6, padding: "7px 11px", marginBottom: 6 } },
                            React.createElement("div", { style: { width: 6, height: 6, borderRadius: "50%",
                                    background: "#a855f7", boxShadow: "0 0 6px #a855f7",
                                    animation: "blink 0.6s ease-in-out infinite" } }),
                            React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: "#a855f7" } },
                                q.a,
                                "\u2192",
                                q.b,
                                " HATTI KARANT\u0130NADA"),
                            React.createElement("span", { style: { fontSize: 8, color: "#64748b" } }, "\uD83D\uDD00 d\u00FC\u011F\u00FCm de\u011Fil, hatt\u0131n kendisi su\u00E7land\u0131 \u2014 y\u00F6nlendirme motoru alternatif fiziksel yol kullan\u0131yor"),
                            React.createElement("button", { onClick: () => linkOutageController.cancel(), style: { marginLeft: "auto", padding: "4px 10px", fontSize: 8, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                                    background: "none", border: "1px solid #33415577", color: "#94a3b8" } }, "\u2715 HEMEN ONAR")))))),
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 6 } },
                        "D\u00DC\u011E\u00DCM SA\u011ELIK IZGARASI (",
                        nodes.filter(n => n.on).length,
                        "/",
                        nodes.length,
                        " \u00E7evrimi\u00E7i)"),
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(110px,1fr))", gap: 6 } }, nodes.map(n => {
                        var _a, _b;
                        ...
                        const statusColor = !n.on ? (preventive && !anomaly ? "#f59e0b" : "#f43f5e") : ((h === null || h === void 0 ? void 0 : h.status) === "CRIT" ? "#f59e0b" : "#10b981");
                        const offLabel = anomaly
                            ? (anomaly.phase === "healing" ? "YENİDEN BAŞLATILIYOR" : "KARANTİNADA")
                            : preventive ? "ÖNLEYİCİ BAKIMDA" : "KAPALI (elle)";
                        return (React.createElement("div", { key: n.id, style: { background: "#030610",
                            ...
                            React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 } },
                                React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: "#00d4ff" } }, n.id),
                                React.createElement("div", { style: { width: 5, height: 5, borderRadius: "50%", background: statusColor,
                                        boxShadow: `0 0 4px ${statusColor}`,
                                        animation: !n.on ? "blink 0.8s ease-in-out infinite" : "none" } })),
                            React.createElement("div", { style: { fontSize: 7, color: !n.on ? statusColor : "#475569" } }, !n.on ? offLabel : "ÇEVRİMİÇİ"),
                            h && n.on && (React.createElement("div", { style: { fontSize: 6, color: "#334155", marginTop: 2, fontFamily: "monospace" } },
                                "CPU ", (_a = h.cpu) === null || _a === void 0 ? void 0 :
                                _a.toFixed(0),
                                "% \u00B7 MEM ", (_b = h.mem) === null || _b === void 0 ? void 0 :
                                _b.toFixed(0),
                                "%"))));
                    })))),
                tab === "PRED" && (React.createElement("div", { style: { flex: 1, overflowY: "auto", padding: 14, zIndex: 1, position: "relative" } },
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 10 } }, "TAHM\u0130NLEME MODEL\u0130 \u2014 PROAKT\u0130F DARBO\u011EAZ/GEC\u0130KME \u00D6NG\u00D6R\u00DCS\u00DC"),
                    React.createElement("div", { style: { fontSize: 9, color: "#475569", lineHeight: 1.5, marginBottom: 14,
                            background: "#020409", border: "1px solid #0a1628", borderRadius: 5, padding: "8px 10px" } },
                        "Her link ve d\u00FC\u011F\u00FCm metri\u011Fi s\u00FCrekli \u00F6rneklenir; ",
                        React.createElement("b", { style: { color: "#00d4ff" } }, "hareketli ortalama"),
                        " (g\u00FCr\u00FClt\u00FCy\u00FC yumu\u015Fat\u0131r), ",
                        React.createElement("b", { style: { color: "#00d4ff" } }, "h\u0131z"),
                        " (en k\u00FC\u00E7\u00FCk kareler regresyon e\u011Fimi) ve ",
                        React.createElement("b", { style: { color: "#00d4ff" } }, "ivme"),
                        " (h\u0131z\u0131n de\u011Fi\u015Fim oran\u0131) hesaplan\u0131r; kinematik ekstrapolasyonla ",
                        React.createElement("b", { style: { color: "#7c3aed" } }, "5 dakika sonras\u0131n\u0131n"),
                        " tahmini \u00FCretilir. Risk e\u015Fi\u011Fi a\u015F\u0131lacaksa sistem KR\u0130Z OLU\u015EMADAN \u00D6NCE trafi\u011Fi proaktif olarak kayd\u0131r\u0131r (linkler) veya d\u00FC\u011F\u00FCm\u00FC k\u0131sa bir \u00F6nleyici bak\u0131m d\u00F6ng\u00FCs\u00FCne al\u0131r (d\u00FC\u011F\u00FCmler)."),
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 7, marginBottom: 12 } }, [
                        ...
                        React.createElement("div", { style: { fontSize: 18, fontWeight: 700, color: m.c, fontFamily: "monospace" } }, m.v))))),
                    React.createElement("div", { style: { background: "#0a0d1f", border: "1px solid #00d4ff22", borderRadius: 8, padding: "13px 15px", marginBottom: 12 } },
                        React.createElement("div", { style: { fontSize: 10, color: "#00d4ff", fontWeight: 700, letterSpacing: ".08em", marginBottom: 8 } }, "\uD83D\uDCC8 TREND ENJEKS\u0130YONU (DEMO)"),
                        React.createElement("div", { style: { fontSize: 8, color: "#64748b", marginBottom: 10, lineHeight: 1.5 } }, "Bir hatt\u0131n/d\u00FC\u011F\u00FCm\u00FCn y\u00FCk\u00FCn\u00FC YAVA\u015E\u00C7A (ani de\u011Fil) y\u00FCkseltin \u2014 motorun krizi tepe noktas\u0131ndan \u00C7OK \u00D6NCE \u00F6ng\u00F6rd\u00FC\u011F\u00FCn\u00FC ve \u00F6nlem ald\u0131\u011F\u0131n\u0131 canl\u0131 izleyin."),
                        React.createElement("div", { style: { marginBottom: 10 } },
                            React.createElement("div", { style: { fontSize: 7, color: "#334155", marginBottom: 4 } }, "HAT TREND\u0130"),
                            React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap" } },
                                React.createElement("div", { style: { flex: "1 1 200px", minWidth: 170 } },
                                    React.createElement("select", { value: trendLinkSel, disabled: linkTrendBusy, onChange: e => setTrendLinkSel(e.target.value), style: { width: "100%", padding: "5px 6px", fontSize: 9 } },
                                        React.createElement("option", { value: "" }, "\u2014 hat se\u00E7in \u2014"),
                                        [...links].sort((a, b) => b.km - a.km).map(l => (React.createElement("option", { key: `${l.a}-${l.b}`, value: `${l.a}-${l.b}` },
                                            l.a,
                                            " \u2192 ",
                                            l.b,
                                            " (",
                                            l.km,
                                            "km)"))))),
                                !linkTrendBusy ? (React.createElement("button", { onClick: injectLinkTrend, disabled: !trendLinkSel, style: { padding: "6px 14px", fontSize: 9, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                                        background: "linear-gradient(135deg,#00d4ff,#7c3aed)", border: "none", color: "#fff",
                                        opacity: !trendLinkSel ? 0.4 : 1 } }, "\uD83D\uDCC8 HAT Y\u00DCK\u00DCN\u00DC Y\u00DCKSELT")) : (React.createElement("button", { onClick: cancelLinkTrend, style: { padding: "6px 14px", fontSize: 9, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                                        background: "none", border: "1px solid #33415577", color: "#94a3b8" } }, "\u2715 \u0130PTAL ET")))),
                        React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 7, color: "#334155", marginBottom: 4 } }, "D\u00DC\u011E\u00DCM TREND\u0130"),
                            React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap" } },
                                React.createElement("div", { style: { flex: "1 1 160px", minWidth: 140 } },
                                    React.createElement("select", { value: trendNodeSel, disabled: nodeTrendBusy, onChange: e => setTrendNodeSel(e.target.value), style: { width: "100%", padding: "5px 6px", fontSize: 9 } },
                                        React.createElement("option", { value: "" }, "\u2014 d\u00FC\u011F\u00FCm se\u00E7in \u2014"),
                                        nodes.filter(n => n.id !== src && n.id !== dst).map(n => (React.createElement("option", { key: n.id, value: n.id },
                                            n.id,
                                            " \u2014 ",
                                            n.label))))),
                                React.createElement("div", { style: { display: "flex", gap: 4 } }, [["cpu", "CPU"], ["mem", "BELLEK"]].map(([k, l]) => (React.createElement("button", { key: k, disabled: nodeTrendBusy, onClick: () => setTrendMetricSel(k), style: { padding: "5px 9px", fontSize: 8, borderRadius: 4, cursor: "pointer",
                                        background: trendMetricSel === k ? "#7c3aed22" : "transparent",
                                        border: `1px solid ${trendMetricSel === k ? "#7c3aed77" : "#0a1628"}`,
                                        color: trendMetricSel === k ? "#7c3aed" : "#334155", fontWeight: 700 } }, l)))),
                                !nodeTrendBusy ? (React.createElement("button", { onClick: injectNodeTrend, disabled: !trendNodeSel, style: { padding: "6px 14px", fontSize: 9, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                                        background: "linear-gradient(135deg,#00d4ff,#7c3aed)", border: "none", color: "#fff",
                                        opacity: !trendNodeSel ? 0.4 : 1 } }, "\uD83D\uDCC8 D\u00DC\u011E\u00DCM Y\u00DCK\u00DCN\u00DC Y\u00DCKSELT")) : (React.createElement("button", { onClick: cancelNodeTrend, style: { padding: "6px 14px", fontSize: 9, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                                        background: "none", border: "1px solid #33415577", color: "#94a3b8" } }, "\u2715 \u0130PTAL ET"))))),
                    Object.keys(activePreventive).length > 0 && (React.createElement("div", { style: { marginBottom: 12 } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 6 } }, "AKT\u0130F \u00D6NLEY\u0130C\u0130 BAKIM"),
                        Object.entries(activePreventive).map(([nodeId, pv]) => (React.createElement("div", { key: nodeId, style: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap",
                                background: "#2a1a0533", border: "1px solid #f59e0b44",
                                borderRadius: 6, padding: "7px 11px", marginBottom: 6 } },
                            React.createElement("div", { style: { width: 6, height: 6, borderRadius: "50%", background: "#f59e0b",
                                    boxShadow: "0 0 6px #f59e0b", animation: "blink 0.6s ease-in-out infinite" } }),
                            React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: "#f59e0b" } },
                                "\uD83D\uDD2E ",
                                nodeId,
                                " \u2014 proaktif bak\u0131mda"),
                            React.createElement("span", { style: { fontSize: 8, color: "#64748b" } }, pv.reason),
                            React.createElement("button", { onClick: () => forceHealAnomaly(nodeId), style: { marginLeft: "auto", padding: "4px 10px", fontSize: 8, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                                    background: "none", border: "1px solid #33415577", color: "#94a3b8" } }, "\u2715 HEMEN TAMAMLA")))))),
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 6 } }, "R\u0130SK L\u0130DER TABLOSU (en riskliden aza, ufuk: 5dk)"),
                    predictiveInsights.length === 0 ? (React.createElement("div", { style: { fontSize: 8, color: "#334155", padding: "6px 2px" } }, "Hen\u00FCz yeterli \u00F6rnek toplanmad\u0131 \u2014 birka\u00E7 saniye bekleyin veya bir trend enjekte edin.")) : (React.createElement("div", { style: { display: "grid", gap: 4 } }, predictiveInsights.map(ins => {
                        ...
                        return (React.createElement("div", { key: ins.key, style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap",
                                background: "#020409", border: `1px solid ${lc}22`, borderRadius: 4, padding: "6px 9px", fontFamily: "monospace" } },
                            React.createElement("span", { style: { fontSize: 11 } }, ins.kind === "link" ? "🔗" : "⚙"),
                            React.createElement("span", { style: { fontSize: 9, color: "#94a3b8", fontWeight: 700, flexShrink: 0, minWidth: 110 } }, ins.label),
                            React.createElement("span", { style: { fontSize: 7, color: lc, fontWeight: 700, padding: "1px 6px", borderRadius: 3, background: lc + "18" } }, ins.level),
                            React.createElement("span", { style: { fontSize: 8, color: "#334155" } },
                                "SMA %",
                                (ins.sma * 100).toFixed(0)),
                            React.createElement("span", { style: { fontSize: 8, color: ins.velocity > 0 ? "#f43f5e" : "#10b981" } },
                                ins.velocity > 0 ? "↑" : "↓",
                                " ",
                                (Math.abs(ins.velocity) * 100).toFixed(2),
                                "/sn"),
                            React.createElement("span", { style: { fontSize: 8, color: "#334155" } },
                                "ivme ",
                                ins.acceleration >= 0 ? "+" : "",
                                ins.acceleration.toFixed(3),
                                "/sn\u00B2"),
                            React.createElement("span", { style: { fontSize: 8, color: "#7c3aed" } },
                                "tahmin(5dk) %",
                                (ins.predicted * 100).toFixed(0)),
                            React.createElement("span", { style: { fontSize: 8, color: "#64748b", marginLeft: "auto" } },
                                "ETA ",
                                etaTxt),
                            React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: lc } },
                                "%",
                                (ins.risk * 100).toFixed(0))));
                    }))))),
                tab === "NGM" && (React.createElement("div", { style: { flex: 1, overflowY: "auto", padding: 14, zIndex: 1, position: "relative" } },
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 10 } }, "KUANTUM G\u00DCR\u00DCLT\u00DC F\u0130LTRELEME VE KAL\u0130BRASYON \u2014 NOISE-GATE MIDDLEWARE"),
                    React.createElement("div", { style: { fontSize: 9, color: "#475569", lineHeight: 1.5, marginBottom: 14,
                            background: "#020409", border: "1px solid #0a1628", borderRadius: 5, padding: "8px 10px" } },
                        "SNSPD (Superconducting Nanowire Single-Photon Detector) m\u00FCkemmel de\u011Fildir: s\u0131cakl\u0131k s\u00FCr\u00FCklenmesi veya fiber s\u0131z\u0131nt\u0131s\u0131, foton gelmedi\u011Fi h\u00E2lde dedekt\u00F6r\u00FC ",
                        React.createElement("b", { style: { color: "#00d4ff" } }, "hayalet bir \"click\""),
                        " \u00FCretmeye zorlayabilir (karanl\u0131k say\u0131m). Bu katman g\u00FCr\u00FClt\u00FCy\u00FC periyodik olarak ",
                        React.createElement("b", { style: { color: "#00d4ff" } }, "kalibre eder"),
                        " ve zaman-korelasyonlu bir ",
                        React.createElement("b", { style: { color: "#7c3aed" } }, "kap\u0131 (gate)"),
                        " ile filtreler \u2014 kap\u0131y\u0131 daraltmak g\u00FCr\u00FClt\u00FCy\u00FC azalt\u0131r ama ger\u00E7ek fotonlar\u0131n bir k\u0131sm\u0131n\u0131n ka\u00E7\u0131r\u0131lma riskini de (k\u00FC\u00E7\u00FCk \u00F6l\u00E7\u00FCde) art\u0131r\u0131r \u2014 d\u00FCr\u00FCst bir m\u00FChendislik \u00F6d\u00FCnle\u015Fimi, sahte bir kesinlik de\u011Fil."),
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 7, marginBottom: 8 } }, [
                        ...
                        { l: "KAPI GENİŞLİĞİ", v: `%${(noiseGateState.gateRatio * 100).toFixed(0)}`, c: noiseGateState.gateRatio < 0.999 ? "#7c3aed" : "#334155" },
                            ...
                            borderRadius: 4, padding: "9px 10px" } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 3 } }, m.l),
                        React.createElement("div", { style: { fontSize: 16, fontWeight: 700, color: m.c, fontFamily: "monospace" } }, m.v))))),
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 7, marginBottom: 12 } }, [
                        ...
                        React.createElement("div", { style: { fontSize: 16, fontWeight: 700, color: m.c, fontFamily: "monospace" } }, m.v))))),
                    React.createElement("div", { style: { background: "#0a0d1f", border: "1px solid #00d4ff22", borderRadius: 8, padding: "13px 15px", marginBottom: 12 } },
                        React.createElement("div", { style: { fontSize: 10, color: "#00d4ff", fontWeight: 700, letterSpacing: ".08em", marginBottom: 8 } }, "\uD83C\uDF21\uFE0F SNSPD SICAKLIK S\u00DCR\u00DCKLENMES\u0130"),
                        React.createElement("div", { style: { position: "relative", height: 14, borderRadius: 7, overflow: "hidden",
                                background: "linear-gradient(90deg,#f43f5e 0%,#f59e0b 30%,#10b981 46%,#10b981 54%,#f59e0b 70%,#f43f5e 100%)", marginBottom: 6 } },
                            React.createElement("div", { style: { position: "absolute", top: -2, bottom: -2, width: 3, borderRadius: 2,
                        ...
                        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", fontSize: 7, color: "#334155", fontFamily: "monospace" } },
                            React.createElement("span", null, "-25mK"),
                            React.createElement("span", null, "nominal (0mK)"),
                            React.createElement("span", null, "+25mK")),
                        React.createElement("div", { style: { marginTop: 8, fontSize: 8, color: "#64748b" } },
                            "Trip e\u015Fi\u011Fi: \u00B1",
                            SNSPD_TEMP_TRIP_MK,
                            "mK \u2014 bu e\u015Fi\u011Fi a\u015F\u0131nca karanl\u0131k say\u0131m oran\u0131 \u00FCstel olarak artmaya ba\u015Flar (Arrhenius-benzeri termal aktivasyon).")),
                    React.createElement("div", { style: { background: "#030610", border: "1px solid #0a162888",
                            borderRadius: 4, padding: "9px 10px", marginBottom: 12 } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 5 } }, "KAL\u0130BRE ED\u0130LM\u0130\u015E KARANLIK ORANI \u2014 GE\u00C7M\u0130\u015E"),
                        React.createElement(Spark, { data: noiseGateHistory, color: "#00d4ff", h: 38 })),
                    React.createElement("div", { style: { background: "#0a0d1f", border: "1px solid #00d4ff22", borderRadius: 8, padding: "13px 15px", marginBottom: 12 } },
                        React.createElement("div", { style: { fontSize: 10, color: "#00d4ff", fontWeight: 700, letterSpacing: ".08em", marginBottom: 8 } }, "\u26A1 G\u00DCR\u00DCLT\u00DC ENJEKS\u0130YONU (DEMO)"),
                        React.createElement("div", { style: { fontSize: 8, color: "#64748b", marginBottom: 10, lineHeight: 1.5 } }, "SNSPD'nin bias noktas\u0131n\u0131 yapay olarak zorlay\u0131n veya fiber hattan bir s\u0131z\u0131nt\u0131 sim\u00FCle edin \u2014 G\u00FCr\u00FClt\u00FC Kap\u0131s\u0131'n\u0131n otonom kalibrasyon/daralma d\u00F6ng\u00FCs\u00FCn\u00FC canl\u0131 izleyin."),
                        React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" } },
                            React.createElement("button", { onClick: injectThermalFluctuation, style: { padding: "6px 14px", fontSize: 9, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                                    background: "linear-gradient(135deg,#f59e0b,#f43f5e)", border: "none", color: "#fff" } }, "\uD83C\uDF21\uFE0F Is\u0131l Dalgalanma Enjekte Et"),
                            React.createElement("button", { onClick: toggleFiberLeak, style: { padding: "6px 14px", fontSize: 9, fontWeight: 700, borderRadius: 4, cursor: "pointer",
                    ...
                    React.createElement("div", { style: { fontSize: 8, color: "#334155", lineHeight: 1.5 } }, "Not: bu katman yaln\u0131zca BB84 anahtar-de\u011Fi\u015Fim kanal\u0131n\u0131 (deriveSiftedKey) etkiler \u2014 mesaj fizi\u011Fi (propPhoton) ve rotalama motoruna hi\u00E7 dokunmaz (bkz. EK 44 DetectorNoiseModel ile ayn\u0131 mimari s\u0131n\u0131r)."))),
                tab === "HW" && (React.createElement("div", { style: { flex: 1, overflowY: "auto", padding: 14, zIndex: 1, position: "relative" } },
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 10 } }, "DONANIM K\u00D6PR\u00DCS\u00DC \u2014 HAL (Hardware Abstraction Layer)"),
                    React.createElement("div", { style: { fontSize: 9, color: "#475569", lineHeight: 1.5, marginBottom: 14,
                            background: "#020409", border: "1px solid #0a1628", borderRadius: 5, padding: "8px 10px" } },
                        "Bu panel, PhotonNet'in kendi taray\u0131c\u0131-i\u00E7i sim\u00FClasyonundan (propPhoton/deriveSiftedKey)",
                        " ",
                        React.createElement("b", { style: { color: "#7c3aed" } }, "tamamen ba\u011F\u0131ms\u0131z"),
                        " ayr\u0131 bir Python s\u00FCrecine (",
                        React.createElement("code", { style: { color: "#00d4ff" } }, "python3 -m hal.bridge_server"),
                        ") ba\u011Flan\u0131r. Orada QBER, RNG ile \u00FCretilmez \u2014 ger\u00E7ek (veya ",
                        React.createElement("code", { style: { color: "#00d4ff" } }, "hal/simulated_hardware.py"),
                        "'nin \u00FCretti\u011Fi ger\u00E7ek\u00E7i) zaman-damgal\u0131 click'lerin ",
                        React.createElement("code", { style: { color: "#00d4ff" } }, "TimeTagCorrelator"),
                        " ile e\u015Fle\u015Ftirilmesiyle hesaplan\u0131r. Ger\u00E7ek cihaz ba\u011Fland\u0131\u011F\u0131nda yaln\u0131zca",
                        " ",
                        React.createElement("code", { style: { color: "#00d4ff" } }, "hardware_interface.HardwareInterface"),
                        "'ten t\u00FCreyen yeni bir s\u00FCr\u00FCc\u00FC yaz\u0131l\u0131r \u2014 bu panel DE\u011E\u0130\u015EMEDEN \u00E7al\u0131\u015Fmaya devam eder."),
                    React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 12 } },
                        ...
                        React.createElement("input", { value: hwBridgeUrl, onChange: e => setHwBridgeUrl(e.target.value), placeholder: "http://localhost:8765", style: { flex: 1, background: "#020409", border: "1px solid #0a1628", borderRadius: 4,
                                color: "#00d4ff", fontSize: 10, fontFamily: "monospace", padding: "6px 8px", outline: "none" } }),
                        React.createElement("button", { className: "nbtn", onClick: hwCheckHealth, style: { fontSize: 8, color: "#334155", letterSpacing: ".06em", flexShrink: 0 } }, "SUNUCUYU DENE")),
                    React.createElement("div", { style: { background: "#020409", border: "1px solid #0a1628", borderRadius: 5,
                            padding: "9px 10px", marginBottom: 12 } },
                        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 } },
                            React.createElement("span", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em" } }, "LINKMANAGER DURUMU"),
                            React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: hwStatus.connected ? "#10b981" : "#475569" } }, hwStatus.connected ? (hwStatus.armed ? "● BAĞLI / ARM" : "● BAĞLI") : "○ BAĞLI DEĞİL")),
                        ...
                        ].map(([l, v, c]) => (React.createElement("div", { key: l, style: { background: "#01030a", border: "1px solid #0a1628", borderRadius: 3, padding: "5px 6px" } },
                            React.createElement("div", { style: { fontSize: 6, color: "#1e3a5f", marginBottom: 1 } }, l),
                            React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: c, fontFamily: "monospace" } }, v))))),
                        hwStatus.last_error && (React.createElement("div", { style: { fontSize: 8, color: "#f43f5e", marginTop: 6 } },
                            "\u26A0 ",
                            hwStatus.last_error))),
                    React.createElement("div", { style: { background: "#020409", border: "1px solid #0a1628", borderRadius: 5,
                            padding: "9px 10px", marginBottom: 12 } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 8 } }, "ED\u0130N\u0130M PARAMETRELER\u0130"),
                        React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 12, marginBottom: 8 } },
                            React.createElement("label", { style: { fontSize: 8, color: "#475569", display: "flex", flexDirection: "column", gap: 3, flex: 1 } },
                                "MESAFE (km)",
                                React.createElement("input", { type: "number", min: 0, step: 1, value: hwDistanceKm, disabled: hwStatus.connected, onChange: e => setHwDistanceKm(Math.max(0, +e.target.value || 0)), style: { background: "#01030a", border: "1px solid #0a1628", borderRadius: 3,
                                        color: "#00d4ff", fontSize: 10, fontFamily: "monospace", padding: "5px 6px", outline: "none",
                                        opacity: hwStatus.connected ? .5 : 1 } })),
                            React.createElement("label", { style: { fontSize: 8, color: "#475569", display: "flex", flexDirection: "column", gap: 3, flex: 1 } },
                                "ED\u0130N\u0130M S\u00DCRES\u0130 (ms)",
                                React.createElement("input", { type: "number", min: 0.01, step: 0.1, value: hwDurationMs, onChange: e => setHwDurationMs(Math.max(0.01, +e.target.value || 0.01)), style: { background: "#01030a", border: "1px solid #0a1628", borderRadius: 3,
                                        color: "#00d4ff", fontSize: 10, fontFamily: "monospace", padding: "5px 6px", outline: "none" } })),
                            React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4 } },
                                React.createElement("div", { onClick: () => !hwStatus.connected && setHwEavesdrop(v => !v), style: { width: 34, height: 18, borderRadius: 9, cursor: hwStatus.connected ? "not-allowed" : "pointer",
                                    ...
                                    React.createElement("div", { style: { width: 12, height: 12, borderRadius: "50%", background: "#fff",
                                            position: "absolute", top: 2, left: hwEavesdrop ? 18 : 2, transition: "left .3s" } })),
                                React.createElement("span", { style: { fontSize: 7, color: hwEavesdrop ? "#00d4ff" : "#475569", whiteSpace: "nowrap" } }, "Eve"))),
                        React.createElement("div", { style: { display: "flex", gap: 8 } },
                            !hwStatus.connected ? (React.createElement("button", { className: "nbtn", onClick: hwConnect, disabled: hwConnecting, style: { flex: 1, fontSize: 9, color: "#00d4ff", letterSpacing: ".06em", padding: "7px 0",
                                    opacity: hwConnecting ? .5 : 1 } }, hwConnecting ? "BAĞLANIYOR…" : "🔗 BAĞLAN")) : (React.createElement("button", { className: "nbtn", onClick: hwDisconnect, style: { flex: 1, fontSize: 9, color: "#f43f5e", letterSpacing: ".06em", padding: "7px 0" } }, "\u2715 BA\u011ELANTIYI KES")),
                            React.createElement("button", { className: "nbtn", onClick: hwAcquire, disabled: hwAcquiring, style: { flex: 1, fontSize: 9, color: "#10b981", letterSpacing: ".06em", padding: "7px 0",
                                    opacity: hwAcquiring ? .5 : 1 } }, hwAcquiring ? "EDİNİLİYOR…" : "⚡ TEK-SEFERLİK EDİNİM")),
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", marginTop: 6, lineHeight: 1.4 } }, "BA\u011ELAN, LinkManager'\u0131 s\u00FCrekli arka-plan modunda ba\u015Flat\u0131r (durum/telemetri SSE ile akar). TEK-SEFERL\u0130K ED\u0130N\u0130M ise ba\u011F\u0131ms\u0131z, senkron bir BB84 oturumu \u00E7al\u0131\u015Ft\u0131r\u0131r \u2014 ba\u011Fl\u0131 olmak GEREKMEZ.")),
                    hwResult && (React.createElement("div", { style: { background: "#020409", border: "1px solid #0a1628", borderRadius: 5, padding: "9px 10px" } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 8 } }, "SON DONANIM ED\u0130N\u0130M SONUCU"),
                        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6, marginBottom: 8 } }, [
                            ...
                            React.createElement("div", { style: { fontSize: 6, color: "#1e3a5f", marginBottom: 1 } }, l),
                            React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: c, fontFamily: "monospace" } }, v))))),
                        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 8 } }, [
                            ...
                            React.createElement("div", { style: { fontSize: 6, color: "#1e3a5f", marginBottom: 1 } }, l),
                            React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: c, fontFamily: "monospace" } }, v))))),
                        React.createElement("div", { style: { fontSize: 9, fontWeight: 700, letterSpacing: ".06em",
                                color: hwResult.result.eavesdrop_detected ? "#f43f5e" : "#10b981",
                                display: "flex", alignItems: "center", gap: 5 } },
                            React.createElement("div", { style: { width: 6, height: 6, borderRadius: "50%", flexShrink: 0,
                        ...
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", marginTop: 4, fontFamily: "monospace" } },
                            "ger\u00E7ek i\u015Flem s\u00FCresi: ",
                            hwResult.wall_clock_s,
                            "s \u00B7 mesafe: ",
                            hwResult.distance_km,
                            "km"))))),
                tab === "LOG" && (React.createElement("div", { style: { flex: 1, overflowY: "auto", padding: 10, zIndex: 1, position: "relative" } },
                    React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 7 } },
                        React.createElement("span", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em" } },
                            "S\u0130STEM KAYDI \u2014 ",
                            log.length,
                            " SATIR"),
                        React.createElement("button", { className: "nbtn", onClick: () => setLog([]), style: { fontSize: 8, color: "#334155", letterSpacing: ".08em" } }, "TEM\u0130ZLE")),
                    log.map(e => {
                        var _a;
                        return (React.createElement("div", { key: e.id, style: { display: "flex", gap: 7, padding: "2px 4px", borderRadius: 2,
                                background: (_a = LB[e.type]) !== null && _a !== void 0 ? _a : "transparent", marginBottom: 1 } },
                            React.createElement("span", { style: { color: "#1e3a5f", flexShrink: 0, fontSize: 8, fontFamily: "monospace" } }, e.ts),
                            React.createElement("span", { style: { color: LC[e.type] || "#334155", flexShrink: 0, fontSize: 8, fontWeight: 700, minWidth: 26 } }, e.type),
                            React.createElement("span", { style: { color: LC[e.type] || "#475569", fontSize: 9, lineHeight: 1.4 } }, e.text)));
                    })))),
            React.createElement("div", { style: { width: 230, flexShrink: 0, background: "#030610",
                    borderLeft: "1px solid #0a162888", display: "flex", flexDirection: "column" } },
                result && (React.createElement("div", { style: { padding: "11px 11px", borderBottom: "1px solid #0a162888", flexShrink: 0 } },
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 8 } }, "SON \u0130LET\u0130M SONUCU"),
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 7 } }, [["TX", `"${result.original}"`, "#00d4ff"],
                        ["RX", `"${result.decoded}"`, result.success ? "#10b981" : "#f59e0b"]].map(([l, v, c]) => (React.createElement("div", { key: l, style: { background: "#020409", border: `1px solid ${c}22`,
                            borderRadius: 4, padding: "6px 8px" } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", marginBottom: 2 } }, l),
                        React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: c,
                                fontFamily: "monospace", wordBreak: "break-all" } }, v))))),
                    ...
                    ].map(s => (React.createElement("div", { key: s.l, style: { background: "#020409", border: "1px solid #0a1628",
                            borderRadius: 3, padding: "5px 7px" } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", marginBottom: 1 } }, s.l),
                        React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: s.c, fontFamily: "monospace" } }, s.v))))),
                    Object.keys(result.ec).length > 0 && (React.createElement("div", { style: { background: "#020409", border: "1px solid #0a1628",
                            borderRadius: 4, padding: "6px 8px", marginBottom: 6 } },
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".1em", marginBottom: 4 } }, "HATA DA\u011EILIMI"),
                        Object.entries(result.ec).map(([k, v]) => {
                            const m = ERR[k] || { c: "#64748b", l: k, i: "·" };
                            const tot = Object.values(result.ec).reduce((a, x) => a + x, 0) || 1;
                            return (React.createElement("div", { key: k, style: { marginBottom: 4 } },
                                React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 1 } },
                                    React.createElement("span", { style: { fontSize: 8, color: m.c } },
                                        m.i,
                                        " ",
                                        m.l),
                                    React.createElement("span", { style: { fontSize: 8, color: m.c, fontFamily: "monospace" } }, v)),
                                React.createElement("div", { style: { height: 2, background: "#0a1628", borderRadius: 1 } },
                                    React.createElement("div", { style: { height: "100%", background: m.c, width: `${(v / tot) * 100}%`,
                                            borderRadius: 1, boxShadow: `0 0 3px ${m.c}` } }))));
                        }))),
                    result.corrected > 0 && (React.createElement("div", { style: { fontSize: 8, color: "#10b981", marginBottom: 4, letterSpacing: ".04em" } },
                        "\u2713 ECC: ",
                        result.corrected,
                        " B\u0130T D\u00DCZELT\u0130LD\u0130")),
                    React.createElement("div", { style: { fontSize: 9, fontWeight: 700, letterSpacing: ".06em",
                    ...
                    React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", marginTop: 3, fontFamily: "monospace" } },
                        "\uD83D\uDEF0 ",
                        result.sat,
                        " \u00B7 ",
                        result.satEl,
                        "\u00B0 EL"))),
                React.createElement("div", { style: { padding: "7px 10px", borderBottom: "1px solid #0a162888",
                        display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 } },
                    React.createElement("span", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em" } }, "CANLI KAYIT"),
                    React.createElement("button", { className: "nbtn", onClick: () => setLog([]), style: { fontSize: 8, color: "#1e3a5f", letterSpacing: ".08em" } }, "TEMIZLE")),
                React.createElement("div", { ref: logRef, style: { flex: 1, overflowY: "auto", padding: "5px 7px" } }, log.length === 0 ? (React.createElement("div", { style: { color: "#0a1628", fontSize: 9, textAlign: "center", marginTop: 50 } }, "\u2014 BEKLENIYOR \u2014")) : log.map(e => {
                    var _a;
                    return (React.createElement("div", { key: e.id, style: { padding: "2px 5px", borderRadius: 2, marginBottom: 1,
                            background: (_a = LB[e.type]) !== null && _a !== void 0 ? _a : "transparent",
                            borderLeft: LB[e.type] ? `2px solid ${LC[e.type] || "#334155"}33` : "none" } },
                        React.createElement("div", { style: { fontSize: 7, color: "#0a1628", fontFamily: "monospace" } }, e.ts),
                        React.createElement("div", { style: { fontSize: 9, color: LC[e.type] || "#334155", lineHeight: 1.4, fontFamily: "monospace" } }, e.text)));
                })),
                React.createElement("div", { style: { padding: "6px 9px", borderTop: "1px solid #0a162888", display: "flex", gap: 4, flexShrink: 0 } }, [["OK", "#10b981", "✓"], ["WARN", "#f59e0b", "⚠"], ["ERR", "#f43f5e", "✗"], ["SYS", "#7c3aed", "◉"]].map(([t, c, s]) => {
                    const n = log.filter(e => e.type === t).length;
                    return (React.createElement("div", { key: t, style: { flex: 1, textAlign: "center" } },
                        React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: n > 0 ? c : "#0a1628", fontFamily: "monospace" } }, n),
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f" } }, s)));
                })))),
        drillData && (React.createElement("div", { onClick: closeDrillDown, style: { position: "fixed", inset: 0, zIndex: 200,
                background: "#020409cc", backdropFilter: "blur(4px)",
                display: "flex", alignItems: "center", justifyContent: "center", padding: 20,
                animation: "drillBackdropIn .18s ease-out" } },
            React.createElement("div", { onClick: e => e.stopPropagation(), style: { width: "min(720px,94vw)", maxHeight: "88vh", overflowY: "auto",
                ...
                React.createElement("div", { style: { padding: "16px 20px", borderBottom: "1px solid #0a162888",
                        display: "flex", alignItems: "center", gap: 12 } },
                    React.createElement("div", { style: { width: 34, height: 34, borderRadius: 8, flexShrink: 0,
                    ...
                    React.createElement("div", { style: { flex: 1, minWidth: 0 } },
                        React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: "#00d4ff", letterSpacing: ".04em" } },
                            drillData.node.id,
                            " \u2014 ",
                            drillData.node.label),
                        React.createElement("div", { style: { fontSize: 8, color: "#475569", letterSpacing: ".08em", marginTop: 2 } },
                            "M\u0130KRO KATMAN \u00B7 ",
                            (drillData.node.type || "node").toUpperCase(),
                            " \u00B7 ",
                            drillData.node.on ? "ÇEVRİMİÇİ" : "KARANTİNADA",
                            drillAutoOpened && " · 🛡️ WATCHDOG TARAFINDAN OTOMATİK AÇILDI")),
                    React.createElement("button", { onClick: closeDrillDown, style: { background: "none", border: "1px solid #33415577", color: "#94a3b8",
                            borderRadius: 5, width: 26, height: 26, cursor: "pointer", fontSize: 12, flexShrink: 0 } }, "\u2715")),
                drillData.anomaly && (React.createElement("div", { style: { margin: "12px 20px 0", padding: "8px 11px", borderRadius: 6,
                        background: "#3f151533", border: "1px solid #f43f5e44",
                        display: "flex", alignItems: "center", gap: 8 } },
                    React.createElement("div", { style: { width: 6, height: 6, borderRadius: "50%", background: "#f43f5e",
                            boxShadow: "0 0 6px #f43f5e", animation: "blink 0.6s ease-in-out infinite" } }),
                    React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: "#f43f5e" } },
                        "\uD83D\uDEE1\uFE0F WATCHDOG AKT\u0130F: ",
                        drillData.anomaly.type === "lockup" ? "THREAD KİLİTLENMESİ" : drillData.anomaly.type === "leak" ? "BELLEK SIZINTISI" : "YANITSIZ",
                        " ",
                        "\u2014 ",
                        drillData.anomaly.phase === "healing" ? "asenkron olarak yeniden başlatılıyor" : "karantinada, tanılanıyor"))),
                React.createElement("div", { style: { padding: "16px 20px", display: "grid", gap: 16 } },
                    React.createElement("div", null,
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 7 } }, "YA\u015EAMSAL BULGULAR"),
                        ...
                        ].map(m => (React.createElement("div", { key: m.l, style: { background: "#020409", border: "1px solid #0a1628", borderRadius: 4, padding: "7px 9px" } },
                            React.createElement("div", { style: { fontSize: 6, color: "#1e3a5f", marginBottom: 2 } }, m.l),
                            React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: m.c, fontFamily: "monospace" } }, m.v)))))),
                    React.createElement("div", null,
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 7 } }, "ALT B\u0130LE\u015EENLER"),
                        React.createElement("div", { style: { display: "grid", gap: 4 } }, drillData.submodules.map(sm => (React.createElement("div", { key: sm.name, style: { display: "flex", alignItems: "center", gap: 8,
                                background: "#020409", border: "1px solid #0a1628", borderRadius: 4, padding: "6px 9px" } },
                            React.createElement("div", { style: { width: 5, height: 5, borderRadius: "50%", flexShrink: 0,
                                    background: drillData.anomaly ? "#f43f5e" : "#10b981",
                                    boxShadow: `0 0 4px ${drillData.anomaly ? "#f43f5e" : "#10b981"}` } }),
                            React.createElement("span", { style: { fontSize: 9, color: "#94a3b8", fontWeight: 700, flexShrink: 0 } }, sm.name),
                            React.createElement("span", { style: { fontSize: 8, color: "#334155", marginLeft: "auto", textAlign: "right" } }, sm.detail)))))),
                    React.createElement("div", null,
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 7 } },
                            "A\u00C7IK ASENKRON SOKETLER (",
                            drillData.sockets.length,
                            ")"),
                        drillData.sockets.length === 0 ? (React.createElement("div", { style: { fontSize: 8, color: "#334155", padding: "6px 2px" } }, "Bu d\u00FC\u011F\u00FCme ba\u011Fl\u0131 fiziksel hat yok.")) : (React.createElement("div", { style: { display: "grid", gap: 4 } }, drillData.sockets.map(sk => {
                            const sc = sk.state === "CLOSED" ? "#f43f5e" : sk.state === "CONGESTED" ? "#f59e0b" : "#10b981";
                            return (React.createElement("div", { key: sk.peer, style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap",
                                    background: "#020409", border: `1px solid ${sc}22`, borderRadius: 4, padding: "6px 9px", fontFamily: "monospace" } },
                                React.createElement("span", { style: { fontSize: 8, color: sc, fontWeight: 700, flexShrink: 0 } }, sk.state),
                                React.createElement("span", { style: { fontSize: 8, color: "#64748b" } },
                                    "ws://",
                                    drillData.node.id,
                                    ":",
                                    sk.port,
                                    " \u2194 ",
                                    sk.peer),
                                React.createElement("span", { style: { fontSize: 7, color: "#1e3a5f", marginLeft: "auto" } },
                                    sk.km,
                                    "km \u00B7 y\u00FCk %",
                                    (sk.load * 100).toFixed(0))));
                        })))),
                    React.createElement("div", null,
                        React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", letterSpacing: ".12em", marginBottom: 7 } },
                            "THREAD HAVUZU ANLIK DURUMU (",
                            drillData.poolSize,
                            " worker)"),
                        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(90px,1fr))", gap: 6 } }, drillData.threads.map(th => {
                            ...
                            return (React.createElement("div", { key: th.id, style: { background: "#020409", border: `1px solid ${tc}33`, borderRadius: 4,
                                    padding: "6px 7px", textAlign: "center" } },
                                React.createElement("div", { style: { fontSize: 7, color: "#1e3a5f", marginBottom: 3 } },
                                    "Worker-",
                                    th.id),
                                React.createElement("div", { style: { width: 6, height: 6, borderRadius: "50%", background: tc, margin: "0 auto 3px",
                                        boxShadow: `0 0 4px ${tc}`,
                                        animation: (th.state === "BLOCKED" || th.state === "NO_RESPONSE") ? "blink 0.7s ease-in-out infinite" : "none" } }),
                                React.createElement("div", { style: { fontSize: 6, color: tc, fontWeight: 700 } }, label)));
                        })))))))));
}
